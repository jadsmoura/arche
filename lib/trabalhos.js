...
/* O NOME DO ARQUIVO ENTREGA O AUTOR (achado de set/2026): o PDF gerado pelo
   sistema sai anonimizado, mas o anexo do autor chega ao revisor com o nome
   que ele deu ao arquivo — "TCC-ANA-PAULA-orientador-Carlos-Lima.pdf" desfaz
   a revisão cega antes de ela começar. O link continua abrindo (o revisor
   precisa do documento); o que sai é o nome, trocado pelo do trabalho. */
const nomeNeutro = (numero, name) => {
  const ext = /\.([A-Za-z0-9]{1,6})$/.exec(String(name || ""));
  return `${numero || "trabalho"} — arquivo enviado pelo autor${ext ? "." + ext[1].toLowerCase() : ""}`;
};
const versaoPublica = (v, { anonimo = false, numero = "" } = {}) => ({
  ...
  arquivo: v.arquivo ? { name: anonimo ? nomeNeutro(numero, v.arquivo.name) : v.arquivo.name,
    link: v.arquivo.link, size: v.arquivo.size } : null,
});
// o índice do `map` não pode chegar como opções: quem lê o histórico de versões
// é o autor e a gestão, e os dois veem o nome do arquivo como ele foi enviado
const versoesPublicas = (t) => (t.versoes || []).map((v) => versaoPublica(v));
...
export function paraRevisor(t, token) {
  ...
  return {
    ...
    estado: t.estado,
    versao: v ? versaoPublica(v, { anonimo: true, numero: t.numero }) : null,
