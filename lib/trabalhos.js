...
export function normalizarConfig(c = {}) {
  ...
  return {
    ...
    // o autor INDICA um revisor ao submeter (decisão do dono, set/2026: "esse
    // campo deve ser incluído dentro do trabalho submetido"); a comissão
    // decide se o convida, ou se decide direto
    pedeRevisor: c?.pedeRevisor !== false,
...
export function normasPadrao(cfg = {}) {
  ...
  b.push(`5. RESUMO\nUm único parágrafo, com no mínimo ${MIN_PALAVRAS_RESUMO} e no máximo ${c.maxPalavrasResumo} palavras, contemplando introdução, objetivo, metodologia, resultados e conclusões. Não inclua referências, tabelas ou figuras. O abstract (em inglês) é opcional.`);
...
export function configPublica(c, hoje, { cursos = [] } = {}) {
  ...
  return {
    ...
    pedeRevisor: cfg.pedeRevisor, exigeInscricao: cfg.exigeInscricao,
...
/** O revisor que o autor indica na submissão (nome, e-mail, instituição). */
export function normalizarRevisorIndicado(r = {}) {
  return { nome: txt(r?.nome, 120), email: baixo(r?.email), instituicao: txt(r?.instituicao, 120) };
}
...
export function validarSubmissao(cfg, d = {}, { cursos = [] } = {}) {
  ...
  // o revisor indicado: nome e e-mail, e não pode ser quem assina o trabalho
  if (c.pedeRevisor) {
    const rv = normalizarRevisorIndicado(d.revisor);
    if (!rv.nome || rv.nome.split(/\s+/).length < 2) faltas.push("o nome completo do revisor indicado");
    if (!RE_EMAIL.test(rv.email)) faltas.push("um e-mail válido do revisor indicado");
    else if ([...autores, orientador].some((p) => p.email && p.email === rv.email)) faltas.push("um revisor que não seja autor nem orientador do trabalho");
  }
...
export function novoTrabalho(cfg, d, { arquivo = null, agora = new Date().toISOString(), numero, conta = "" } = {}) {
  ...
  return {
    ...
    autores, orientador,
    revisorIndicado: normalizarRevisorIndicado(d.revisor),   // quem o autor sugeriu; a comissão decide se convida
...
export function paraAutor(t) {
  return {
    numero: t.numero, titulo: t.titulo, tituloEn: t.tituloEn, modalidade: t.modalidade, area: t.area, curso: t.curso,
    autores: t.autores, orientador: t.orientador, revisorIndicado: t.revisorIndicado || null,
