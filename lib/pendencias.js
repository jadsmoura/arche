 ...
 */
export function pendenciasDoPerfil(falta = [], { preCadastroDe = "" } = {}) {
  if (!falta.length) return [];
  const nomes = falta.map((f) => f.rotulo || f.campo || "").filter(Boolean);
  /* Cadastro PRÉ-PREENCHIDO pelo sistema (dos documentos do edital ou da lista
     da coordenação) e ainda não reivindicado: a linha diz EM NOME DE QUEM ele
     está, porque o e-mail pode ter sido informado por outra pessoa — e quem
     abre o portal e vê o nome de outro precisa saber que pode desmenti-lo. */
  const pre = t(preCadastroDe);
  return [{
    setor: "Seu cadastro", urgente: true, link: "/perfil/?completar=1",
    texto: pre ? `Confira o cadastro preparado em nome de ${pre} e complete o seu`
      : "Complete o seu cadastro para entrar nos setores",
    detalhe: (nomes.length ? `Falta: ${nomes.join(", ")}.` : "")
      + (pre ? " Se esse nome não é o seu, no perfil você troca pelos seus dados." : ""),
  }];
}
