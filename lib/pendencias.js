...
export function pendenciasIC(projetos = [], quem = {},
  ...
  for (const p of projetos) {
    ...
    if (orienta) {
      ...
      const aValidar = (p.relatorios || []).filter((r) => r.situacao === "enviado").length;
      if (aValidar) {
    ...
    if (meuAluno) {
      ...
      // o cadastro do contrato é do ALUNO: sem ele a bolsa não se paga
      if (meuAluno.bolsista && faltaNoCadastroDe) {
...
export function pendenciasICEM(registros = [], { exigidosDe, turmaDe } = {}) {
  ...
  for (const b of registros) {
    ...
    if (b.bolsa && b.bolsa !== "voluntario" && (b.faltaNoCadastro || []).length) {
      itens.push({ setor: "Pesquisa · ICEM", link: "/pesquisa/ic/", urgente: true,
