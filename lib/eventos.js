...
export const SLUGS_RESERVADOS = new Set(["credenciar", "index", "inscricao", "evento", "assistir"]);
...
export function numerosDoEvento(acao) {
  ...
  const porAtividade = (ev.programacao || [])
    ...
    .map((p) => ({ id: p.id, titulo: p.titulo || "", inscritos: marcaramA(p.id), presentes: presencasDe(p.id) }));
  return {
    inscritos: inscritos.length,
    presentes: inscritos.filter((i) => i?.presente === true).length,
    online,
