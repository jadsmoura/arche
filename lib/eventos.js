...
/* A TOLERÂNCIA DO CAMINHO DA INSCRIÇÃO (pedido do dono, set/2026: "se um
   usuário não inscrito abrir o QR code de presença, ele deve registrar
   presença e se inscrever ao mesmo tempo — fui levado à página de inscrição,
   mas minha presença não foi computada; tive que fazer novamente o
   processo"). O código rotativo vale a janela corrente e a anterior — dois
   minutos no padrão —, e preencher a ficha leva mais que isso: nome, CPF,
   telefone, a LGPD e, quase sempre, criar a conta e esperar o código por
   e-mail. Levado ao formulário, o código chegava SEMPRE vencido do outro
   lado, e a presença se perdia justamente no caso que o pedido descreve.
   Por isso o caminho da INSCRIÇÃO — e só ele — aceita o código por 20
   minutos. O que a folga concede é a presença no ato de uma inscrição NOVA,
   feita do zero (conta, cadastro completo, LGPD) dentro desses 20 minutos a
   partir da foto do telão: é o mesmo ato de sempre, partido em duas telas.
   A leitura comum (a pessoa já inscrita, o QR lido na hora) NÃO muda — é ali
   que a foto que circula no WhatsApp tem de morrer em minutos. */
export const TELAO_TOLERANCIA_INSCRICAO = 20 * 60;
 ...
 *
 * `tolerancia` (em segundos, padrão 0) estende a validade PARA TRÁS, nunca
 * para a frente: o código do futuro continua não valendo. Só o caminho da
 * inscrição a usa — ver acima.
 */
export function lerCodigoTelao(chaveQr, atividadeId, codigo, { agora = Date.now(), janela = TELAO_JANELA_PADRAO, tolerancia = 0 } = {}) {
  const folga = Math.max(0, Number(tolerancia) || 0) * 1000;
  ...
  if (tipo === "e") {
    ...
    if (tempo * 60000 < agora - folga) return { ok: false, motivo: "expirado", validoAte: new Date(tempo * 60000).toISOString() };
  ...
  // da janela não pode ser recusado pelo tempo que a rede levou. A tolerância
  // (quando há) recua mais algumas janelas; o código do FUTURO nunca vale.
  const atras = 1 + Math.ceil(folga / 1000 / seg);
  if (tempo <= idx && tempo >= idx - atras) return { ok: true, fase, tipo: "rotativo" };
  return { ok: false, motivo: tempo > idx ? "invalido" : "expirado" };
}

/* O PASSE DE PROJEÇÃO (pedido do dono, set/2026: "um botão pra gerar um link
   da projeção do QR, para enviar ao responsável pela projeção e ele poder
   abrir, sem a necessidade de logar no sistema"). Quem opera o som e o
   projetor num congresso raramente é quem organiza o evento, e exigir conta
   do portal para abrir uma tela que só mostra um QR é a burocracia que faz a
   coordenação projetar do próprio notebook a tarde inteira.

   O que o passe dá é EXATAMENTE a projeção: o código da atividade, renovado
   a cada janela. Isso é poder de verdade — quem o tem pode gerar código
   válido enquanto ele durar, e dar presença a quem não está na sala. Daí as
   quatro travas: ele VENCE numa hora escolhida por quem o gera (é a mesma
   régua do QR estático, e sem ela o link circularia para sempre); vale para
   UMA atividade (o mesmo HMAC por atividade dos códigos); é REVOGÁVEL em
   bloco (`telaoPasseVersao` entra na assinatura — a gestão a incrementa e
   todo passe emitido morre, como o `AV_LINK_VERSAO` do avaliador); e não
   abre nada além da tela: o servidor devolve nome, atividade, o código e as
   CONTAGENS, nunca a lista de inscritos.

   Ele NÃO carrega fase: quem projeta a entrada projeta a saída, e a fase é
   escolha da tela. E o `tipo` da assinatura é outro ("proj"), então um passe
   nunca vale como código de presença nem o contrário — `lerCodigoTelao` só
   aceita prefixo `r`/`e`, e este nasce com `p`. */
const assinarPasse = (chaveQr, aid, min, versao) =>
  crypto.createHmac("sha256", String(chaveQr))
    .update(`${aid}|proj|${min}|${versao}`).digest("hex").slice(0, 16);

export const versaoDoPasse = (evento) => Math.trunc(Number(evento?.telaoPasseVersao)) || 0;

/** O passe válido até `validoAte` (ISO ou ms). Nunca no passado. */
export function passeDeProjecao(chaveQr, atividadeId, validoAte, { versao = 0, agora = Date.now() } = {}) {
  const ms = typeof validoAte === "number" ? validoAte : Date.parse(validoAte || "");
  if (!Number.isFinite(ms) || ms <= agora || !chaveQr || !atividadeId) return null;
  const min = Math.ceil(ms / 60000);
  const v = Math.trunc(Number(versao)) || 0;
  return {
    passe: `p${min.toString(36)}.${v.toString(36)}.${assinarPasse(chaveQr, atividadeId, min, v)}`,
    validoAte: new Date(min * 60000).toISOString(),
  };
}

/** Lê o passe: { ok: true, validoAte } ou { ok: false, motivo }. */
export function lerPasseDeProjecao(chaveQr, atividadeId, passe, { versao = 0, agora = Date.now() } = {}) {
  const m = /^p([0-9a-z]{1,12})\.([0-9a-z]{1,8})\.([0-9a-f]{16})$/.exec(String(passe || "").trim().toLowerCase());
  if (!m || !chaveQr || !atividadeId) return { ok: false, motivo: "invalido" };
  const min = parseInt(m[1], 36), v = parseInt(m[2], 36);
  if (!Number.isFinite(min) || !Number.isFinite(v)) return { ok: false, motivo: "invalido" };
  // a versão do passe tem de ser a VIGENTE: incrementá-la invalida em bloco
  if (v !== (Math.trunc(Number(versao)) || 0)) return { ok: false, motivo: "revogado" };
  const esperado = Buffer.from(assinarPasse(chaveQr, atividadeId, min, v));
  const dado = Buffer.from(m[3]);
  let bate = false;
  try { bate = esperado.length === dado.length && crypto.timingSafeEqual(esperado, dado); } catch { bate = false; }
  if (!bate) return { ok: false, motivo: "invalido" };
  if (min * 60000 < agora) return { ok: false, motivo: "expirado", validoAte: new Date(min * 60000).toISOString() };
  return { ok: true, validoAte: new Date(min * 60000).toISOString() };
}
...
/* ------------------- de onde o participante vem ---------------------------
   Decisão do dono (set/2026): antes do campo do curso, a ficha pergunta se a
   pessoa é do UNIEGO. É o que permite a lista FECHADA para quem é daqui — o
   texto escrito à mão era o que fazia um curso aparecer em quatro grafias — e
   o campo livre para quem é de outra instituição, sem obrigá-lo a procurar a
   saída no fim de uma lista de doze cursos que não são dele.
   E responde o que o texto sozinho não responde: "Enfermagem" digitado por
   alguém da UFG é letra por letra igual ao da casa. A declaração desfaz o
   empate, e é o número de público externo que o relatório de extensão pede.
   VAZIO é resposta legítima: a inscrição feita antes desta pergunta, e a
   lista que a coordenação cola de uma planilha, não têm o que declarar. */
export const VINCULOS_INSCRITO = [
  { codigo: "uniego", rotulo: "do UNIEGO" },
  { codigo: "externo", rotulo: "de outra instituição" },
];
export const normalizarVinculoInscrito = (v) =>
  (VINCULOS_INSCRITO.some((x) => x.codigo === v) ? v : "");
export const rotuloVinculoInscrito = (v) =>
  VINCULOS_INSCRITO.find((x) => x.codigo === v)?.rotulo || "";

/** Até quantas pessoas ministram a MESMA atividade — mesa redonda, painel. */
export const MAX_PESSOAS_ATIVIDADE = 8;
/** E a quantos cursos ela pode se destinar (a instituição tem doze). */
export const MAX_CURSOS_ATIVIDADE = 20;

/**
 * Os cursos a que a atividade se destina. Mesma régua dos co-realizadores da
 * ação: fora do catálogo não entra, repetido não entra, e o que entra fica
 * com a GRAFIA DO CATÁLOGO — "engenharia civil" e "Engenharia Civil" não
 * podem virar dois cursos na etiqueta da página.
 */
export function cursosDaAtividade(valor, catalogo = []) {
  ...
  for (const bruto of Array.isArray(valor) ? valor : []) {
    ...
    if (permitido.size && !permitido.has(k)) continue;
    vistos.add(k);
    out.push(permitido.size ? permitido.get(k) : nome);
    if (out.length >= MAX_CURSOS_ATIVIDADE) break;
  }
  return out;
}

/**
 * QUEM MINISTRA a atividade — uma LISTA desde set/2026 ("mesa redonda tem
 * mais de uma pessoa, e preciso incluir mais de uma foto").
 *
 * COMPAT: a atividade antiga tinha UMA pessoa em quatro campos soltos
 * (`responsavel`, `instituicao`, `miniBio`, `foto`); sem `pessoas`, é deles
 * que a lista nasce. Sem NOME não há pessoa: um vínculo ou uma foto sem
 * ninguém a quem pertencer não vai à vitrine.
 */
function pessoasDaAtividade(p, usados, txt) {
  const bruta = Array.isArray(p?.pessoas) && p.pessoas.length
    ? p.pessoas
    : [{ nome: p?.responsavel, instituicao: p?.instituicao, miniBio: p?.miniBio, foto: p?.foto }];
  const out = [];
  for (const x of bruta) {
    const nome = txt(x?.nome, 120);
    if (!nome) continue;
    const id = ID_CURTO.test(String(x?.id || "")) && !usados.has(x.id) ? x.id : gerarIdCurto(usados);
    usados.add(id);
    out.push({
      id, nome,
      instituicao: txt(x?.instituicao, 120),
      miniBio: txt(x?.miniBio, 400),
      foto: imagemPequena(x?.foto, FOTO_MAX),
    });
    if (out.length >= MAX_PESSOAS_ATIVIDADE) break;
  }
  return out;
}
 ...
 *
 * `catalogo` são os cursos a que a atividade pode se destinar; vazio aceita
 * o que vier (é o caso das RE-normalizações, que não podem descartar o que
 * já está gravado só por não terem o catálogo à mão).
 */
export function normalizarProgramacao(lista, catalogo = []) {
  const txt = (v, max = 200) => String(v ?? "").trim().slice(0, max);
  const hora = (v) => { const s = txt(v, 5); return HORA_RE.test(s) ? s : ""; };
  const usados = new Set();
  return (Array.isArray(lista) ? lista : [])
    .map((p) => {
      /* o id da atividade entra em `usados` ANTES das pessoas: os dois vivem
         no mesmo espaço de ids (a foto de cada pessoa é servida pelo id dela,
         e a rota o procura em toda a programação) */
      const id = ID_CURTO.test(String(p?.id || "")) && !usados.has(p.id) ? p.id : gerarIdCurto(usados);
      usados.add(id);
      const pessoas = pessoasDaAtividade(p, usados, txt);
      const item = {
        id,
        ...
        // A quem esta atividade se destina (pedido do dono, set/2026: "para
        // facilitar o aluno a identificar se a programação foi pensada para
        // seu curso"). VAZIO quer dizer "para todos" — num congresso a maior
        // parte da grade é isso, e uma lista obrigatória faria o organizador
        // marcar os doze cursos em cada linha para dizer a mesma coisa.
        cursos: cursosDaAtividade(p?.cursos, catalogo),
        ...
        // QUEM MINISTRA é uma LISTA (pedido do dono, set/2026: "mesa redonda
        // tem mais de uma pessoa, e preciso incluir mais de uma foto"). Cada
        // pessoa tem id próprio — é por ele que a foto dela é servida —, e a
        // foto fica na configuração como a capa: pequena, cortada em quadrado
        // pelo navegador antes de subir, e nunca nos payloads.
        pessoas,
        /* COMPAT — `responsavel`, `instituicao`, `miniBio` e `foto` eram os
           campos de UMA pessoa, e continuam saindo DERIVADOS da lista: é por
           eles que o conteúdo programático do relatório, o verso do
           certificado e o cartão do setor leem quem ministra. São escrita do
           servidor, não do cliente — a lista é a única fonte. */
        responsavel: pessoas.map((x) => x.nome).join(" · ").slice(0, 200),
        instituicao: pessoas[0]?.instituicao || "",
        miniBio: pessoas[0]?.miniBio || "",
...
export function buscarPessoasDoPortal(contas, q, { limite = 8 } = {}) {
  ...
  return (contas || [])
    ...
    .map((c) => ({
      ...
      temMatricula: !!String(c.matricula || "").trim(),
    }));
}
/* Completa CPF, telefone e matrícula em branco com o cadastro da conta do
   MESMO e-mail. Cada campo se completa SOZINHO (achado do dono, set/2026: "acabei
   de cadastrar um monitor, e não puxou a matrícula do sistema"): a matrícula só
   entrava quando o CPF estava vazio — e como o CPF é o primeiro a ser
   completado, ela nunca entrava para quem tinha os dois no perfil. A planilha
   da AEE aceita um OU outro, mas o estudante tem os dois, e a matrícula é a
   chave que liga a ele os certificados de monitoria dos semestres antigos. */
export function completarPeloPortal(pessoa, perfil) {
  ...
  if (vazio(pessoa.matricula) && !vazio(perfil.matricula)) pessoa.matricula = String(perfil.matricula).trim().slice(0, 40);
  return pessoa;
}
