 ...
 */
/* `soOrientacao` (o caso Luana, set/2026): a conta que sai é o Gmail de UMA
   ESTUDANTE, que a professora usou por engano — se esse endereço constar como
   aluno em algum projeto, o vínculo é da estudante, e trocá-lo poria a
   professora no lugar dela. Move-se só o que é AUTORIA e ORIENTAÇÃO; o aluno
   fica como está, e a gestão é avisada de que ficou. */
export function fundirProjeto(p, de, para, { soOrientacao = false } = {}) {
  ...
  const orientador = p.orientador
    ...
    : p.orientador;
  const alunos = soOrientacao
    ? (p.alunos || [])
    : (p.alunos || []).map((a) => ({ ...a, email: conta(a.email, troca(a.email, de, para)) }));
  if (soOrientacao && (p.alunos || []).some((a) => baixo(a.email) === baixo(de)))
    avisos.push(`no projeto ${p.numero || p.id} a conta removida consta como ALUNO — o vínculo ficou como estava (é da estudante, não da orientação)`);
