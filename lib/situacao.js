...
export function situacaoDaAcao(acao, hoje = hojeLocalISO()) {
  ...
  if (ev) {
    ...
    /* "Aguardando validação" dizia DUAS coisas opostas: a proposta esperando
       aprovação e o relatório esperando encerramento (achado do dono,
       ago/2026: "tantas etapas que ficaram de versões anteriores que deixa
       tudo confuso"). Num selo lido pela PROPPEX para decidir o que fazer,
       isso é o pior defeito possível — os dois atos são de pontas opostas do
       processo. O selo passa a dizer O QUE espera validação. */
    if (enc === "solicitado")
      return saida("encerramento-em-validacao", "Encerramento aguardando validação",
  ...
  if (txt(a.status) === "registrada")
    return saida("encerrado", "Registrada e certificados emitidos",
      "O relatório final está encerrado e os certificados foram liberados.");
  if (entregue)
    return saida("encerramento-em-validacao", "Relatório aguardando validação",
