...
export function retrato() {
  ...
  return { nome: p.nome, rotulo: p.rotulo, configurado: p.configurado(), modo: p.modo(), webhook: !!p.segredoWebhook(), meios: p.meios(),
    ultimoErro: typeof p.ultimoErro === "function" ? p.ultimoErro() : null };
}

/** Testa a credencial do provedor vigente (adaptador sem teste próprio responde só se está configurado). */
export async function testar() {
  const p = ativo();
  if (typeof p.testar === "function") return p.testar();
  return { provedor: p.nome, ambiente: p.modo(), configurado: p.configurado(), ok: p.configurado(), erro: p.configurado() ? "" : "credencial ausente no ambiente do servidor" };
}
