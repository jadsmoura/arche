...
/* Cookie é texto que o visitante escolhe: um "%" solto faz decodeURIComponent
   lançar, e lá em cima quem chama é um middleware async — a promessa rejeitada
   derrubaria o processo inteiro, o portal fora do ar por uma requisição
   anônima. Cookie ilegível é cookie inválido, não acidente. */
export function decodificarCookie(v) {
  try { return decodeURIComponent(v); } catch { return String(v || ""); }
}
/* Comparação em BYTES: String.length conta unidades UTF-16, e um caractere
   multibyte passaria pela conferência de tamanho para estourar dentro do
   timingSafeEqual, que exige buffers iguais. */
function macConfere(mac, esperado) {
...
function verify(token) {
  ...
  if (!macConfere(mac, esperado)) return null;
  try {
...
export function lerSessao(req) {
  ...
  return raw ? verify(decodificarCookie(raw.slice(COOKIE.length + 1))) : null;
}
...
/**
 * Apaga a senha de uma conta. Usado na fusão de cadastros: senha é da CONTA,
 * não da pessoa — mover o hash para o outro e-mail daria a alguém uma senha
 * que ele nunca escolheu.
 */
export async function removerSenha(storage, email) {
  const e = String(email || "").toLowerCase();
  const senhas = await carregarSenhas(storage);
  if (!senhas[e]) return false;
  delete senhas[e];
  await storage.set(SENHAS_KEY, JSON.stringify(senhas));
  await storage.flush?.();
  return true;
...
// "eventos" (ago/2026): o ARCHÉ EV virou setor próprio — a coordenação do
// módulo opera TODOS os eventos sem herdar a gestão da Extensão inteira
// "espacos" (ARCHÉ ES) entrou em ago/2026: quem coordena o módulo é a
// responsável pela reserva — recebe os pedidos, confirma o que está
// pré-autorizada a decidir e encaminha o resto à PROPPEX.
// "monitoria" (ARCHÉ MO) entrou em ago/2026: a reitoria trouxe o Programa de
// Monitoria Acadêmica, até 2025 conduzido pela Diretoria Acadêmica, para a
// PROPPEX — quem coordena o módulo analisa os projetos e homologa os
// relatórios, que é o que libera a certificação.
/* Os módulos que a gestão designa coordenação. `praticas` é o ARCHÉ AP
   (Aulas Práticas, da PROAC): quem o coordena é a **coordenação
   pedagógica**, que vê todos os cursos. O coordenador DE CURSO é outra
   figura, e vive no cadastro do próprio módulo (`ap-equipe-v1`) — aqui não
   caberia, porque este catálogo é por MÓDULO e não por curso. */
export const MODULOS = ["extensao", "pesquisa", "inovacao", "atas", "eventos", "espacos",
  "monitoria", "praticas",
  // a GESTÃO da Avaliação Institucional (ARCHÉ AV, set/2026): quem a
  // recebe alcança os doze cursos nos indicadores e na produção docente —
  // é o que a PROAC precisa; a coordenação de CURSO vem da composição dele
  "avaliacao"];
...
export const FUNCOES = [
  // o aluno indicado para a IC também tem conta no portal: sem esta linha
  // ele ficava preso na etapa de completar o perfil, tendo de se declarar
  // "professor" ou "outro" para chegar ao próprio projeto
  { codigo: "aluno", nome: "Estudante (aluno de graduação)" },
  /* O BOLSISTA DO ICEM É ESTUDANTE DO ENSINO MÉDIO (set/2026). Sem esta linha
     ele tinha de se declarar "aluno de graduação" — que não é — e, pior, o
     perfil passava a lhe cobrar MATRÍCULA e CURSO do UNIEGO, dois dados que
     ele não tem. A etapa de completar o cadastro virava uma parede: ele
     inventava um curso para conseguir passar dela. */
  { codigo: "em", nome: "Estudante do Ensino Médio (ICEM)" },
...
/** As funções que são COORDENAÇÃO DE PRÓ-REITORIA (PROPPEX e PROAC): quem as
 *  declara no perfil tem, por decisão do dono (set/2026), acesso livre à
 *  Avaliação Institucional — todos os cursos, como a gestão. A lista sai do
 *  próprio catálogo (as entradas com `orgao`), para não haver duas. */
export const FUNCOES_PRO_REITORIA = FUNCOES.filter((f) => f.orgao).map((f) => f.codigo);
export const ehCoordenacaoDeProReitoria = (funcao) =>
  FUNCOES_PRO_REITORIA.includes(normalizarFuncao(funcao));

/* ------------------------- perfil completo ------------------------------ */
/**
 * O que o portal PRECISA saber de cada pessoa. Não é burocracia: cada campo
 * aqui é usado em algum lugar do sistema, e sem ele alguma coisa deixa de
 * funcionar para o próprio dono do cadastro.
 *   nome     — sai nos documentos (ata, certificado, resultado do edital);
 *   funcao   — decide o que a pessoa faz na instituição;
 *   curso    — é o recorte de quase todo lugar (uploads, atas, relatórios);
 *   cpf      — a chave que liga os projetos vindos de fora e os certificados
 *              de edições anteriores. Sem ele a pessoa não encontra o que é
 *              dela (foi o que aconteceu com os importados do edital);
 *   telefone — como a PROPPEX fala com quem submete quando algo trava.
 * A TITULAÇÃO entra só para quem é docente: o edital cobra titulação mínima
 * por modalidade (item 4.4), e sem ela a proposta não se enquadra. Secretaria
 * e "outro" não têm por que informar.
 */
export const CAMPOS_OBRIGATORIOS = [
  { campo: "nome", rotulo: "nome completo" },
  { campo: "funcao", rotulo: "função na instituição" },
  { campo: "curso", rotulo: "curso ou departamento" },
  { campo: "cpf", rotulo: "CPF" },
  { campo: "telefone", rotulo: "telefone" },
];
const SEM_TITULACAO = ["aluno", "em", "secretaria", "outro", ""];
/* O ESTUDANTE DO ENSINO MÉDIO NÃO TEM CURSO NO UNIEGO (set/2026): ele está no
   2º ou no 3º ano de uma escola da cidade, e o campo "curso ou departamento"
   é o recorte dos setores da graduação. Cobrá-lo dele é exigência impossível
   de cumprir — a mesma razão pela qual o gestor geral não informa CPF —, e o
   resultado prático era o estudante escolher um curso qualquer para sair da
   etapa, sujando o recorte de todo mundo. O curso que ele PRETENDE cursar é
   outra coisa, e vive no registro do ICEM (`cursoInteresse`). */
const SEM_CURSO = ["em"];

/**
 * O que falta no perfil. Lista vazia = perfil completo.
 *
 * O CPF não se cobra do gestor geral: a conta pessoal da pró-reitoria é só de
 * gestão (a identidade acadêmica vive na institucional), e um CPF não pode
 * pertencer a duas contas — cobrar aqui deixaria a conta pessoal num beco sem
 * saída, com uma exigência impossível de cumprir.
 *
 * A MATRÍCULA entra para quem é ESTUDANTE (decisão do dono, ago/2026). Não é
 * burocracia nova: os históricos de monitoria dos semestres que correram fora
 * do ARCHÉ chegam em planilha da coordenação do curso, **sem CPF e sem
 * e-mail**, e a matrícula é o único dado único que eles têm. Sem ela o
 * certificado do aluno existe e ele não o encontra. `semMatricula` é a
 * exceção nominal do bolsista do ICEM: ele é estudante do ensino médio e não
 * tem matrícula no UNIEGO — cobrá-la seria exigência impossível de cumprir,
 * como o CPF do gestor geral.
 */
export function faltaNoPerfil(perfil, { gestorGeral = false, semMatricula = false } = {}) {
  const p = perfil || {};
  const funcao = normalizarFuncao(p.funcao);
  const falta = CAMPOS_OBRIGATORIOS
    .filter((c) => !(gestorGeral && c.campo === "cpf"))
    .filter((c) => !(SEM_CURSO.includes(funcao) && c.campo === "curso"))
    .filter((c) => !String(p[c.campo] ?? "").trim());
  /* O NOME É NOME DE GENTE (set/2026, inscritos de evento com o e-mail no lugar
     do nome): o perfil alimenta crachá, certificado, termo e ata — um e-mail
     ou uma palavra só ali sai impresso em documento. Nome preenchido que não
     é nome completo conta como faltando, e a etapa de completar o cadastro o
     aponta no próprio campo. */
  const nome = String(p.nome ?? "").trim();
  if (nome && (nome.includes("@") || nome.split(/\s+/).filter((x) => x.length >= 2).length < 2)
      && !falta.some((c) => c.campo === "nome")) {
    falta.push({ campo: "nome", rotulo: "nome completo (nome e sobrenome, sem e-mail)" });
  }
  if (!SEM_TITULACAO.includes(funcao) && !String(p.titulacao ?? "").trim()) {
    falta.push({ campo: "titulacao", rotulo: "titulação" });
  }
  if (funcao === "aluno" && !semMatricula && !String(p.matricula ?? "").trim()) {
    falta.push({ campo: "matricula", rotulo: "matrícula" });
  }
  return falta;
}
export const perfilCompleto = (perfil, opcoes) => faltaNoPerfil(perfil, opcoes).length === 0;
...
export function normalizarFuncao(v) {
  ...
  if (/(aluno|aluna|estudante|discente|graduand|bolsista)/.test(t)) return "aluno";
...
/** As duas contas da pró-reitoria: nunca se removem nem se fundem. */
export const ehGestorFixo = (email) => GESTORES_FIXOS.includes(String(email || "").trim().toLowerCase());
...
export async function carregarUsuarios(storage) {
  ...
  /* REMOVIDOS: a decisão da gestão de tirar alguém do portal (achado de
     ago/2026). Sem esta lista, "remover acesso" durava um login: a conta
     voltava a `pendente`, e `aprovarCadastroNovo` — que aprova todo cadastro
     novo — a reaprovava no acesso seguinte. O botão prometia uma coisa e
     fazia outra, e a PROPPEX não conseguia tirar ninguém.
     As duas contas da pró-reitoria nunca entram aqui, pelo mesmo motivo que
     nunca saem de `gestores`: seria trancar a instituição para fora. */
  u.removidos = (u.removidos || []).filter((e) => !ehGestorFixo(e));
  return u;
}
...
export function papelDe(email, usuarios) {
  const e = String(email || "").toLowerCase();
  // a remoção pela gestão vence tudo — inclusive o domínio institucional,
  // que sozinho aprova. Só um gestor geral a desfaz, em /usuarios/.
