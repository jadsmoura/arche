...
export function emailCadastroEM(b, turma, {
  ...
  faltaBanco = null, faltaResponsavel = null,
} = {}) {
  const entrada = linkEntrada(baseUrl, "/pesquisa/ic/", b?.email);
  ...
  const assunto = pedeBanco && pedeResp ? "complete o seu cadastro"
    : pedeBanco ? "informe os seus dados bancários" : "informe os dados do seu responsável";
  return {
    para: String(b?.email || "").trim(),
    ...
    corpoHtml: `
      <div style="font-family:Segoe UI,Roboto,sans-serif;max-width:640px">
        ...
        <p>Você está na turma <b>${esc(turma?.ciclo || "")}</b> do Programa de Iniciação Científica
          ...
          Falta ${pedeBanco && pedeResp ? "você informar duas coisas" : "uma informação sua"}:</p>

        ${pedeResp ? `<div style="background:#e6f5fa;border-left:4px solid #40717e;padding:12px 16px;border-radius:6px;margin-bottom:12px">
          <b>${pedeBanco ? "1 · " : ""}Os dados do seu responsável</b> — ${esc(faltaResponsavel?.length
        ...
        ${pedeBanco ? `<div style="background:${exigeBB ? "#fff7e6" : "#e6f5fa"};border-left:4px solid ${exigeBB ? "#d08700" : "#40717e"};padding:12px 16px;border-radius:6px;margin-bottom:12px">
          <b>${pedeResp ? "2 · " : ""}A conta em que a sua bolsa será paga</b> — banco, agência,
        ...
        <p style="color:#5b7280;font-size:13px">Entre com <b>este mesmo e-mail</b>
          (${esc(b?.email || "")}) e abra a guia <b>Ensino Médio</b>.</p>
        <p style="color:#5b7280;font-size:12px">Em caso de dúvidas, responda a este e-mail ou procure a coordenação de pesquisa.</p>
      </div>`,
  };
}
