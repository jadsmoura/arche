...
export function emailIsencaoDecidida(acao, inscrito, base = "https://arche.app.br") {
  ...
  return {
    para: inscrito.email,
    pessoal: true,
    assunto: `${ok ? "Isenção da taxa concedida" : "Pedido de isenção indeferido"} — ${p.nomeAtividade || "evento do UNIEGO"}`,
    corpoHtml: `
      <div style="font-family:Segoe UI,Roboto,sans-serif;max-width:620px">
        <h2 style="color:#1c3742;margin-bottom:4px">${ok ? "Isenção da taxa concedida" : "Pedido de isenção indeferido"}</h2>
        <p style="color:#5b7280;margin-top:0">Inscrição · PROPPEX / UNIEGO</p>
        <p>Olá${inscrito.nome ? `, <b>${esc(String(inscrito.nome).split(/\s+/)[0])}</b>` : ""}!</p>
        ${ok
