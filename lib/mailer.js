...
export function emailDadosBancariosEM(b, turma, {
  ...
  return {
    ...
    assunto: `[ARCHÉ IC] ${lembrete ? "Lembrete: informe" : "Informe"} os seus dados bancários — ICEM ${turma?.ciclo || ""}`,
    corpoHtml: `
      <div style="font-family:Segoe UI,Roboto,sans-serif;max-width:640px">
        <h2 style="color:#1c3742;margin-bottom:4px">Dados bancários — ICEM</h2>
        <p style="color:#5b7280;margin-top:0">ARCHÉ · PROPPEX / UNIEGO</p>
        <p>Olá, <b>${esc(b?.nome || "")}</b>!</p>
        ${blocoMensagem(mensagem)}
        <p>A coordenação de pesquisa concedeu a você a <b>${esc(nomeBolsa)}</b>${esc(valor)}
          na turma ${esc(turma?.ciclo || "")} do Programa de Iniciação Científica no Ensino Médio.</p>
        <p><b>Para receber o pagamento, falta você informar a sua conta</b> — banco, agência,
          conta e chave Pix. É rápido, e é você mesmo quem preenche: assim ninguém digita o
          número da sua conta por você.</p>
        ${exigeBB ? `<p style="background:#fff7e6;border-left:4px solid #d08700;padding:12px 16px;border-radius:6px">
          <b>Atenção — bolsa do CNPq:</b> a agência paga a bolsa <b>obrigatoriamente em conta do
          Banco do Brasil</b>, em seu nome. Se você ainda não tem, abra uma conta no BB (há conta
          gratuita para menores de idade, com o responsável) antes de preencher.
          Conta de outro banco não recebe o pagamento.</p>`
      : `<p style="background:#e6f5fa;border-left:4px solid #40717e;padding:12px 16px;border-radius:6px">
          A bolsa do UNIEGO é paga em <b>qualquer banco</b> — informe a conta que você já usa,
          em seu nome.</p>`}
        <p><a href="${entrada}" style="display:inline-block;background:#1c3742;color:#fff;text-decoration:none;
          padding:12px 22px;border-radius:10px;font-weight:600">Informar os meus dados bancários</a></p>
        <p style="color:#5b7280;font-size:13px">Entre com <b>este mesmo e-mail</b>
