  ...
  return {
    ...
    corpoHtml: `
      <div style="font-family:Segoe UI,Roboto,sans-serif;max-width:640px">
        ...
        <p>A PROPPEX analisou a proposta <b>${p.nomeAtividade || "(sem título)"}</b>
          (${acao.curso || "—"}) e a devolveu para ajustes. <b>Ela não foi recusada</b>:
        ...
        <div style="background:#fdf3e3;border-left:4px solid #b8863b;border-radius:8px;padding:12px 16px;margin:14px 0">
          <b>Motivo da devolução</b><br>${String(acao.motivoDevolucao || "").replace(/\n/g, "<br>")}
        </div>
 ...
 */
export function emailConviteEM(b, turma, { baseUrl = "https://arche.app.br" } = {}) {
 ...
 */
export function emailCobrancaRelatorioIC({ para, nome, papel, itens = [] }) {
 ...
 */
export function emailChamadaRelatorioEM(b, turma, { baseUrl = "https://arche.app.br", tipos = ["final"] } = {}) {
  const entrada = `${baseUrl}/entrar/?next=${encodeURIComponent("/pesquisa/ic/")}`;
    ...
    corpoHtml: `
      <div style="font-family:Segoe UI,Roboto,sans-serif;max-width:640px">
        ...
        <p>A PROPPEX pede que você preencha ${oQue} da sua participação no
          Programa de Iniciação Científica no Ensino Médio${prazo}. É rápido — três perguntas:</p>
        <ol>
