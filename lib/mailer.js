...
/* ======================================================================
   ...
   ====================================================================== */
const intervaloMs = () => Math.max(0, Number(process.env.MAIL_INTERVALO_MS ?? 500));
let ultimoEnvio = 0;         // instante da última mensagem entregue ao Gmail
...
export function retratoDoEnvio() {
  ...
  return {
    remetente: process.env.MAIL_FROM_ADDR || "jadsonbelem@gmail.com",
    configurado: !!(process.env.GDRIVE_OAUTH_REFRESH_TOKEN || process.env.GDRIVE_KEY_JSON || process.env.GDRIVE_KEY_FILE),
...
const ehCotaDiaria = (e) => /daily.*(quota|limit)|quota.*exceeded.*day/i.test(String(e?.message || ""));
...
function erroDeRitmo(ate, diaria = false) {
  const e = new Error(diaria
    ? "a conta de e-mail do sistema atingiu o limite DIÁRIO de envios do Gmail — o que faltou sai amanhã"
    : ate > Date.now()
      ? `o Gmail limitou o ritmo de envio desta conta — tente de novo a partir das ${hhmm(ate)}`
      : "o Gmail limitou o ritmo de envio desta conta — tente de novo em alguns minutos");
  e.ritmo = true;
  e.diaria = diaria;
  e.esperarAte = ate;
  return e;
...
/** Manda ao Gmail, repetindo enquanto a recusa for de ritmo e couber a espera.
    ...
    tentando entrar. É uma mensagem só, e o limite do Gmail é de instante:
    tentar e falhar é estritamente melhor que falhar sem tentar. */
export async function mandarComRitmo(gmail, bruto, opcoes = {}) {
  try {
    return await tentarNoGmail(gmail, bruto, opcoes);
  } catch (e) {
    contar("falhas", e);
    throw e;
  }
...
async function tentarNoGmail(gmail, bruto, { prioritaria = false } = {}) {
  let ultimo = null;
  for (let tentativa = 1; tentativa <= MAIL_TENTATIVAS; tentativa++) {
    ...
    /* O passo entre duas mensagens: é ele que mantém a conta abaixo do
       limite POR SEGUNDO do Gmail, que é o que derrubava os lotes. A vaga
    ...
    try {
      ...
      /* Teto do DIA: repetir não resolve, então sai na primeira. E NÃO se
         ...
         quem tenta entrar. Quem responde continua sendo o Gmail, numa
      ...
      console.warn(`[mailer] Gmail recusou por ritmo (tentativa ${tentativa}/${MAIL_TENTATIVAS}); espera ${Math.round(espera / 1000)}s`);
...
export async function enviarEmail({ para, assunto, corpoHtml, anexos, exato = false, prioritaria = false }) {
  ...
  await mandarComRitmo(gmail, bruto, { prioritaria });
  try {
