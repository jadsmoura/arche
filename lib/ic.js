...
export function resumir(p, u) {
  ...
  return {
    ...
    // Planilha em branco não é zero ponto: é planilha não preenchida. Passar
    // 0 daria uma NFC que o processo ainda não apurou — e ela sairia impressa
    // no resultado como se fosse nota.
    classificacao: papel === "gestao"
      ? notaClassificacao({
        notaPareceres: placar.media,
        pontuacaoProducao: Object.keys(p.producao || {}).length ? pontuarProducao(p.producao).total : undefined,
      })
      : undefined,
