...
/**
 * Quem DECIDE a seleção (aprovar, devolver, reprovar). Não confundir com dar
 * parecer: o parecer é o juízo de mérito, e ninguém julga o próprio projeto
 * (`podeDarParecer` continua barrando isso, sempre).
 *
 * A decisão é ato administrativo, e o gestor geral a pratica mesmo na própria
 * proposta (decisão do dono, ago/2026): no edital 01/2026 o mérito foi julgado
 * por parecerista AD HOC, fora do sistema — o pró-reitor não se avaliou, ele
 * registra o resultado. Sem isso, as propostas dele ficavam sem ninguém que
 * pudesse concluí-las e o edital não fechava. Fica marcado no histórico e no
 * próprio registro da decisão (`proprioProjeto`), que é o que torna o ato
 * defensável depois. Coordenador de módulo NÃO tem a prerrogativa: acima dele
 * há quem decida.
 */
export const podeAvaliar = (u, p) => {
  if (!p || !["submetido", "aprovado", "reprovado"].includes(p.status)) return false;
  const papel = papelNoProjeto(u, p);
  if (papel === "gestao") return true;
  return !!u?.gestorGeral && !!u?.gestao && !!papel;   // a própria proposta
};

/** A decisão está sendo tomada pelo gestor sobre proposta em que ele é parte? */
export const decidindoOProprio = (u, p) => podeAvaliar(u, p) && papelNoProjeto(u, p) !== "gestao";
...
export function visaoDoProjeto(p, u) {
  ...
  return {
    ...
    // proposta é autoria dela, e o aluno entrou depois da seleção.
    ...(papel === "orientador"
...
export function resumir(p, u) {
  ...
  return {
    ...
    // professor, e assim o quadro abre na hora, sem ida ao servidor
    ...(papel === "orientador" ? { avaliacaoRecebida: avaliacaoRecebida(p) } : {}),
