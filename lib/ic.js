...
export const STATUS = ["rascunho", "submetido", "devolvido", "aprovado", "concluido", "reprovado", "encerrado"];
export const ROTULO_STATUS = {
  ...
  // interrompido NO MEIO da vigência — não é "concluído" (o ciclo não se
  // completou) nem "reprovado" (o projeto foi aprovado e começou): o
  // desligamento do orientador, a desistência do aluno, o que a execução
  // não sobreviveu. Quem encerra é a PROPPEX, sobre pedido no relatório.
  encerrado: "Encerrado",
};
...
function normalizarAluno(a) {
  return {
    ...
    conta: t(a?.conta, 30),
    pix: t(a?.pix, 120),
    /* De quem é a mão que escreveu. O cadastro é do aluno e quase sempre é
       ele quem o preenche; quando não consegue — e-mail que não chega,
       celular que não colabora, prazo em cima —, a PROPPEX preenche EM NOME
       DELE (a mesma decisão da Avaliação, "editar em nome do docente"). Um
       cadastro que sustenta contrato e pagamento precisa dizer isso: some
       quando o próprio aluno grava depois, que é como deve ser. */
    ...(a?.cadastroPelaGestao?.em
      ? { cadastroPelaGestao: { por: email(a.cadastroPelaGestao.por), em: t(a.cadastroPelaGestao.em, 30) } }
      : {}),
  };
}
...
/* O QUE SÓ O PRÓPRIO ALUNO ESCREVE. É a lista que o formulário da orientação
   nunca toca — e é ela que responde à pergunta "o convite chegou?": nenhum
   destes campos existe sem alguém ter entrado no portal com aquele e-mail. */
export const CAMPOS_DO_ALUNO_PROTEGIDOS = ["cpf", "rg", "nascimento", "endereco",
  "vinculo", "vinculoOnde", "banco", "agencia", "conta", "pix"];

/**
 * O aluno indicado JÁ ENTROU no sistema? Diferente de `cadastroDoBolsistaCompleto`,
 * que pergunta se ele terminou: aqui basta um campo dele preenchido para se
 * saber que o convite chegou e a conta foi usada.
 *
 * É este sinal que separa CORRIGIR de TROCAR. Enquanto ninguém entrou, o
 * e-mail não abriu conta nenhuma e corrigi-lo não tira nada de ninguém —
 * é conserto de digitação. Depois que entrou, o endereço é a identidade de
 * quem já está no projeto, e trocá-lo é substituir alguém.
 */
export const bolsistaEntrou = (a = {}) =>
  CAMPOS_DO_ALUNO_PROTEGIDOS.some((c) => String(a?.[c] || "").trim() !== "");

/* Os campos da INDICAÇÃO — o que a orientação digitou e pode corrigir. Não
   se confundem com os do aluno acima: estes são o contato que o professor
   informou ao indicar, e sair errado é o defeito que trava o aluno na porta. */
export const CAMPOS_DA_INDICACAO = [
  ["nome", "nome"], ["email", "e-mail"], ["telefone", "telefone"],
  ["curso", "curso"], ["periodo", "período"],
];

/** O que mudou entre a indicação gravada e a corrigida — para o histórico
    dizer O QUE se corrigiu, e não só que alguém mexeu. */
export function camposDaIndicacaoAlterados(antes, depois) {
  if (!antes || !depois) return [];
  return CAMPOS_DA_INDICACAO
...
function normalizarRelatorio(r) {
  ...
  return {
    ...
    /* PEDIDO DE ENCERRAMENTO (decisão do dono, ago/2026): projeto pode ser
       interrompido no meio — orientador desligado da instituição, aluno que
       desiste, campo que se inviabiliza. Quem relata é o ALUNO, no relatório
       que estiver entregando (parcial OU final: a interrupção pode acontecer
       entre um e outro), com justificativa; quem decide é a PROPPEX, e não a
       orientação — no caso mais comum ela é justamente a parte ausente. */
    encerramento: {
      pedido: r?.encerramento?.pedido === true,
      motivo: t(r?.encerramento?.motivo, 4000),
      em: t(r?.encerramento?.em, 40),
      por: email(r?.encerramento?.por),
      decisao: umDe(r?.encerramento?.decisao, ["aceito", "recusado"], ""),
      parecer: t(r?.encerramento?.parecer, 4000),
      decididoPor: email(r?.encerramento?.decididoPor),
      decididoEm: t(r?.encerramento?.decididoEm, 40),
    },
    // a PROPPEX validou em nome da orientação (orientador desligado):
    // a marca sobrevive à normalização — é o que explica, depois, por que
    // a avaliação de desempenho pode estar em branco num final validado
    ...(r?.validadoPelaGestao === true ? { validadoPelaGestao: true } : {}),
  };
}

/** O pedido de encerramento que ainda espera a PROPPEX, se houver. */
export function pedidoEncerramentoPendente(p) {
  return (p?.relatorios || []).find((r) => r?.encerramento?.pedido && !r.encerramento.decisao) || null;
}
/** O encerramento ACEITO — o que explica um projeto com status "encerrado". */
export function encerramentoAceito(p) {
  return (p?.relatorios || []).find((r) => r?.encerramento?.decisao === "aceito") || null;
}
...
export function normalizarProjeto(bruto, { base = null, autor = null, agora = new Date(), grupos = null } = {}) {
  ...
  return {
    ...
    criadoEm,
    /* `autor` só carimba o projeto NOVO (varredura de set/2026): com `base`,
       o fallback ao autor fazia a GESTÃO virar orientação de todo projeto
       importado sem e-mail — bastava um "Salvar o cronograma" num dos 102
       projetos transcritos só com CPF, e o gestor passava a `orientador`
       (perdia parecer e correção), o professor de verdade nunca mais se
       vinculava pelo CPF (`vincularPorCpf` exige e-mail vazio) e os
       certificados da orientação iam para o gestor. Quem preenche o e-mail
       de um projeto já gravado é o vínculo pelo CPF, nunca quem salva. */
    criadoPor: base ? (base.criadoPor || "") : (email(autor) || ""),
    ...
    orientador: {
      ...
      email: email(b.orientador?.email) || base?.orientador?.email || (base ? "" : email(autor)),
    ...
    // o rastro das indicações removidas pela gestão (o que o aluno já tinha
    // digitado de seu): só a rota própria escreve — e sumia no primeiro
    // salvar seguinte, porque não estava nesta lista (varredura de set/2026)
    ...(base?.indicacoesRemovidas ? { indicacoesRemovidas: base.indicacoesRemovidas } : {}),
...
export function papelNoProjeto({ email: quem, cpf, gestao } = {}, p) {
  if (!p) return null;
  const orientaPorEmail = mesmo(quem, p.orientador?.email) || mesmo(quem, p.criadoPor);
  const orientaPorCpf = mesmoCpf(cpf, p.orientador?.cpf);
  const alunoPorEmail = (p.alunos || []).some((a) => mesmo(quem, a.email));
  /* O CPF gravado na ORIENTAÇÃO pode ser o do aluno (o caso real de set/2026:
     a aluna preencheu o formulário do edital e pôs o próprio CPF no campo da
     orientadora). Quando a orientação já tem OUTRO e-mail no projeto e a
     pessoa foi INDICADA como aluna por este e-mail, a indicação — deliberada,
     escrita pela orientação — vale mais que a coincidência do CPF: ela é
     aluna, e o CPF no campo errado é dado a corrigir, não um cargo. */
  if (alunoPorEmail && !orientaPorEmail && orientaPorCpf
    && p.orientador?.email && !mesmo(quem, p.orientador.email)) return "aluno";
  if (orientaPorEmail || orientaPorCpf) return "orientador";
  if (alunoPorEmail || (p.alunos || []).some((a) => mesmoCpf(cpf, a.cpf))) return "aluno";
...
/* ------------------- O ALUNO ESTÁ EM UM PROJETO SÓ ----------------------
   Pedido do dono (ago/2026): "não permita que mais de um professor indique o
   mesmo aluno; se o aluno já tiver sido indicado, ele trava para outra
   indicação". O caso que dói é a BOLSA — duas concessões à mesma pessoa,
   pagas em duplicidade —, mas a trava é da INDICAÇÃO inteira: bolsista num
   projeto e voluntário noutro seriam dois termos, dois cronogramas e dois
   relatórios da mesma pessoa no mesmo ciclo, e o edital prevê um plano de
   trabalho por acadêmico.

   Três recortes que a tornam justa:
   · por CICLO (o `edital` do projeto) — quem foi bolsista em 2025 pode ser
     indicado de novo em 2026; a trava não é uma condenação perpétua;
   · o projeto REPROVADO e o RASCUNHO não prendem ninguém: um não existe mais
     e o outro ainda não foi submetido;
   · a identidade é e-mail OU CPF, as mesmas chaves com que o setor reconhece
     a pessoa. Nome NÃO entra: dois homônimos travariam um ao outro, e aqui o
     falso positivo impede uma indicação legítima. */
const PRENDEM = new Set(["submetido", "aprovado", "concluido", "encerrado", "devolvido"]);

/** O projeto (do mesmo ciclo) em que este aluno já está — ou null. */
export function projetoQueJaTemOAluno(projetos, aluno, { exceto = "", edital = "" } = {}) {
  const email = String(aluno?.email || "").trim().toLowerCase();
  const cpf = soDigitos(aluno?.cpf || "");
  if (!email && !cpf) return null;            // sem chave forte não se trava ninguém
  for (const p of projetos || []) {
    if (!p || p.id === exceto) continue;
    if (!PRENDEM.has(String(p.status || ""))) continue;
    if (edital && editalDe(p) !== String(edital)) continue;
    const achado = (p.alunos || []).some((a) =>
      (email && mesmo(email, a.email)) || (cpf && mesmoCpf(cpf, a.cpf)));
    if (achado) return p;
  }
  return null;
}

/** A frase que a tela e a rota mostram quando a trava pega. */
export const motivoAlunoJaIndicado = (aluno, projeto) =>
  `${aluno?.nome || aluno?.email || "Este acadêmico"} já consta no projeto `
  + `${projeto?.numero || "(sem protocolo)"} — ${projeto?.titulo || ""}`.trim()
  + `, orientado por ${projeto?.orientador?.nome || projeto?.orientador?.email || "outro professor"}. `
  + "Cada acadêmico participa de UM projeto por ciclo: para indicá-lo aqui, a orientação do outro "
  + "projeto precisa liberá-lo antes (pela Substituição de bolsista, com a PROPPEX).";
...
/**
 * O CONTRÁRIO de `vincularPorCpf`: o pré-cadastro do edital nasceu num e-mail
 * que NÃO é da pessoa cujo nome e CPF ele carrega (o caso real de set/2026: o
 * formulário do edital 01/2026 foi preenchido pela ALUNA com o Gmail dela, e o
 * pré-cadastro da orientadora — nome, CPF, curso — nasceu na conta da
 * estudante; ao entrar, ela encontrava o perfil de outra pessoa com o CPF
 * travado, e "não achava onde colocar os dados"). Quando o titular do e-mail
 * informa OUTRO CPF, o vínculo que o pré-cadastro tinha escrito nos projetos
 * (e-mail da orientação e `criadoPor`, postos por `vincularPorCpf` a partir
 * do CPF do pré-cadastro) sai, e o projeto volta a esperar o titular do CPF —
 * que se vincula ao informá-lo no próprio perfil. `mantem(nome)` é a exceção
 * do mesmo nome: se quem grava É a pessoa do projeto (só o CPF do formulário
 * estava errado), o vínculo por e-mail fica. Altera a lista no lugar e
 * devolve os protocolos tocados.
 */
export function desligarPreCadastro(projetos, { email: mail, cpf, mantem = () => false, agora = new Date() } = {}) {
  const e = String(mail || "").trim().toLowerCase();
  const digitos = soDigitos(cpf);
  const tocados = [];
  if (!e || digitos.length !== 11) return { desligados: 0, projetos: tocados };
  const mesmoEmail = (v) => String(v || "").trim().toLowerCase() === e;
  for (let i = 0; i < (projetos || []).length; i++) {
    const p = projetos[i];
    let mudou = false;
    let novo = p;
    if (mesmoEmail(p.orientador?.email) && mesmoCpf(digitos, p.orientador?.cpf) && !mantem(p.orientador?.nome)) {
      novo = { ...novo, orientador: { ...novo.orientador, email: "" } };
      if (mesmoEmail(novo.criadoPor)) novo = { ...novo, criadoPor: "" };
      mudou = true;
    }
    const alunos = (novo.alunos || []).map((a) =>
      (mesmoEmail(a.email) && mesmoCpf(digitos, a.cpf) && !mantem(a.nome)) ? (mudou = true, { ...a, email: "" }) : a);
    if (mudou) {
      projetos[i] = anotar({ ...novo, alunos, atualizadoEm: agora.toISOString() }, {
        quem: e, agora,
        oQue: "pré-cadastro desligado: o titular deste e-mail informou outro CPF — o vínculo por "
          + "e-mail sai, e o projeto volta a esperar quem tem o CPF do cadastro",
      });
      tocados.push(novo.numero || novo.id || "");
    }
  }
  return { desligados: tocados.length, projetos: tocados };
}

/**
 * O e-mail "parece ser" da pessoa quando a parte antes do @ carrega TODAS as
 * palavras do nome (fora as de ligação e as muito curtas):
 * "saraaragaobarbosa0321" é de "Sara Aragão Barbosa"; não é de "Luana de
 * Miranda Santos". É evidência forte num sentido só — o e-mail que NÃO se
 * parece com o nome não diz nada (jbm@… pode ser de qualquer um).
 */
export function emailPareceDe(email, nome) {
  const local = String(email || "").toLowerCase().split("@")[0]
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z]/g, "");
  const tokens = String(nome || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .split(/\s+/).filter((t) => t.length >= 3 && !["dos", "das", "del", "van", "von"].includes(t));
  if (!local || tokens.length < 2) return false;
  return tokens.every((t) => local.includes(t));
}

/**
 * O e-mail do formulário do edital é de quem PREENCHEU o formulário — e nem
 * sempre foi a orientação. Quando ele se parece com o nome de um ALUNO da
 * própria submissão e não com o da orientação, é o aluno quem o preencheu, e
 * esse endereço não pode virar a identidade do professor (pré-cadastro,
 * convite para "informar o seu CPF", vínculo do projeto).
 */
export function emailDoFormularioEhDeAluno(p) {
  const email = String(p?.origem?.emailFormulario || "").trim().toLowerCase();
  if (!email.includes("@")) return false;
  if (emailPareceDe(email, p?.orientador?.nome)) return false;
  return (p?.alunos || []).some((a) => emailPareceDe(email, a?.nome));
}

/* ------------- CORREÇÃO DE TEXTO EM PROPOSTA JÁ SUBMETIDA ---------------
   Pedido do dono (ago/2026): a coordenação encontra erros de digitação e
   trechos trocados na leitura das propostas, e precisa poder corrigi-los —
   "somente para os gestores".

   A gestão sempre pôde gravar; o que não havia era o REGISTRO. Uma proposta
   que já foi submetida (e muitas vezes avaliada e aprovada) é documento do
   processo: mexer nela sem deixar rastro é o que torna impossível, meses
   depois, explicar por que o texto do projeto não é o que o parecerista leu.
   Daí a régua ser a lista dos campos que MUDARAM, e não um "editou a
   proposta" genérico — o histórico passa a dizer o que foi corrigido. */
const CAMPOS_DA_PROPOSTA = [
  ["titulo", "título"], ["resumo", "resumo"], ["palavrasChave", "palavras-chave"],
  ["objetivos", "objetivos"], ["justificativa", "justificativa"], ["metodologia", "metodologia"],
  ["resultadosEsperados", "resultados esperados"], ["referencias", "referências"],
  ["linhaPesquisa", "linha de pesquisa"], ["curso", "curso"], ["linha", "linha do edital"],
  ["modalidade", "modalidade"], ["edital", "edital"],
];
/** Proposta fechada: já saiu do rascunho — está em avaliação, execução ou fim. */
export const propostaFechada = (p) =>
  !!p && !["rascunho", "devolvido"].includes(p.status || "rascunho");
/** Os campos da proposta que mudaram, pelo nome que a tela lhes dá. */
export function camposDaPropostaAlterados(base, novo) {
  if (!base || !novo) return [];
  return CAMPOS_DA_PROPOSTA
    .filter(([c]) => String(base[c] ?? "") !== String(novo[c] ?? ""))
    .map(([, rot]) => rot);
}
...
/** Relatório é do aluno indicado — é ele quem envia. O ciclo em
 *  regularização reabre o envio mesmo com o projeto concluído; um ciclo
 *  concluído FORA de regularização está fechado (achado de ago/2026: o
 *  "concluido" de EM_ANDAMENTO abria a bandeira para os históricos, e a
 *  tela mostrava formulário que o servidor ia recusar). */
export function podeEnviarRelatorio(u, p) {
  return papelNoProjeto(u, p) === "aluno"
    && (p?.status === "aprovado" || !!regularizacaoDe(p));
}
...
export function alunosVisiveis(p, u) {
  ...
  // a gestão vê tudo — e também o que falta, para não ter de recalcular na tela
  if (papel === "gestao") {
    return alunos.map((a) => ({ ...a,
      dadosCompletos: cadastroDoBolsistaCompleto(a), faltaCadastro: faltaNoCadastroDoBolsista(a) }));
  }
  if (papel === "avaliador") {
  ...
  return alunos.map((a) => {
    ...
    return { ...a, cpf: "", rg: "", nascimento: "", endereco: "",
      ...
      /* E O QUE FALTA, por extenso (set/2026): "aguardando o aluno" diz que
         o cadastro não fechou e não diz o que pedir a ele. São NOMES DE
         CAMPO, não valores — o CPF e a conta continuam saindo em branco. */
      faltaCadastro: faltaNoCadastroDoBolsista(a),
      /* Se o aluno já ENTROU (ago/2026): a orientação precisa saber disso, e
         não tem como deduzi-lo — os campos que provam a entrada são
         justamente os que saem em branco aqui. Sem o sinal, o professor que
         digitou o e-mail errado fica sem resposta para a única pergunta que
         importa: "o convite chegou?". É ele também que decide se a correção
         do e-mail ainda é conserto ou já é substituição. */
      entrou: bolsistaEntrou(a) };
  });
}
...
export function visaoDoProjeto(p, u) {
  ...
  if (papel === "avaliador") {
    ...
    // contestações (trazem o e-mail de quem orienta) e fomento (revelam o
    // desfecho da seleção) também ficam de fora — achado da revisão
    // adversarial de ago/2026: sobravam no spread e desanonimizavam a visão
    const { relatorios, historico, avaliacao, orientador, alunos, notaDireta,
      criadoPor, origem, inclusaoManual, substituicoes, contestacoes, fomento, ...proposta } = p;
    return {
      ...proposta,
      orientador: null,
...
export function relatoriosDe(projetos, u) {
  ...
  for (const p of projetos || []) {
    ...
    for (const r of p.relatorios || []) {
      ...
      // o SIGILO CRUZADO vale aqui também (achado da revisão adversarial de
      // ago/2026): a lista achatada saía com o relatório CRU — o orientador
      // recebia a avaliação que o aluno fez dele, e o aluno, as notas e o
      // parecer da orientação sobre ele. O recorte é o mesmo de visaoDoProjeto.
      let rec = r;
      if (papel === "orientador") { const { avaliacaoOrientador, ...vis } = r; rec = vis; }
      else if (papel === "aluno") { const { avaliacaoAluno, parecerConclusivo, ...vis } = r; rec = vis; }
      linhas.push({
 ...
 */
/**
 * REGULARIZAÇÃO de ciclo encerrado (decisão do dono, ago/2026): o ciclo
 * 2025/2026 encerrou sem os relatórios no sistema, e a PROPPEX reabriu a
 * entrega — o PARCIAL e o FINAL de 01/2025, os dois ATÉ 31/10/2026. O
 * projeto segue "concluido" (o arquivo não mente); o que reabre são só as
 * janelas dos relatórios listados aqui. Depois do prazo o envio continua
 * aceito e marcado atrasado, como nas janelas normais.
 */
export const REGULARIZACAO_RELATORIOS = {
  // o ciclo 2025/2026 encerrou sem os relatórios no sistema: a PROPPEX
  // reabriu o PARCIAL e o FINAL (decisão do dono, ago/2026) para todo o
  // processo se regularizar por aqui — envio, validação e arquivo
  "01/2025": {
    parcial: { abre: "2026-08-16", vence: "2026-10-31" },
    final: { abre: "2026-08-16", vence: "2026-10-31" },
  },
};
export const regularizacaoDe = (p) =>
  (p?.status === "concluido" ? REGULARIZACAO_RELATORIOS[editalDe(p)] || null : null);
...
export function prazosRelatorios(p, hoje = hojeLocalISO()) {
  ...
  // …salvo o ciclo em REGULARIZAÇÃO, cujos relatórios listados reabrem
  if (p?.status !== "aprovado") {
    const reg = regularizacaoDe(p);
    if (!reg) return null;
    const pend = relatoriosPendentes(p);
    const prazos = Object.entries(reg).map(([tipo, j]) => {
      const falta = pend.some((r) => r.tipo === tipo);
      return { tipo, abre: j.abre, vence: j.vence, falta, atrasado: falta && hoje > j.vence };
    });
    return { prazos, atrasado: prazos.some((x) => x.atrasado), regularizacao: true };
  }
...
export function relatoriosPendentes(p) {
  const reg = regularizacaoDe(p);
  // ciclo CONCLUÍDO sem regularização está finalizado (decisão do dono,
  // ago/2026): os relatórios dos ciclos anteriores correram fora do sistema
  // e nada ali é pendência — o aluno de 2023 não deve ver "a entregar"
  if (p?.status !== "aprovado" && !reg) return [];
  // na regularização só contam os relatórios reabertos no catálogo;
  // um "validado" continua quitado como sempre
  const tipos = p?.status !== "aprovado" && reg ? Object.keys(reg) : TIPOS_RELATORIO;
  const faltando = [];
  for (const a of p.alunos || []) {
...
export function resumir(p, u) {
  ...
  return {
    ...
    // o pedido de encerramento aparece na lista da gestão: é decisão dela
    encerramentoPendente: !!pedidoEncerramentoPendente(p),
