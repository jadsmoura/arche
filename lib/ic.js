 ...
 *  regularização reabre o envio mesmo com o projeto concluído. */
export function podeEnviarRelatorio(u, p) {
  return papelNoProjeto(u, p) === "aluno"
    && (EM_ANDAMENTO.has(p.status) || !!regularizacaoDe(p));
}
 ...
 * 2025/2026 encerrou sem os relatórios finais, e a PROPPEX reabriu a
 * entrega — o artigo do relatório final da graduação pode ser enviado
 * ATÉ OUTUBRO/2026. O projeto segue "concluido" (o arquivo não mente);
 * o que reabre é só a janela do relatório listado aqui. Depois do prazo
 * o envio continua aceito e marcado atrasado, como nas janelas normais.
 */
...
export function relatoriosPendentes(p) {
  ...
  // na regularização só contam os relatórios reabertos (o final de 01/2025);
