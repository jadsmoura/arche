...
export function visaoDoProjeto(p, u) {
  ...
  if (papel === "avaliador") {
    ...
    const { relatorios, historico, avaliacao, orientador, alunos,
      notaDireta, criadoPor, origem, inclusaoManual, ...proposta } = p;
    return {
      ...proposta,
      orientador: null,
  ...
  const { avaliacoes, notaDireta, ...resto } = p;
...
export function prazosRelatorios(p, hoje = hojeLocalISO()) {
  if (!EM_ANDAMENTO.has(p?.status)) return null;
...
export function pendenciasDoProjeto(p) {
  ...
  // bolsa concedida pede a indicação completa do bolsista: sem CPF e dados
  // bancários não há como a PROPPEX montar o contrato
  if (p?.fomento && p.fomento.tipo !== "voluntario") {
    for (const a of (p.alunos || []).filter((x) => x.bolsista)) {
      ...
      if (falta.length) f.push({ tipo: "bolsista-incompleto", grave: false,
        texto: `${a.nome || "Bolsista"}: falta ${falta.join(", ")} na indicação do bolsista — sem isso o contrato da bolsa não sai.` });
    }
  }
