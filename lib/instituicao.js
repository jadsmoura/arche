...
/** A gravação da composição MESCLA com o que está gravado (achado da revisão
    adversarial de set/2026): a tela edita uma parte por vez — a dupla numa
    guia, NDE e Colegiado noutra — e a gravação era substituição total, então
    uma aba carregada antes de o NDE ser incluído o apagava ao salvar os
    dados do curso, sem aviso. Campo AUSENTE no corpo é "não mexi" e fica como
    está; campo presente (lista vazia inclusive) é decisão de quem gravou. */
export const CAMPOS_COMPOSICAO = ["coordenador", "pedagogico", "nde", "colegiado"];
export function mesclarComposicao(antes = {}, corpo = {}) {
  const base = normalizarComposicao(antes);
  const c = corpo && typeof corpo === "object" ? corpo : {};
  const out = {};
  for (const k of CAMPOS_COMPOSICAO) out[k] = c[k] !== undefined ? c[k] : base[k];
  return out;
}
...
/* ===================== O CURSO DIGITADO VIRA O DO CATÁLOGO =================
   Achado do dono (set/2026), abrindo o filtro "Curso" da lista de inscritos do
   CONINT: "tem vários cursos escritos de formas diferentes mas que estão na
   mesma categoria — o sistema tem colhido essas informações de forma escrita".
   Estava: "Agronomia-uniego", "Agronomia-UNIEGO", "Agronômia/Uniego";
   "Enfermagem", "Enfermagem uniego", "Enfermagem Uniego", "Enfermagem UNIEGO";
   "Ciências contábeis", "Ciências Contábeis", "Ciências Contábeis - UNIEGO",
   "Ciências Contábeis- UNIEGO" — quatro linhas para um curso só. O filtro não
   estava errado: ele agrupa pelo texto EXATO, e o texto era o que a pessoa
   digitou. Um curso partido em quatro linhas não filtra, não conta e não vira
   número de relatório.

   O campo é livre de propósito — a inscrição é PÚBLICA e o participante pode
   ser de fora ("Curso / instituição de origem"), então uma lista fechada
   trancaria justamente quem vem de outra instituição. O que se unifica é só o
   que é INEQUIVOCAMENTE um curso da casa.

   A régua compara o que se ESCREVE, não os bytes: sem acento (a "Agronômia"
   do print é a mesma Agronomia), sem caixa, com pontuação virando espaço (o
   "-", o "—" e o "/" que separam o curso do nome da instituição), e com os
   conectivos fora ("Engenharia de Software" = "engenharia software"). Só o
   marcador da PRÓPRIA instituição é descartado — `uniego` e `faceg`, os dois
   nomes que ela já teve. É isso que impede o falso positivo que custaria caro:
   "Enfermagem — UFG" e "Medicina Veterinária/UFU" NÃO viram curso da casa,
   porque a sigla que sobra não é nossa; ficam como a pessoa escreveu, que é o
   que o campo existe para guardar. */
const SO_DA_CASA = ["uniego", "faceg", "centro universitario evangelico de goianesia",
  "faculdade evangelica de goianesia"];
const CONECTIVOS = new Set(["de", "da", "do", "das", "dos", "e", "em"]);

/** O curso reduzido ao que ele diz: sem acento, sem caixa, sem pontuação. */
export const chaveDeCurso = (v) => String(v ?? "").trim().toLowerCase()
  .normalize("NFD").replace(/[̀-ͯ]/g, "")
  .replace(/[^a-z0-9]+/g, " ").trim();

/** A mesma chave sem os conectivos — "engenharia de software" = "engenharia software". */
const chaveSolta = (v) => chaveDeCurso(v).split(" ").filter((p) => p && !CONECTIVOS.has(p)).join(" ");

/**
 * O curso do catálogo que este texto nomeia — ou `null` quando não é um deles.
 * Olha o catálogo INTEIRO, o desativado inclusive: o curso que saiu dos
 * formulários novos continua tendo inscritos antigos, e o histórico deles não
 * deixa de merecer a unificação por causa disso.
 */
export function cursoDoCatalogo(valor) {
  const k = chaveDeCurso(valor);
  if (!k) return null;
  // "Enfermagem UNIEGO", "UNIEGO Enfermagem", "Enfermagem" — o marcador da
  // casa sai das duas pontas, e o que resta é o que se compara
  const semCasa = new Set([k]);
  for (const m of SO_DA_CASA) {
    if (k.endsWith(` ${m}`)) semCasa.add(k.slice(0, -(m.length + 1)).trim());
    if (k.startsWith(`${m} `)) semCasa.add(k.slice(m.length + 1).trim());
  }
  const exatas = [...semCasa].filter(Boolean);
  const soltas = new Set(exatas.map((x) => chaveSolta(x)).filter(Boolean));
  return CURSOS.find((c) => exatas.includes(chaveDeCurso(c.nome))
    // a SIGLA vale só na forma exata: "ADM" é o curso, "adm de empresas" não é
    || exatas.includes(chaveDeCurso(c.sigla))
    || soltas.has(chaveSolta(c.nome))) || null;
}

/**
 * O curso como ele deve ficar GRAVADO: a grafia do catálogo quando o texto
 * nomeia um curso da casa; o texto como veio, quando não nomeia. Vale em toda
 * porta por onde curso entra escrito — a inscrição pública, a lista que a
 * coordenação cola e o cadastro do usuário.
 */
export const unificarCurso = (valor) => cursoDoCatalogo(valor)?.nome || String(valor ?? "").trim();
