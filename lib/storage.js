...
import { mesclarEstado } from "./mesclarEstado.js";
...
/* `ctx` ({ drive, rootId }) é injetável para o teste subir DUAS instâncias
   sobre o mesmo Drive falso — é assim que a corrida do deploy se reproduz. */
export async function initDriveStore(ctx) {
  const { drive, rootId } = ctx || await (await import("./files.js")).driveContext();
  const FILE = "_estado.json";
  let fileId = null;
  let cache = {};
  /* A CORRIDA DO DEPLOY (set/2026): cada instância lê o arquivo UMA vez e
     escreve o arquivo INTEIRO da memória. No deploy as duas convivem, e o que
     a antiga grava enquanto a nova arranca some na primeira escrita da nova.
     A defesa: guardar a VERSÃO do arquivo que esta instância viu por último
     (`versaoVista`) e a foto que ela conhecia (`base`); antes de subir, se a
     versão no Drive é outra, alguém gravou no meio — relê e MESCLA a três
     (lib/mesclarEstado.js) em vez de cobrir. Custa um GET de metadados por
     upload; a releitura só acontece quando houve conflito. */
  let versaoVista = null;
  let base = {};
  const lerVersao = async () => {
    try { const m = await drive.files.get({ fileId, fields: "version" }); return m?.data?.version != null ? String(m.data.version) : null; }
    catch { return null; }
  };
  ...
  async function gravar() {
    ...
    try {
      ...
      /* O UPLOAD QUE FALHA NÃO PODE APAGAR A MARCA (varredura de set/2026):
         `sujo` ia a false ANTES de subir, e um erro de rede no meio deixava
         a gravação só na memória sem que nada a agendasse de novo — ela só
         subiria de carona na próxima escrita, e se a instância morresse
         antes (deploy, hibernação), morria com ela. A marca volta, e a
         próxima janela tenta de novo; quem chamou com `agora` continua
         recebendo o erro. */
      sujo = true;
      if (!timer) timer = setTimeout(() => { timer = null; flushing = flushing.then(gravar).catch((e2) => console.error("Erro ao regravar estado no Drive:", e2.message)); }, JANELA_MS);
      throw e;
    }
  }

  async function subir() {
    if (fileId && versaoVista != null) {
      const agora = await lerVersao();
      if (agora != null && agora !== versaoVista) {
        /* Outra instância gravou desde a última vez que esta viu o arquivo.
           A foto desta memória é tirada ANTES da releitura: o que mudar aqui
           enquanto o Drive responde (uma inscrição chegando) é reaplicado
           depois, senão a troca do cache o engoliria. */
        const local = { ...cache };
        const resp = await drive.files.get({ fileId, alt: "media" }, { responseType: "text" });
        const remoto = resp.data ? JSON.parse(resp.data) : {};
        const { estado, conflitos } = mesclarEstado({ base, local, remoto });
        for (const k of new Set([...Object.keys(cache), ...Object.keys(local)])) {
          if (cache[k] !== local[k]) { if (cache[k] === undefined) delete estado[k]; else estado[k] = cache[k]; }
        }
        cache = estado;
        console.warn(`[storage] o estado mudou no Drive desde a última leitura (versão ${versaoVista} → ${agora}): mesclado antes de gravar`
          + (conflitos.length ? ` — chaves alteradas dos dois lados: ${conflitos.join(", ")}` : ""));
      }
    }
    ...
    if (!fileId) {
      ...
      versaoNova = u?.data?.version;
    }
    versaoVista = versaoNova != null ? String(versaoNova) : await lerVersao();
    base = { ...cache };
  }
