/* ========================================================================
   ...
   ======================================================================== */
import { diaSerial, fimDoSemestre, hojeLocalISO, numeroDoSemestre, somaDias } from "./datas.js";
...
export const RITUAL = {
  ...
  // Pauta livre, sem checklist: comissões e órgãos que a instituição criou
  // não têm indicador do INEP a cumprir — o ARCHÉ só guarda as atas deles.
  COMISSAO: { ordinarias: 0, momentos: [] },
  OUTRO_CURSO: { ordinarias: 0, momentos: [] },
  OUTRO: { ordinarias: 0, momentos: [] },
};
...
/* ============================== PERIODICIDADE =========================== */
/* A conta do semestre civil vem de lib/datas.js — é a mesma do Relatório
   Semestral e a que faz o ciclo virar sozinho em 01/01 e 01/07. Aqui ela só
   veste o formato que as pautas usam ("2026-S2"). */
/** Semestre civil de uma data: "2026-S1" (jan–jun) ou "2026-S2" (jul–dez). */
export function janelaDe(iso) {
  const n = numeroDoSemestre(iso);
  return n === null ? null : `${String(iso).slice(0, 4)}-S${n}`;
}
export { numeroDoSemestre };
/** Último dia do semestre em que a data cai. */
export const fimDaJanela = (iso) => fimDoSemestre(iso);
...
/* ============================== CONFORMIDADE ============================ */
// Só ata REGISTRADA vale como prova: rascunho e minuta ainda podem mudar, e
// o avaliador vai pedir o documento pronto.
const VALE_COMO_PROVA = new Set(["registrada"]);
...
/**
 * Registros de um tema, do mais recente para o mais antigo.
 *
 * `ate` corta o que veio depois da data de referência. Importa ao lavrar ata
 * retroativa: uma sessão de março não pode aparecer com o tema "já cumprido"
 * porque o órgão o tratou em junho — o cumprimento é posterior à sessão que
 * se está registrando, e mostrá-lo como feito tira do responsável justamente
 * a caixa que ele precisa marcar. No uso corrente (`ate` = hoje) nada muda:
 * ata com data futura não existe.
 */
function registrosDaPauta(atas, pauta, { orgao, curso, ate = "" }) {
  const saida = [];
  for (const a of atasDoOrgao(atas, { orgao, curso, escopo: pauta.escopo })) {
    if (ate && String(a.sessao?.data || "") > ate) continue;
...
export function situacaoPautas(atas, { orgao, curso = "", hoje = hojeLocalISO() } = {}) {
  ...
  return pautasDoOrgao(orgao).map((p) => {
    const regs = registrosDaPauta(atas, p, { orgao, curso, ate: hoje });
...
/**
 * DOSSIÊ DE CONFORMIDADE — o documento que a PROPPEX entrega ao avaliador.
 *
 * A matriz da tela responde "quanto falta"; o dossiê responde a pergunta que
 * o avaliador do INEP faz de verdade: **onde está a ata que comprova este
 * indicador?**. Por isso cada tema sai com o número e a data da ata que o
 * registrou, e não só com um sinal verde — sinal verde ninguém protocola.
 *
 * `curso` vazio monta o dossiê INSTITUCIONAL (CONSU, CONSEPE, CPA, as
 * pró-reitorias e a Reitoria); com curso, o do curso (NDE e Colegiado).
 * Só ata REGISTRADA entra, porque só ela é documento pronto.
 */
export function dossieConformidade(atas, { curso = "", hoje = hojeLocalISO() } = {}) {
  const doCurso = !!curso;
  const orgaos = Object.entries(RITUAL)
    .filter(([cod, r]) => r.ordinarias > 0 && !!orgaoDe(cod)?.porCurso === doCurso)
    .map(([cod]) => {
      const ritual = ritualDoOrgao(atas, { orgao: cod, curso, hoje });
      const temas = situacaoPautas(atas, { orgao: cod, curso, hoje }).map((p) => ({
        id: p.id, titulo: p.titulo, refs: p.refs, porque: p.porque, evidencia: p.evidencia,
        cadencia: p.cadencia, momento: p.momento, momentoRot: p.momentoRot,
        estado: p.estado, exigidaAgora: p.exigidaAgora,
        // as provas: primeiro as do semestre em curso, depois o histórico —
        // um indicador tratado no semestre passado não está "em dia", mas
        // ter a ata à mão é o que evita o avaliador concluir que nunca houve
        provas: (p.historico || []).slice(0, 4),
        ultima: p.ultima || null,
      }));
      const exigidos = temas.filter((t) => t.exigidaAgora);
      return {
        orgao: cod, nome: orgaoDe(cod)?.nome || cod, sigla: orgaoDe(cod)?.sigla || cod,
        ritual, temas,
        exigidos: exigidos.length,
        comprovados: exigidos.filter((t) => t.estado === "em-dia").length,
      };
    })
    .filter((o) => o.temas.length || o.ritual.sessoes.length);

  const exigidos = orgaos.reduce((n, o) => n + o.exigidos, 0);
  const comprovados = orgaos.reduce((n, o) => n + o.comprovados, 0);
  const cursoInfo = doCurso ? CURSOS.find((c) => c.slug === curso) || { slug: curso, nome: curso } : null;
  return {
    escopo: doCurso ? "curso" : "institucional",
    curso, cursoNome: cursoInfo?.nome || "",
    janela: janelaDe(hoje), fimDaJanela: fimDaJanela(hoje), hoje,
    orgaos, exigidos, comprovados,
    percentual: exigidos ? Math.round((comprovados / exigidos) * 100) : 0,
    // total de atas registradas que sustentam o dossiê — é o que o avaliador
    // vai querer ver anexado
    atas: [...new Set(orgaos.flatMap((o) => [
      ...o.ritual.sessoes.map((s) => s.numero),
      ...o.temas.flatMap((t) => t.provas.map((p) => p.numero)),
    ]).filter(Boolean))],
  };
}
