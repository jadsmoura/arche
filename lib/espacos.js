...
/* Instituições de fora que usam os espaços com REGULARIDADE (pedido do dono,
   ago/2026, com o Colégio Couto): a linha genérica "Instituição ou empresa
   externa" resolve o pedido de uma vez só, mas apaga quem pediu — e a
   ocupação por órgão, que é o número levado ao conselho, não consegue dizer
   quanto do auditório foi de uma escola parceira e quanto foi de um evento
   avulso. Com código próprio, os pedidos dela se agrupam sozinhos e o
   histórico dela se lê numa linha.

   Preserve os `codigo`: são a chave do que já está reservado. Elas continuam
   sendo EXTERNAS para todos os efeitos — ofício obrigatório, inclusive. */
export const PARCEIROS_EXTERNOS = [
  { codigo: "colegio-couto", rotulo: "Colégio Couto" },
];

/** Órgãos que NÃO são da casa: aí o pedido precisa vir por ofício. */
export const ORGAOS_EXTERNOS = ["externo", "comunidade",
  ...PARCEIROS_EXTERNOS.map((p) => p.codigo)];
...
export function gruposDeOrgao(cursos = []) {
  return [
    ...
    // as parceiras de fora ficam num grupo próprio: uma escola no meio de
    // Reitoria e Biblioteca leria como setor da casa, e ela não é
    { grupo: "Instituições parceiras", opcoes: PARCEIROS_EXTERNOS },
  ];
}
...
export function normalizarOficio(bruto) {
  ...
  // só vale o que SUBIU pela rota do sistema (`/api/files/<id>`, o que
  // `POST /api/espacos/oficio` devolve): um link digitado satisfazia o ofício
  // "obrigatório" da parceira externa sem documento nenhum (set/2026)
  if (!link || !/^\/api\/files\/[^/?#]+/.test(link)) return null;
  return {
...
export function validarReserva(r, { espacos = [], hoje = hojeLocalISO(), cursos = null } = {}) {
  ...
  // "termina antes de começar" só é erro DENTRO do mesmo dia. Com data de
  // término posterior, das 18h de um dia às 9h do outro é uma reserva que
  // atravessa a noite — e era exatamente o que a regra antiga recusava,
  // porque comparava as horas sem olhar o dia (achado de uma professora,
  // ago/2026). Ver `atravessaMeiaNoite`.
  else if (minutos(r.horaFim) <= minutos(r.horaInicio) && !atravessaMeiaNoite(r))
    erros.push("O término tem de ser depois do início.");
...
/* ------------------------------- conflitos ------------------------------
   ...
   COMO O TEMPO DE UMA RESERVA É LIDO. Há duas coisas diferentes que um
   pedido de vários dias pode querer dizer, e o sistema precisa das duas:

   1. **Janela que se repete a cada dia** — o congresso de 19 a 30/10, das
      8h às 18h. Não é o auditório ocupado 24 horas por doze dias: é a mesma
      faixa, todo dia, e a noite fica livre para outro. É o caso comum, e é
      o que o módulo sempre fez.
   2. **Ocupação contínua que atravessa a noite** — das 18h do dia 1 às 9h
      do dia 2. Não havia como dizer isto: a validação comparava só as horas
      e recusava com "o término tem de ser depois do início", sem reparar
      que o término era no dia seguinte.

   O que separa uma da outra é o próprio pedido: com mais de um dia, hora de
   término MENOR OU IGUAL à de início só pode ser a segunda leitura — como
   janela diária ela seria vazia, isto é, não significaria nada. Por isso a
   travessia se reconhece sozinha, sem uma caixa a mais no formulário.

   Reconhecida a travessia, a ocupação é UM bloco contínuo do primeiro
   instante ao último — inclusive os dias inteiros do meio, se houver. É o
   lado seguro: uma reserva de três noites reserva também os dois dias entre
   elas. Bloquear demais devolve um espaço que se pode liberar; bloquear de
   menos põe duas atividades na mesma sala.
   ------------------------------------------------------------------------ */

/**
 * A reserva atravessa a meia-noite? Só com mais de um dia e com o término
 * em hora que não vem depois do início.
 */
export function atravessaMeiaNoite(r) {
  if (!r || !RE_HORA.test(String(r.horaInicio)) || !RE_HORA.test(String(r.horaFim))) return false;
  const fim = r.dataFim && r.dataFim >= r.dataInicio ? r.dataFim : r.dataInicio;
  return fim > r.dataInicio && minutos(r.horaFim) <= minutos(r.horaInicio);
}

/**
 * O tempo que a reserva ocupa, em minutos absolutos (dia × 1440 + hora), como
 * uma lista de intervalos. É o que torna comparáveis os dois modelos acima:
 * a janela diária vira um intervalo POR DIA; a travessia, um só, contínuo.
 * Sem hora, o dia inteiro.
 */
export function janelasDaReserva(r) {
  const dias = diasDaReserva(r);
  if (!dias.length) return [];
  const temHora = RE_HORA.test(String(r.horaInicio)) && RE_HORA.test(String(r.horaFim));
  const hi = temHora ? minutos(r.horaInicio) : 0;
  const hf = temHora ? minutos(r.horaFim) : 24 * 60;
  if (atravessaMeiaNoite(r)) {
    return [{ ini: diaSerial(dias[0]) * 1440 + hi, fim: diaSerial(dias[dias.length - 1]) * 1440 + hf }];
  }
  return dias.map((d) => ({ ini: diaSerial(d) * 1440 + hi, fim: diaSerial(d) * 1440 + hf }));
}

/** Quantas horas a reserva ocupa ao todo — o número da ocupação. */
export function horasDaReserva(r) {
  return janelasDaReserva(r)
    .reduce((soma, j) => soma + Math.max(0, j.fim - j.ini), 0) / 60;
}
...
/** Os tempos se cruzam? Encostar não é sobrepor: quem termina às 10h e quem
    começa às 10h cabem no mesmo espaço. */
const cruzaHoras = (a, b) => {
  const ja = janelasDaReserva(a), jb = janelasDaReserva(b);
  return ja.some((x) => jb.some((y) => x.ini < y.fim && y.ini < x.fim));
};
