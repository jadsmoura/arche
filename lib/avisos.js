...
export const AVISOS = [
  ...
  { codigo: "ev-inscricao-pagamento", setor: "eventos", destino: "pessoa", critico: true,
    rotulo: "Inscrição recebida — aguardando pagamento",
    texto: "Em evento pago, quem se inscreve recebe o link para pagar e o prazo da reserva da vaga; a credencial só sai depois do pagamento.",
    seDesligar: "A pessoa fica só com o link da tela para pagar; se fechar a aba, perde o caminho." },
  { codigo: "ev-pagamento-divergente", setor: "eventos", destino: "gestao",
    rotulo: "Pagamento com valor diferente do devido",
    texto: "O provedor confirmou um pagamento menor que o valor da inscrição — a coordenação decide se aceita, isenta a diferença ou estorna.",
    seDesligar: "A inscrição fica aguardando sem ninguém saber por quê." },
  /* Isenção da taxa pelo CadÚnico (set/2026) */
  { codigo: "ev-isencao-recebida", setor: "eventos", destino: "pessoa", critico: true,
    rotulo: "Pedido de isenção recebido (ao candidato)",
    texto: "Confirma que o pedido entrou e que a vaga fica reservada durante a análise — ou, quando o próprio comprovante nega o critério, diz por que não foi atendido.",
    seDesligar: "Quem pede isenção não sabe se o pedido chegou, e quem foi indeferido na hora descobre só quando o prazo de pagamento vencer." },
  { codigo: "ev-isencao-gestao", setor: "eventos", destino: "gestao",
    rotulo: "Pedido de isenção aguardando decisão",
    texto: "Avisa eventos@uniego.edu.br de que há pedido na fila, com a leitura do comprovante e o que precisa ser conferido à mão.",
    seDesligar: "A coordenação só vê a fila abrindo a guia Isenção da taxa — e o prazo do candidato fica parado esperando." },
  { codigo: "ev-isencao-decidida", setor: "eventos", destino: "pessoa", critico: true,
    rotulo: "Decisão sobre o pedido de isenção (ao candidato)",
    texto: "Deferida ou indeferida, com o motivo. Deferida, a credencial com o QR sai na mensagem seguinte.",
    seDesligar: "A pessoa não fica sabendo da decisão nem do novo prazo para pagar." },
  /* ARCHÉ TR — submissão de trabalhos com revisão cega (set/2026) */
  { codigo: "tr-recebido", setor: "eventos", destino: "pessoa", critico: true,
    rotulo: "Trabalho recebido (recibo ao autor)",
    texto: "O autor recebe o número do trabalho e o link de acompanhamento — a única chave dele para ver os pareceres e enviar a correção.",
    seDesligar: "O autor fica só com o link da tela de confirmação; se fechar a aba, perde o acesso ao próprio trabalho." },
  { codigo: "tr-convite-revisao", setor: "eventos", destino: "pessoa", critico: true,
    rotulo: "Convite ao revisor (link do parecer)",
    texto: "Cada revisor designado recebe o link pessoal do parecer; o lembrete usa o mesmo aviso.",
    seDesligar: "Ninguém recebe o convite — a avaliação para, porque o link só existe no e-mail." },
  { codigo: "tr-decisao", setor: "eventos", destino: "pessoa", critico: true,
    rotulo: "Decisão sobre o trabalho (ao autor)",
    texto: "Aceito, devolvido para correção ou rejeitado — com a mensagem da comissão e o link dos pareceres.",
    seDesligar: "O autor não fica sabendo da decisão nem do prazo de correção." },
  { codigo: "tr-gestao", setor: "eventos", destino: "gestao",
    rotulo: "Movimentação dos trabalhos (à coordenação do evento)",
    texto: "Trabalho novo, versão corrigida enviada e pareceres completos — cada um avisa eventos@uniego.edu.br.",
    seDesligar: "A coordenação só vê a fila abrindo a guia Trabalhos." },
  { codigo: "ev-comunicado", setor: "eventos", destino: "pessoa", critico: true,
    rotulo: "Comunicado da organização aos inscritos",
    texto: "A coordenação escreve na guia Inscritos e o texto vai por e-mail a quem está inscrito no evento — mudança de sala, adiamento, lembrete da véspera.",
    seDesligar: "O botão continua na tela e nenhum e-mail sai: a organização acha que avisou, e ninguém foi avisado." },
  ...
  { codigo: "ic-dados-bancarios", setor: "ic", destino: "pessoa", critico: true,
    rotulo: "Pedido do cadastro ao bolsista (graduação)",
    texto: "Concedida a bolsa, o aluno recebe o pedido do cadastro do contrato — nomeando o que "
      + "falta. A coordenação reenvia pelo botão da guia Bolsistas.",
    seDesligar: "A bolsa é concedida e ninguém avisa o aluno: sem o cadastro, a PROPPEX não tem "
      + "como efetivar o pagamento, e o atraso só aparece na folha." },
  { codigo: "ic-comunicado", setor: "ic", destino: "pessoa", critico: true,
    rotulo: "Comunicado da coordenação aos bolsistas e voluntários",
    texto: "O texto que a coordenação escreve e manda a todos os bolsistas e voluntários do ciclo "
      + "(ou da turma do ICEM), pelo botão das guias Bolsistas e Ensino Médio.",
    seDesligar: "A coordenação clica em enviar, o sistema diz que enviou e ninguém recebe — o aviso "
      + "de mudança de data ou de horário do CONINT some no caminho." },
  ...
  { codigo: "em-dados-bancarios", setor: "em", destino: "pessoa", critico: true,
    rotulo: "Pedido dos dados bancários ao bolsista do ICEM",
    texto: "Atribuída a bolsa, o estudante recebe o pedido da conta em que ela será paga — "
      + "com o aviso de que a do CNPq exige conta do Banco do Brasil. Sai uma vez por bolsa "
      + "concedida; a coordenação reenvia pelo botão da guia Ensino Médio.",
    seDesligar: "O bolsista não é avisado de que precisa informar a conta — e a bolsa dele "
      + "fica sem como ser paga." },
  ...
  { codigo: "ap-lembrete-extensao", setor: "praticas", destino: "pessoa", critico: true,
    rotulo: "Lembrete mensal da curricularização da extensão",
    texto: "Todo dia 1º, um e-mail por professor com as disciplinas que curricularizam extensão "
      + "e ainda não têm relatório no semestre.",
    seDesligar: "A hora extensionista da disciplina deixa de ser registrada, e é ela que comprova "
      + "os 10% que a Resolução CNE/CES nº 7/2018 exige do curso." },
  ...
  { codigo: "auth-juntar-contas", setor: "acessos", destino: "gestao",
    rotulo: "Pedido de junção de duas contas",
    texto: "Quem tem o CPF numa conta e está tentando usar outra pede, por um botão no perfil, "
      + "que a PROPPEX junte as duas.",
    seDesligar: "O pedido continua aparecendo em /usuarios/, ao lado do botão de juntar — o que "
      + "se perde é saber dele no dia em que a pessoa ficou travada." },
