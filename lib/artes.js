...
export function artesEmbutidas(evento) {
  ...
  for (const atv of evento?.programacao || []) {
    ...
    // quem ministra é uma LISTA desde set/2026: a foto é de cada pessoa, e o
    // id dela vive no mesmo espaço do id da atividade
    for (const q of atv?.pessoas || []) {
      if (q?.id && ehDataUrl(q.foto)) achadas.push({ onde: "foto", id: q.id, valor: q.foto });
    }
  }
...
export function aplicarReferencia(evento, { onde, id }, ref) {
  ...
  if (onde === "foto") {
    ...
    if (atv) { atv.foto = ref; return 1; }
    for (const a of evento.programacao || []) {
      const q = (a?.pessoas || []).find((x) => x?.id === id);
      if (q) { q.foto = ref; return 1; }
    }
    return 0;
  }
  if (onde === "logo") {
