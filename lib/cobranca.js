...
export const ACOES_KEY = "extensao-acoes-v1";        // somente leitura aqui
...
export function selecionar(acoes, { hoje, ledger, cfg, ignorarCorte = false }) {
  ...
  for (const a of Array.isArray(acoes) ? acoes : []) {
    ...
    if (dias < 1) continue;
    // 5. janela de elegibilidade e corte histórico
    if (dias > cfg.janelaDias) continue;
    if (corte && fim < corte) continue;
    // 6. registro: dispensada, já cobrada ou em retentativa
    const reg = ledger.acoes?.[a.id];
    if (reg?.dispensadaEm) continue;
    const envio = reg?.envios?.d1;
    if (envio) {
      const podeRetentar =
        (envio.status === "erro" && (envio.tentativas || 0) < 3) ||
        (envio.status === "enviando" && (envio.tentativas || 0) < 2);
      if (!podeRetentar) continue;
    }
    // 7. precisa haver a quem cobrar
    const para = destinatariosDaAcao(a);
    if (!para.length) { semEmail.push(resumoAcao(a)); continue; }
    alvos.push({ acao: a, dias, para, fim });
  }
...
export async function varrer({
  ...
  if (rodando) return rodando;              // já há uma rodada em voo
  rodando = (async () => {
    const t0 = Date.now();
    ...
    try {
      ...
      if (!sel.alvos.length) {
        ledger.lock = null;
        ledger.ultimaVarredura = new Date().toISOString();
        await gravarLedger(storage, ledger);
        return { ...base, modo: "nada-a-cobrar", ms: Date.now() - t0 };
      }
  ...
  })().finally(() => { rodando = null; ultimaVarredura = Date.now(); });
  return rodando;
}

/** Resumo à PROPPEX. Silêncio é o estado normal: só avisa quando há o que dizer. */
async function avisarGestores(storage, cfg, relatorio, enviarMsg) {
  const temAssunto = relatorio.modo === "backfill" ||
    relatorio.enviados.length || relatorio.erros.length ||
    relatorio.semEmail.length || relatorio.inconsistentes.length;
...
function podar(ledger, acoes) {
  ...
  for (const id of Object.keys(ledger.acoes)) if (!vivos.has(id)) delete ledger.acoes[id];
}
