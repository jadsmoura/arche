...
import { AVISOS_KEY, avisoLigado, normalizarConfig as normalizarAvisos } from "./avisos.js";
...
export async function varrer({
  ...
  const execucao = (async () => {
    ...
    /* O INTERRUPTOR dos avisos automáticos (pedido do dono, ago/2026): a
       cobrança e o resumo à coordenação podem ser silenciados na guia
       "Envios automáticos" do painel de acessos. A varredura continua
       rodando e marcando o que venceu — o que para é o e-mail. */
    let avisosCfg = {};
    try { avisosCfg = normalizarAvisos(JSON.parse((await storage.get(AVISOS_KEY)) || "{}")); }
    catch { avisosCfg = {}; }
    const enviarMsg = enviar || ((msg) => {
      const cod = msg.aviso || "ex-cobranca-relatorio";
      if (!avisoLigado(avisosCfg, cod, hoje)) {
        console.log(`[cobranca] ${cod} silenciado — e-mail não enviado a ${msg.para}`);
        return null;
      }
      return enviarPadrao(msg, cfg);
    });
...
async function avisarGestores(storage, cfg, relatorio, enviarMsg) {
  ...
  try {
    ...
    if (para.length) await enviarMsg({ para, aviso: "ex-resumo-cobranca", ...emailResumoCobranca(relatorio) });
  } catch (e) {
