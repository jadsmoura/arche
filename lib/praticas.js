...
/* Os 17 Objetivos de Desenvolvimento Sustentável da ONU (Agenda 2030), no
   lugar da seção 13 do modelo (decisão do dono, ago/2026): a lista de produtos
   repetia o que os ANEXOS já mostram — a cartilha, o folder e a lista de
   presença estão lá, em arquivo —, enquanto o ODS é a informação que não está
   em lugar nenhum e que a instituição precisa consolidar. O que se grava é o
   NÚMERO: o nome do objetivo é rótulo, e o número é como a ONU, o MEC e o
   próprio PDI se referem a ele. */
export const ODS = [
...
export const rotuloOds = (c) => {
  const o = ODS.find((x) => x.codigo === String(c || ""));
  return o ? `ODS ${o.codigo} — ${o.nome}` : "";
};
...
export const CAMPOS_EXTENSAO = [
  ...
  /* 3 — OBJETIVOS E RESULTADOS: só o objetivo, que vem do catálogo comum. A
     escolha "alcançados / parcialmente / não" saiu por decisão do dono
     (ago/2026) — o que o desfecho tem a dizer, quem diz são os resultados
     descritos no próprio objetivo e no resumo, e um seletor a mais é uma
     pergunta a mais no formulário que ele acabou de mandar enxugar. */
  ...
  /* ODS — no lugar da seção 13 do modelo */
  { campo: "ods", rotulo: "Objetivos de Desenvolvimento Sustentável (ODS)", tipo: "marcar",
    obrigatorio: false, opcoes: ODS, secao: "ODS — Agenda 2030",
    ajuda: "Marque os ODS da Agenda 2030 que esta atividade contempla. Pode marcar mais de um." },
];
...
/** O registro fotográfico é o que comprova a realização — a mesma razão do
    mínimo da Extensão. A aula prática pede CINCO (decisão do dono, set/2026:
    subiu de três, igualando-se ao relatório da Extensão); a atividade de
    curricularização fica em TRÊS — o pedido foi só das aulas práticas, e o
    formulário da CE acabou de ser enxugado a pedido do mesmo dono. */
export const MIN_FOTOS = 5;
export const MIN_FOTOS_EXTENSAO = 3;
export const minFotosDe = (r) => (ehExtensao(r) ? MIN_FOTOS_EXTENSAO : MIN_FOTOS);
...
export function normalizarRelatorio(bruto = {}, { base = null } = {}) {
  ...
  return {
    ...
    // grava o NÚMERO do objetivo, na ordem da Agenda 2030
    ods: [...new Set((Array.isArray(bruto.ods ?? b.ods) ? (bruto.ods ?? b.ods) : [])
      .map((x) => txt(x, 4)).filter((x) => ODS.some((o) => o.codigo === x)))]
...
export function faltaNoRelatorio(r, { hoje = hojeLocalISO() } = {}) {
  ...
  const minimo = minFotosDe(r);
  if (fotos < minimo) {
    falta.push(ehExtensao(r)
      ? `Fotos da atividade — ${fotos} de ${minimo} (o registro fotográfico é o que comprova a realização)`
      : `Fotos da prática — ${fotos} de ${minimo} (o registro fotográfico é o que comprova a aula)`);
  }
  return falta;
}
...
export function podeReabrir(r, quem) {
  ...
  /* Olha o ALCANCE, não o papel no relatório: `papelNoRelatorio` devolve
     "professor" para o autor, e desde set/2026 quem coordena valida o próprio
     relatório — sem isto, ele se veria com o poder de decidir e sem o de
     desfazer justamente no documento que é dele. */
  return !!quem?.gestao;
}

/** O AUTOR do relatório — quem deu a aula (ou conduziu a atividade) e o
    registrou. É a mesma dupla de chaves que `papelNoRelatorio` usa. */
export const souAutorDo = (r, quem) =>
  mesmoEmail(r?.professor?.email, quem?.email) || mesmoEmail(r?.criadoPor, quem?.email);

/**
 * O COORDENADOR VALIDA O PRÓPRIO RELATÓRIO (pedido do dono, set/2026:
 * "permita aos coordenadores auto validar seus próprios relatórios de aulas
 * práticas e curricularização de extensão"), revendo a regra anterior de que
 * ninguém valida o que é seu.
 *
 * A razão é a mesma que já valeu na monitoria e na IC: **quem coordena
 * também leciona**, e o fluxo do AC TERMINA na coordenação do curso — não há
 * degrau acima dela. Com a trava, o relatório da aula do próprio coordenador
 * ficava parado para sempre em "enviado", ou dependia de a PROAC decidir por
 * ele: um processo que não fecha por causa de quem o conduz.
 *
 * A régua é ESTREITA: só valida o próprio quem já teria poder de validar
 * aquele relatório se ele fosse de outra pessoa — a coordenação DAQUELE curso,
 * a coordenação do módulo (a PROAC) e o gestor geral. Professor que não
 * coordena nada continua sem validar o que é seu, que é o que sustenta o
 * fluxo. E o ato fica MARCADO (`decisaoSobreORelatorioProprio`): num documento
 * que vai ao MEC, quem o lê meses depois precisa saber que as duas assinaturas
 * são da mesma pessoa, em dois papéis.
 */
export const autoValidacao = (r, quem) => souAutorDo(r, quem) && coordenaCurso(quem, r?.curso);

export function podeValidar(r, quem) {
  if (r?.status !== "enviado") return false;
  if (souAutorDo(r, quem)) return autoValidacao(r, quem);
  const p = papelNoRelatorio(r, quem);
  return p === "coordenador" || p === "gestao";
}

/** A decisão é do próprio autor, que coordena o curso? (acima) */
export const decisaoSobreORelatorioProprio = (r, quem) => autoValidacao(r, quem);
...
/** A decisão foi tomada por quem NÃO coordena aquele curso? (a PROAC ou a
    ...
    Olha o ALCANCE, não o papel no relatório: o gestor que valida um relatório
    SEU de um curso que não coordena também está decidindo no lugar dela. */
export const decisaoNoLugarDaCoordenacao = (r, quem) =>
  !!quem?.gestao && !(quem?.cursos || []).includes(txt(r?.curso, 60));
...
export function visaoDoRelatorio(r, quem) {
  ...
  /* `podeValidar` viaja com cada relatório porque a tela não pode repetir a
     régua: ela dizia "coordena o curso E não é meu", e desde set/2026 o
     próprio autor que coordena valida — duas cópias da mesma regra acabam
     divergindo, e aqui a que erra esconde um botão que o servidor aceitaria. */
  return { ...r, meuPapel: papel, podeValidar: podeValidar(r, quem),
    ...(autoValidacao(r, quem) ? { autoValidacao: true } : {}) };
}
...
export function normalizarProfessor(bruto = {}) {
  return {
    ...
    // a matrícula é a chave FORTE de quem chega sem e-mail: é ela que a
    // planilha da coordenação traz, e é por ela que `casarPorIdentidade`
    // encontra a conta da pessoa depois
    matricula: txt(bruto.matricula, 30),
    // de onde veio o e-mail: "" (digitado pela coordenação ou do lote) ou
    // "casamento" (adotado da conta do portal pela matrícula/nome) — a tela
    // precisa DIZER quando foi o portal que escolheu, para alguém conferir
    emailOrigem: txt(bruto.emailOrigem, 20) === "casamento" ? "casamento" : "",
    ...
    // e as que CURRICULARIZAM extensão. São duas listas porque são dois
    // processos: a mesma disciplina pode ter aula prática e não curricularizar
    // extensão, e vice-versa — a relação de Enfermagem (ago/2026) traz as duas
    // com interseção parcial. Uma lista só faria o denominador de um painel
    // contar as disciplinas do outro.
    disciplinasExtensao: listaUnica(bruto.disciplinasExtensao).map((d) => txt(d, 200)).slice(0, 40),
  };
}

/** A chave de quem ainda não tem e-mail: o nome, sem acento e sem caixa.
    Nome de UMA palavra não é chave (dois "Ana" viram a mesma pessoa). */
export const chaveDeNome = (v) => {
  const s = txt(v, 160).normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .toLowerCase().replace(/\s+/g, " ").trim();
  return s.split(" ").filter(Boolean).length >= 2 ? s : "";
};

/** Uma linha do cadastro identifica alguém? (e-mail, ou nome com o bastante) */
export const identificaProfessor = (p) => !!email(p?.email) || !!chaveDeNome(p?.nome);

/** A chave de uma disciplina no curso: sem acento, sem caixa, espaços
    colapsados (revisão de set/2026 — "Saúde da Mulher" digitada como "Saude
    da  Mulher" era OUTRA disciplina para o painel, e a cobrança mensal não
    cessava nunca depois de o relatório ter sido entregue). */
export const chaveDeDisciplina = (curso, d) =>
  `${txt(curso, 60)}::${txt(d, 200).normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .toLowerCase().replace(/\s+/g, " ").trim()}`;

/** A chave de deduplicação: o e-mail quando há, senão o nome. */
const chaveDoProfessor = (p) => email(p.email) || `nome:${chaveDeNome(p.nome)}`;
...
export function normalizarCadastroDoCurso(bruto = {}) {
  const professores = (Array.isArray(bruto.professores) ? bruto.professores : [])
    ...
    /* A LINHA SEM E-MAIL ENTRA (revisão de ago/2026, ao receber a relação de
       Enfermagem): a exigência de e-mail rejeitava a lista inteira que a
       coordenação manda — matrícula, nome e disciplinas, sem contato, que é
       como ela sai do sistema acadêmico. Ou a lista não entrava, ou alguém
       adivinhava vinte endereços; e endereço adivinhado prende a disciplina
       à conta errada. Sem e-mail a pessoa não recebe o lembrete (e a tela diz
       isso), mas ela CONTA no denominador — que é a razão de o cadastro
       existir — e o e-mail se resolve depois, pela matrícula ou pelo nome. */
    .filter((p) => !!p.nome && identificaProfessor(p));
  /* DUPLICATA SE FUNDE, NÃO SE DESCARTA (revisão adversarial, set/2026): a
     segunda linha da mesma pessoa sumia no salvar com todas as disciplinas
     dela — e o caso é comum: a coordenação já tinha digitado a professora com
     e-mail quando o lote entrou com ela sem e-mail, e o casamento por nome
     adotou o mesmo endereço; ou a coordenadora viu duas linhas e preencheu o
     e-mail na segunda. Agora as listas se somam. E duas pessoas com o MESMO
     NOME e MATRÍCULAS diferentes são duas pessoas — homônimos legítimos
     existem, e a matrícula é a chave que os separa. */
  /* Duas linhas são a MESMA pessoa quando nada nelas as separa: mesmo nome
     completo (ou mesmo e-mail, quando o nome não identifica) e nenhum par de
     e-mails ou de matrículas DIFERENTES e preenchidos. É o que faz "Bruna com
     e-mail" + "Bruna sem e-mail" virarem uma só, e "Maria 111" + "Maria 222"
     continuarem duas. */
  const compativeis = (a, b) =>
    (!a.email || !b.email || a.email === b.email)
    && (!a.matricula || !b.matricula || a.matricula === b.matricula);
  const fundir = (alvo, p) => {
    alvo.email = alvo.email || p.email;
    alvo.matricula = alvo.matricula || p.matricula;
    alvo.emailOrigem = alvo.emailOrigem || p.emailOrigem;
    alvo.disciplinas = listaUnica([...alvo.disciplinas, ...p.disciplinas]);
    alvo.disciplinasExtensao = listaUnica([...alvo.disciplinasExtensao, ...p.disciplinasExtensao]);
  };
  const grupos = new Map();                          // nome (ou e-mail) → pessoas
  for (const p of professores) {
    const kNome = chaveDeNome(p.nome);
    const chave = kNome ? `nome:${kNome}` : `e:${p.email}`;
    // sem nome que identifique, a chave é o e-mail; com nome, o nome — e o
    // e-mail só separa quando os dois estão preenchidos e diferem
    const pessoas = grupos.get(chave) || [];
    const igual = pessoas.find((x) => compativeis(x, p));
    if (igual) fundir(igual, p); else pessoas.push({ ...p });
    grupos.set(chave, pessoas);
  }
  // o mesmo e-mail escrito sob dois nomes ("Bruna P. Ribeiro" / "Bruna Povoa
  // Ribeiro") também é uma pessoa: o e-mail é a chave forte
  const saida = [];
  for (const p of [...grupos.values()].flat()) {
    const igual = p.email && saida.find((x) => x.email === p.email);
    if (igual) fundir(igual, p); else saida.push(p);
  }
  return { professores: saida.slice(0, 300) };
}
...
/** As disciplinas de um professor no tipo pedido. É UM lugar só que sabe qual
    das duas listas responde por qual processo — espalhar a escolha faria um
    painel contar as disciplinas do outro. */
export const disciplinasDe = (p, tipo = "pratica") =>
  (txt(tipo, 20) === "extensao" ? p?.disciplinasExtensao : p?.disciplinas) || [];

/** Todas as disciplinas cadastradas no semestre, com quem as leciona. */
export function disciplinasDoSemestre(cadastro, semestre, curso = "", tipo = "pratica") {
  const mapa = new Map();
  for (const p of professoresDoSemestre(cadastro, semestre, curso)) {
    for (const d of disciplinasDe(p, tipo)) {
...
export const minhasDisciplinas = (cadastro, semestre, quemEmail, tipo = "pratica") =>
  professoresDoSemestre(cadastro, semestre)
    .filter((p) => mesmoEmail(p.email, quemEmail))
    .flatMap((p) => disciplinasDe(p, tipo).map((d) => ({ curso: p.curso, disciplina: d })));

/** Quem está no cadastro sem e-mail — não recebe lembrete, e a tela precisa
    dizer isso à coordenação para ela completar (ou para o casamento por
    matrícula/nome resolver sozinho no próximo arranque). */
export const professoresSemEmail = (cadastro, semestre, curso = "") =>
  professoresDoSemestre(cadastro, semestre, curso).filter((p) => !p.email);
...
export function panorama(relatorios, cadastro, { semestre = "", curso = "" } = {}) {
  ...
  const disciplinas = disciplinasDoSemestre(cadastro, semestre, curso, "pratica");

  const chaveDisc = chaveDeDisciplina;
  ...
  /* O DENOMINADOR DA EXTENSÃO CURRICULAR (ago/2026, quando o cadastro passou
     a guardar as duas listas): até aqui o painel da CE só sabia contar o que
     tinha CHEGADO — horas, acadêmicos, disciplinas alcançadas —, e "9
     atividades" não diz se são todas ou um terço. Com a relação da
     coordenação ele passa a responder a mesma pergunta do painel das
     práticas: quais disciplinas curricularizam extensão neste semestre e
     quais ainda não têm relatório. O denominador é só de quem cadastrou —
     curso sem lista continua com 0, que é a verdade sobre o que se sabe. */
  const discExt = disciplinasDoSemestre(cadastro, semestre, curso, "extensao");
  const profsExt = professores.filter((p) => (p.disciplinasExtensao || []).length);
  const comRelExt = new Set(extEntregues.map((r) => chaveDisc(r.curso, r.disciplina)));
  const registrouExt = new Set(extEntregues.map((r) => email(r.professor?.email)).filter(Boolean));
  ...
  return {
    ...
    extensaoCurricular: {
      ...
      // o que o CADASTRO diz que deve vir — 0 no curso que ainda não mandou
      // a relação, que é o que se sabe sobre ele
      cadastradas: discExt.length,
      professoresCadastrados: profsExt.length,
      professoresQueRegistraram: registrouExt.size,
      disciplinasSemRelatorio: discExt
        .filter((d) => !comRelExt.has(chaveDisc(d.curso, d.disciplina)))
        .map((d) => ({ ...d, professores: d.professores.map((p) => p.nome).filter(Boolean) })),
      professoresSemRegistro: profsExt
        .filter((p) => !registrouExt.has(email(p.email)))
        .map((p) => ({ email: p.email, nome: p.nome, curso: p.curso,
          disciplinas: (p.disciplinasExtensao || []).length }))
        .sort((a, b) => a.nome.localeCompare(b.nome, "pt-BR")),
    },
    // quem a coordenação cadastrou sem e-mail: conta no denominador e NÃO
    // recebe lembrete — a tela precisa dizer isso, senão o silêncio parece
    // que o sistema avisou
    semEmail: professores.filter((p) => !p.email)
      .map((p) => ({ nome: p.nome, curso: p.curso, matricula: p.matricula })),
    total: doSem.length,
...
/* ======================================================================
   6. O LEMBRETE MENSAL DA EXTENSÃO CURRICULAR

   Pedido do dono (ago/2026), junto com o cadastro: "inclua um lembrete
   mensal para professores cadastrados aqui submeterem seus relatórios".

   Por que MENSAL, e não semanal como o da aula prática: são dois ritmos
   diferentes. A aula prática acontece toda semana e o relatório é DELA — o
   lembrete de segunda pergunta pela semana que passou. A curricularização é
   uma atividade por disciplina no semestre, com o relatório FINAL dela; um
   e-mail por semana sobre um relatório que só faz sentido depois da ação
   vira ruído, e ruído semanal é o que faz o professor criar filtro para o
   remetente — e aí ele deixa de receber também o que importa.

   Três regras, e as três vêm do mesmo cuidado que o lembrete de segunda já
   tinha: ele **não afirma atraso** (o sistema não sabe quando a atividade
   está marcada — sabe quais disciplinas curricularizam), ele **nomeia as
   disciplinas que faltam**, e **quem já entregou tudo não recebe nada**.
   ====================================================================== */

/** É o primeiro dia do mês? */
export const ehPrimeiroDoMes = (iso) => /^\d{4}-\d{2}-01$/.test(txt(iso, 10));

/**
 * Quem receberá o lembrete mensal da extensão curricular, e o que ele diz.
 * Uma entrada por PESSOA, com as disciplinas dela que ainda não têm
 * relatório entregue no semestre.
 */
export function pendenciasCobrancaExtensao(relatorios, cadastro, { hoje = hojeLocalISO() } = {}) {
  const semestre = semestreDe(hoje);
  const doSem = (relatorios || []).filter((r) =>
    r.semestre === semestre && ehExtensao(r));
  const entregues = doSem.filter(ENTREGUE);
  const chave = chaveDeDisciplina;
  const jaVeio = new Set(entregues.map((r) => chave(r.curso, r.disciplina)));

  /* UMA entrada por PESSOA (revisão de set/2026): o professor de dois cursos
     saía duas vezes — o automático (marca por e-mail) só citava as disciplinas
     do primeiro curso e o manual mandava dois e-mails. Agrupa-se por e-mail e
     as disciplinas que faltam se somam, com o curso ao lado quando há mais de um. */
  const porEmail = new Map();
  for (const p of professoresDoSemestre(cadastro, semestre)) {
    // sem e-mail não há a quem escrever — ela conta no denominador do painel,
    // mas o lembrete precisa de endereço; a tela é que cobra a coordenação
    if (!p.email || !(p.disciplinasExtensao || []).length) continue;
    // as disciplinas DELE que ainda não têm relatório entregue neste
    // semestre — é isto que o e-mail nomeia, em vez de um "você tem
    // pendências" que ninguém sabe onde resolver
    const faltam = (p.disciplinasExtensao || []).filter((d) => !jaVeio.has(chave(p.curso, d)));
    const atual = porEmail.get(p.email) || {
      email: p.email, nome: p.nome, curso: p.curso, semestre, faltam: [], cursos: [],
      entregues: entregues.filter((r) => mesmoEmail(r.professor?.email, p.email)).length,
      // preencheu e não enviou: o esquecimento mais comum, e o que faria o
      // lembrete parecer injusto se não fosse citado
      rascunhos: doSem.filter((r) =>
        r.status === "rascunho" && mesmoEmail(r.professor?.email, p.email)).length,
    };
    atual.cursos.push(p.curso);
    atual.faltam.push(...faltam);
    porEmail.set(p.email, atual);
  }
  return [...porEmail.values()].filter((p) => p.faltam.length || p.rascunhos > 0);
}

export const _paraTeste = { txt, email, dataISO, mesmoEmail, listaUnica, chaveDeNome };
