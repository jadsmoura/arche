...
export function normalizarCadastroDoCurso(bruto = {}) {
  ...
  const professores = (Array.isArray(bruto.professores) ? bruto.professores : [])
    ...
    .filter((p) => {
      if (!p.nome || !identificaProfessor(p)) return false;
      const chave = chaveDoProfessor(p);
      if (vistos.has(chave)) return false;        // a mesma pessoa não entra duas vezes
      vistos.add(chave);
      return true;
    })
    .slice(0, 300);
...
export function pendenciasCobrancaExtensao(relatorios, cadastro, { hoje = hojeLocalISO() } = {}) {
  ...
  const entregues = doSem.filter(ENTREGUE);
  const chave = (c, d) => `${c}::${String(d || "").toLowerCase()}`;
  const jaVeio = new Set(entregues.map((r) => chave(r.curso, r.disciplina)));

  return professoresDoSemestre(cadastro, semestre)
    ...
    .filter((p) => p.email && (p.disciplinasExtensao || []).length)
    .map((p) => ({
      email: p.email, nome: p.nome, curso: p.curso, semestre,
      ...
      faltam: (p.disciplinasExtensao || []).filter((d) => !jaVeio.has(chave(p.curso, d))),
    ...
    .filter((p) => p.faltam.length || p.rascunhos > 0);
}
