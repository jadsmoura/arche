...
export async function gerarInscritosAeeXlsx(acao, { somentePresentes = false } = {}) {
  const wb = new ExcelJS.Workbook();
  ...
  // texto que a pessoa digitou (nome, e-mail): um valor começando com = + - @
  // vira FÓRMULA quando a AEE abre a planilha — neutraliza-se com um apóstrofo
  // à frente (o Excel trata como texto). Célula numérica/CH não passa por aqui.
  const seguro = (v) => {
    const s = String(v ?? "");
    return /^[=+\-@\t\r]/.test(s) ? "'" + s : s;
  };
  ...
  const inscritos = (parts.inscritos || [])
    .filter((x) => !somentePresentes || x.presente === true);
