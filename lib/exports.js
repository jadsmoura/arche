  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        ...
        // do catálogo ASSINA, como manda a regra do projeto: o nome estava
        // cravado aqui, com o órgão na ordem antiga e sem "Ação Comunitária",
        // e sob a linha ia o nome do ÓRGÃO no lugar do CARGO de quem assina —
        // três divergências em relação ao mesmo ato exportado em PDF (achado
        // da varredura de ago/2026). Uma troca de pró-reitor deixava este
        // documento para trás.
        assinatura(ASSINA.proReitor.nome, ASSINA.proReitor.cargo),
      ],
    }],
  });
...
export async function gerarInscritosAeeXlsx(acao, { somentePresentes = false, atividade = null } = {}) {
  ...
  const inscritos = (parts.inscritos || [])
    // evento pago: a inscrição sem pagamento confirmado não vai à certificação
    .filter((x) => inscricaoValida(x))
...
export async function gerarInscritosCompletoXlsx(acao) {
  ...
  ws.addRow([
    // "Vínculo" é a resposta que a ficha pede antes do curso (set/2026): sem
    // ela, "Enfermagem" digitado por alguém da UFG é igual ao da casa, e o
    // público externo — número do relatório de extensão — não se conta
    // a MATRÍCULA (set/2026) é a do estudante da casa: a ficha a pede a quem se
    // diz estudante de um curso do UNIEGO, e é por ela que a coordenação casa a
    // lista com o sistema acadêmico
  ...
  for (const x of acao.participantes?.inscritos || []) {
    ws.addRow([
      ...
      seguro(x.email || ""),
      seguro(x.telefone || ""),
      x.vinculo === "uniego" ? "UNIEGO" : x.vinculo === "externo" ? "outra instituição" : "",
      seguro(x.curso || ""),
...
/**
 * A PLANILHA DA PRESTAÇÃO DE CONTAS de um evento pago (set/2026): uma linha
 * por inscrição com cobrança — categoria, lote, estado, meio, valor devido,
 * valor pago, taxa do provedor e líquido — e uma aba de resumo com os totais
 * por estado, por meio e por categoria. SEM CPF: é o documento que a
 * coordenação entrega à instituição, e circula. Quem calcula é
 * lib/pagamentos.js (`resumoFinanceiro`/`linhasFinanceiro`); aqui só se
 * desenha.
 */
export async function gerarFinanceiroEventoXlsx(acao, { resumo, linhas }) {
  const p = acao.proposta || {};
  const reaisDe = (c) => Math.round(Number(c) || 0) / 100;
  const dataHora = (iso) => {
    const n = iso ? new Date(iso) : null;
    if (!n || isNaN(n)) return "";
  ...
  ws.addRow(["Código", "Nome", "E-mail", "Categoria", "Lote", "Voucher", "Desconto (R$)", "Situação", "Meio", "Parcelas",
    "Valor devido (R$)", "Valor pago (R$)", "Taxa (R$)", "Líquido (R$)", "Pago em", "Nº no provedor", "Isento por", "Motivo da isenção"]);
  ws.getRow(1).font = { bold: true };
  ws.columns.forEach((col, i) => { col.width = i === 1 ? 34 : i === 2 ? 30 : 16; });
  for (const l of linhas || []) {
    ws.addRow([
      seguro(l.codigo), seguro(l.nome), seguro(l.email), seguro(l.categoria), seguro(l.lote),
      seguro(l.voucher || ""), l.desconto ? Number(l.desconto) : "",
      seguro(l.estado), seguro(l.meio), l.parcelas || "",
      Number(l.valorDevido) || 0, l.valorPago !== "" ? Number(l.valorPago) : "",
      l.taxa !== "" ? Number(l.taxa) : "", l.liquido !== "" ? Number(l.liquido) : "",
      dataHora(l.pagoEm), seguro(l.provedorId), seguro(l.isentoPor), seguro(l.motivoIsencao),
    ]);
  }
  for (const col of [7, 11, 12, 13, 14]) ws.getColumn(col).numFmt = "#,##0.00";

  const r = resumo || {};
  const wr = wb.addWorksheet("Resumo");
  wr.columns = [{ width: 34 }, { width: 14 }, { width: 18 }];
  wr.addRow([seguro(p.nomeAtividade || ""), "", ""]).font = { bold: true, size: 13 };
  wr.addRow([`${acao.numeroAcao || ""} · ${acao.curso || ""}`.trim(), "", ""]);
  wr.addRow([]);
  wr.addRow(["Inscrições com cobrança", r.total || 0, ""]).font = { bold: true };
  wr.addRow(["Recebido (bruto, R$)", reaisDe(r.bruto), ""]);
  wr.addRow(["Taxas do provedor (R$)", reaisDe(r.taxa), ""]);
  wr.addRow(["Líquido (R$)", reaisDe(r.liquido), ""]).font = { bold: true };
  wr.addRow(["A receber — reservas em aberto (R$)", reaisDe(r.aReceber), ""]);
  // a reserva VENCIDA não é dinheiro a entrar: a vaga já voltou à fila
  wr.addRow(["Reservas vencidas sem pagar (R$)", reaisDe(r.vencidoSemPagar || 0), ""]);
  wr.addRow([]);
  wr.addRow(["Por situação", "Inscrições", ""]).font = { bold: true };
  for (const [k, n] of Object.entries(r.porEstado || {})) wr.addRow([seguro(k), n, ""]);
  wr.addRow([]);
  wr.addRow(["Por meio de pagamento", "Inscrições", "Valor (R$)"]).font = { bold: true };
  for (const [k, v] of Object.entries(r.porMeio || {})) wr.addRow([seguro(k), v.n, reaisDe(v.valor)]);
  wr.addRow([]);
  wr.addRow(["Por categoria", "Inscrições", "Pago (R$)"]).font = { bold: true };
  for (const [k, v] of Object.entries(r.porCategoria || {})) wr.addRow([seguro(k), v.n, reaisDe(v.valor)]);
  if (Object.keys(r.porVoucher || {}).length) {
    wr.addRow([]);
    wr.addRow(["Por voucher de desconto", "Inscrições", "Desconto (R$)"]).font = { bold: true };
    for (const [k, v] of Object.entries(r.porVoucher)) wr.addRow([seguro(k), v.n, reaisDe(v.desconto)]);
  }
  wr.addRow([]);
  wr.addRow([`Gerado pelo ARCHÉ em ${dataHora(new Date().toISOString())}. Valores em reais; a taxa é a informada pelo provedor em cada pagamento.`, "", ""]);
  wr.getColumn(2).numFmt = "#,##0.00";
  wr.getColumn(3).numFmt = "#,##0.00";
  return Buffer.from(await wb.xlsx.writeBuffer());
}
