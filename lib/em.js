...
export function normalizarBolsistaEM(b = {}, { base = null } = {}) {
  ...
  return {
    ...
    /* O RESPONSÁVEL se PRESERVA quando o corpo não o traz (set/2026, a mesma
       lição da ficha do monitor): estes dois campos passaram a ser escritos
       pelo PRÓPRIO estudante, e o formulário da coordenação não os manda —
       sem o `base` aqui, salvar o cadastro do bolsista apagaria em silêncio a
       autorização que ele acabou de informar, e o Anexo 01 do termo voltaria
       à linha pontilhada. Corrigir continua possível: campo PREENCHIDO vence. */
    responsavel: {
      nome: t(b.responsavel?.nome, 120) || base?.responsavel?.nome || "",
      cpf: normalizarCpf(b.responsavel?.cpf) || base?.responsavel?.cpf || "",
    },
    banco: t(b.banco, 60), agencia: t(b.agencia, 20), conta: t(b.conta, 30), pix: t(b.pix, 120),
    /* De quem é a mão que escreveu a conta. O normal é o próprio estudante —
       quem tem o cartão na mão é ele —, mas quando ele não consegue (são
       adolescentes, e o e-mail cai no spam) a coordenação preenche EM NOME
       dele, e um dado que sustenta pagamento precisa dizer isso. Some quando
       o estudante grava depois, que é como deve ser. */
    ...(b.cadastroPelaGestao?.em || base?.cadastroPelaGestao?.em
      ? { cadastroPelaGestao: b.cadastroPelaGestao?.em ? {
        por: t(b.cadastroPelaGestao.por, 120), em: t(b.cadastroPelaGestao.em, 30),
      } : base.cadastroPelaGestao }
      : {}),
    ...
    /* OS E-MAILS RECONHECIDOS NÃO SE PERDEM NUMA EDIÇÃO DE CADASTRO
       (set/2026): `adotarEmailNoEM` acrescenta aqui a conta que casou pelo
       CPF — é o que faz a bolsista que entra pelo Gmail pessoal continuar
       sendo reconhecida sem perder a escolar. Esta função devolve um conjunto
       FIXO de campos, então o que não estiver escrito aqui some na primeira
       gravação: sem estas duas linhas, salvar o cadastro pela coordenação
       desfazia a adoção e devolvia o estudante ao painel vazio. */
    emails: [...new Set([...(Array.isArray(b.emails) ? b.emails : []),
      ...(Array.isArray(base?.emails) ? base.emails : [])]
      .map((e) => String(e || "").trim().toLowerCase()).filter(Boolean))].slice(0, 5),
    ...(t(b.emailAnterior, 120) || base?.emailAnterior
      ? { emailAnterior: t(b.emailAnterior, 120) || base.emailAnterior } : {}),
    /* A SAÍDA e a SUBSTITUIÇÃO (set/2026) — pelo mesmo motivo: são escritas
       por rota própria, e o formulário do cadastro não as manda. */
    ...(b.desligamento || base?.desligamento
      ? { desligamento: normalizarDesligamentoEM(b.desligamento || base.desligamento) } : {}),
    ...(b.substituicao || base?.substituicao
      ? { substituicao: normalizarSubstituicaoEM(b.substituicao || base.substituicao) } : {}),
...
const normalizarDesligamentoEM = (d = {}) => ({
  em: t(d.em, 40), por: t(d.por, 120), motivo: t(d.motivo, 1000),
});
/* Os dois lados da troca apontam um para o outro: quem SAIU guarda
   `entrou`, quem ENTROU guarda `saiu`. É o que responde, meses depois, por
   que o termo assinado não é o do nome que está no resultado publicado. */
const normalizarSubstituicaoEM = (x = {}) => ({
  papel: umDe(x.papel, ["saiu", "entrou"], "saiu"),
  id: t(x.id, 60), nome: t(x.nome, 120),
  em: t(x.em, 40), por: t(x.por, 120), motivo: t(x.motivo, 1000),
});
...
/* --------------------------- A SAÍDA DA TURMA ---------------------------
   Pedido do dono (set/2026): "insira a opção de substituir bolsista de EM —
   precisamos fazer uma troca antes da assinatura, houve uma desistência".

   O registro já sabia o que fazer com quem sai: `desligado` tira a pessoa dos
   TERMOS, da folha de pagamento, das cotas, dos comunicados e das chamadas —
   tudo isso já filtrava por ela. O que não existia era o ATO: nenhuma tela
   chamava a rota que muda a situação, e a coordenação não tinha como tirar da
   pilha de assinaturas quem havia desistido.

   Desligar fecha também o trecho ABERTO da trajetória: o professor que estava
   recebendo o estudante deixa de estar, e é a data de hoje que diz até quando
   aquilo valeu. O que já se encerrou não se toca — a trajetória nunca se
   apaga, é ela que conta por onde a pessoa passou.

   A bolsa FICA no registro de propósito: ela diz qual cota a pessoa ocupava,
   e `cotasDaTurma` já não conta desligado. Zerá-la apagaria a informação de
   que aquela vaga era do CNPq e passou a ser de outro — que é justamente o
   que a substituição precisa dizer. */
export function desligarEM(b, { motivo, por = "", agora = new Date(), hoje = hojeLocalISO() } = {}) {
  const fechado = trocarProjeto(b, null, { hoje });
  return anotarEM({
    ...fechado,
    situacao: "desligado",
    desligamento: { em: agora.toISOString(), por: t(por, 120), motivo: t(motivo, 1000) },
  }, { quem: por, oQue: `desligou da turma — ${t(motivo, 200)}`, agora });
}
...
/**
 * O QUE FALTA DO RESPONSÁVEL. Vale para TODO bolsista, voluntário inclusive:
 * são menores de idade, e o que a autorização do Anexo 01 exige não é a bolsa
 * — é o acesso às dependências da instituição e a permissão para desenvolver
 * as atividades. O termo imprime exatamente estes dois campos (nome e CPF),
 * e é por isso que a lista é esta e não outra: pedir telefone ou endereço do
 * responsável seria coletar dado de um terceiro que nenhum documento usa.
 */
export function faltaDoResponsavelEM(b = {}) {
  return [
    !t(b.responsavel?.nome, 120) && "nome do responsável",
    !normalizarCpf(b.responsavel?.cpf) && "CPF do responsável",
  ].filter(Boolean);
}

/**
 * OS DADOS DO PRÓPRIO ESTUDANTE — nome, CPF, telefone e escola.
 *
 * Eles sempre foram COBRADOS (o selo do cartão da gestão os conta) e nunca
 * tiveram onde ser informados: a turma 2026/2027 veio do resultado da seleção
 * — colocação, nota, presença na entrevista —, que não pergunta CPF, e o
 * painel do estudante só oferecia o responsável e a conta. O CPF que ele
 * digitava era o do PERFIL, noutra página, e nunca chegava ao registro do
 * ICEM: a coordenação seguia lendo "falta CPF" sobre alguém que já o tinha
 * informado ao portal.
 *
 * A série fica de fora da cobrança de propósito: nenhum documento a imprime
 * (o campo existe no formulário para o estudante corrigi-la, não como falta).
 */
export function faltaDosSeusDadosEM(b = {}) {
  return [
    !t(b.nome, 120) && "nome", !normalizarCpf(b.cpf) && "CPF",
    !t(b.telefone, 30) && "telefone", !t(b.escola, 120) && "escola",
  ].filter(Boolean);
}

/**
 * TUDO O QUE O PRÓPRIO ESTUDANTE PREENCHE (set/2026) — a lista que o e-mail
 * de cobrança nomeia e que o painel dele cobra, agora num FORMULÁRIO SÓ
 * (pedido do dono set/2026: "em um único formulário, todos os dados; o mesmo
 * enviado por e-mail; o aluno só entra e completa").
 *
 * São três coisas de naturezas diferentes: os DADOS DELE, o RESPONSÁVEL — que
 * todo bolsista tem de informar, porque a autorização é da IDADE — e a CONTA,
 * que só existe para quem recebe bolsa. Juntá-las numa função só é o que
 * permite ao e-mail dizer a um voluntário o que falta a ELE sem lhe pedir
 * conta corrente, e a um bolsista do CNPq as três coisas numa mensagem só.
 */
export const faltaDoEstudanteEM = (b = {}) =>
  [...faltaDosSeusDadosEM(b), ...faltaDoResponsavelEM(b), ...faltaDadosBancariosEM(b)];

/** O que falta no cadastro de um bolsista EM — régua própria do programa. */
/* `incluirProjeto` existe para o SELO do cartão (pedido do dono set/2026:
   "um ícone de alerta me ajuda a identificar quem falta dados"): o projeto
   acompanhado é pendência de outra natureza — quem escolhe é o estudante, e
   o cartão já diz na própria linha se ele tem projeto ou não. Misturá-lo com
   "falta CPF" faria o selo acender em quase todo mundo no começo da turma,
   e selo que acende sempre ninguém olha. */
export function faltaNoBolsistaEM(b = {}, { incluirProjeto = true } = {}) {
  return [
    // os dados dele e o responsável saem da MESMA régua que o painel do
    // estudante cobra — duas listas do mesmo fato acabariam divergindo no
    // primeiro campo novo, e o selo da gestão passaria a cobrar algo que o
    // formulário dele não oferece (foi exatamente o que aconteceu com o CPF)
    ...faltaDosSeusDadosEM(b),
    !b.email && "e-mail",
    ...faltaDoResponsavelEM(b),
    !b.bolsa && "tipo de bolsa",
    incluirProjeto && !projetoAtual(b) && b.situacao === "ativo" && "projeto acompanhado",
  ].filter(Boolean);
}
