...
import { inscricaoValida } from "./pagamentos.js";
...
/* O NOME é a terceira chave, e só onde não há chave forte: é ele que faz o
   acervo de papel (a lista digitada, sem CPF nem e-mail) encontrar dono.
   Mas o nome do perfil é AUTODECLARADO e não é único (revisão adversarial de
   set/2026): no AUTO-SERVIÇO — o histórico `/certificados/` e o PDF que ele
   baixa — qualquer conta cujo nome batesse com o de um participante digitado
   sem CPF e sem e-mail levava o certificado dele, emitido no nome do outro e
   com código de validação legítimo; e há centenas de registros só com nome
   no acervo. Por isso `porNome` é uma ESCOLHA de quem chama: a GESTÃO emite
   pelo nome (é ato de quem confere a lista, com a lista à vista); a pessoa,
   só com CPF ou e-mail. Para o acervo de papel o caminho é a coordenação
   completar a chave forte, ou emitir e entregar ela mesma. */
const ehEstaPessoa = (r, { cpf, email, nome }, { porNome = true } = {}) =>
  mesmoCpf(r?.cpf, cpf) || mesmoEmail(r?.email, email)
  || (porNome && semChaveForte(r) && !!chave(nome) && chave(nomeDe(r)) === chave(nome));
...
/* O QUE O CERTIFICADO AFIRMA NÃO MUDA DEPOIS DE VALIDADO (revisão adversarial
   de set/2026, ampliada nesta rodada): o certificado é recalculado a cada
   pedido — da lista de inscritos, das presenças, da CH das atividades e da
   equipe. O ato de validar existe para a PROPPEX conferir tudo isso ANTES de
   o documento existir e de os avisos saírem; depois dele, toda escrita que
   alteraria o documento é recusada, e para corrigir a PROPPEX devolve o
   encerramento. A régua mora AQUI, junto de quem emite o documento, e não
   nas rotas: ela começou só na gestão, e a PORTA pública (check-in do
   monitor, presença pelo telão) seguiu credenciando — quem tivesse o código
   do monitor punha, depois de emitidos os certificados, um inscrito sem
   presença dentro da certificação. `registrada` entra junto porque a ação
   sem evento se congela pelo registro, que é o ato equivalente. */
export const eventoCongelado = (acao) =>
  txt(acao?.status) === "registrada" || situacaoEncerramento(acao) === "validado";
export const MSG_EVENTO_CONGELADO = "Este evento já foi encerrado e validado pela PROPPEX — o que consta é o "
  + "que os certificados afirmam. Para corrigir, a PROPPEX devolve o encerramento.";
...
/* ======================================================================
   ...
   ====================================================================== */
/* A CH de UMA atividade: a declarada e, na falta dela, a DURAÇÃO que a
   própria grade anuncia (revisão adversarial de set/2026). O campo `ch` é
   texto livre e o coordenador o deixa em branco o tempo todo — e uma soma
   zerada caía na carga horária do EVENTO INTEIRO: três participantes de duas
   horas saíam com certificado de quarenta. A grade já diz de que horas a que
   horas a atividade foi; ler isso é honesto, e é o que a pessoa cumpriu. */
const HHMM = /^(\d{1,2}):(\d{2})$/;
const emMinutos = (hhmm) => {
  const m = HHMM.exec(txt(hhmm));
  if (!m) return null;
  const h = Number(m[1]), min = Number(m[2]);
  return h >= 0 && h <= 23 && min >= 0 && min <= 59 ? h * 60 + min : null;
};
export function chDaAtividade(atv) {
  const declarada = num(atv?.ch);
  if (declarada > 0) return declarada;
  const ini = emMinutos(atv?.horaInicio), fim = emMinutos(atv?.horaFim);
  if (ini === null || fim === null || fim <= ini) return 0;
  return Math.round(((fim - ini) / 60) * 10) / 10;   // uma casa: 1,5 h existe
}
...
export function chDoParticipante(acao, inscrito) {
  ...
  /* SEM presença por atividade — evento de atividade única, ou sem controle
     de frequência — e SEM PROGRAMAÇÃO NENHUMA (a ação que correu fora do
     ARCHÉ, com a lista digitada na Extensão) vale a CH da ação, que é o que
     o projeto aprovado declara. HAVENDO grade e presença por atividade,
     vale a soma delas e SÓ ela: o
     fallback existia também para o caso de a soma dar zero, e era por ali
     que a hora inflada saía (atividade sem CH, ou presença apontando uma
     atividade que a coordenação removeu da programação depois). Soma zero
     não é "carga horária do evento inteiro": é carga horária que ninguém
     apurou, e o certificado fica retido até a coordenação corrigir a grade. */
  if (!ids.size || !prog.length) return num(acao?.proposta?.cargaHoraria);
  return prog.filter((a) => ids.has(a.id)).reduce((s, a) => s + chDaAtividade(a), 0);
}
...
export function certificadosDaAcao(acao, { hoje = hojeISO() } = {}) {
  ...
  for (const i of parts.inscritos || []) {
    ...
    // evento PAGO (set/2026): inscrição sem pagamento confirmado (nem
    // isenção) não certifica — a presença não compra o que não foi pago
    if (!inscricaoValida(i)) continue;
    out.push({
    ...
    /* CARGA HORÁRIA NÃO APURADA: a pessoa tem presença por atividade, e as
       atividades em que ela esteve não dizem quantas horas foram — nem no
       campo, nem no horário — ou saíram da programação. O documento não sai
       com zero nem com a CH do evento inteiro: fica retido, nomeado na guia
       Certificados, até a coordenação corrigir a grade. */
    const ult = out[out.length - 1];
    if (!(ult.ch > 0)) ult.chPendente = true;
  }
 ...
 * inteiro, ou não sai.
 */
export function certificadoDe(acao, { cpf, email, nome, tipo = "" } = {}, { hoje = hojeISO(), porNome = true } = {}) {
  const todos = certificadosDaAcao(acao, { hoje });
  const meus = todos.filter((c) => ehEstaPessoa(c, { cpf, email, nome }, { porNome })
    && (!tipo || c.tipo === tipo));
  // carga horária não apurada: o documento não sai (ver certificadosDaAcao)
  return meus.find((c) => !c.chPendente) || null;
}
...
/* ======================================================================
   ...
   ====================================================================== */
export function certificadosDePessoa(acoes = [], pessoa = {}, { hoje = hojeISO(), porNome = true } = {}) {
  const out = [];
  for (const a of acoes) {
    if (!a) continue;
    for (const c of certificadosDaAcao(a, { hoje })) {
      // o retido por carga horária não apurada não entra no histórico de
      // ninguém: o documento não existe até a coordenação corrigir a grade
      if (c.chPendente) continue;
      if (ehEstaPessoa(c, pessoa, { porNome })) out.push(c);
    }
  }
