...
/** O teto do dia é da CONTA, não do servidor: Gmail comum ~500, Workspace
    ~2.000. Vale para a API e para o `smtp.gmail.com`, que é a mesma conta. */
const tetoDaContaGoogle = (conta) =>
  (conta && !/@(gmail|googlemail)\.com$/i.test(conta) ? 2000 : 500);

export const gmail = {
  nome: "gmail",
  rotulo: "Gmail (API do Google)",
  tetoDiario: tetoDaContaGoogle,
  // o Google escreve no `From` a conta autenticada, a menos que o endereço
  // seja um alias verificado em "Enviar e-mail como" DAQUELA conta
  reescreveRemetente: () => true,
...
const hostSmtp = () => String(process.env.SMTP_HOST || "").trim().toLowerCase();

/* O caminho mais provável aqui é o Workspace da própria instituição, e o
   servidor do Google não é um SMTP qualquer: ele REESCREVE o remetente pelo
   endereço da conta que autenticou, a menos que o endereço seja um alias
   verificado em "Enviar e-mail como" dela. Sem dizer isso, o diagnóstico
   mostraria o `MAIL_FROM_ADDR` como se ele estivesse em uso — afirmando na
   tela algo que não é verdade na caixa de quem recebe.
   O RELÉ (`smtp-relay.gmail.com`, que o administrador habilita à parte) é o
   contrário: aceita qualquer remetente do domínio, e tem teto bem maior. */
const ehGoogle = () => /(^|\.)gmail\.com$/.test(hostSmtp());
const ehRele = () => hostSmtp() === "smtp-relay.gmail.com";
...
export const smtp = {
  ...
  // A exceção é o Google, cujo teto é público e depende da conta.
  tetoDiario: (conta) => (ehRele() ? 10000 : ehGoogle() ? tetoDaContaGoogle(conta) : null),
  reescreveRemetente: () => ehGoogle() && !ehRele(),
...
/** O transporte vai escrever OUTRO remetente no lugar do que pedimos? */
export const reescreveRemetente = () => ativo().reescreveRemetente();
