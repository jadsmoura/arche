/* ========================================================================
   ...
   O sistema não convoca reunião nem cobra ninguém por e-mail: ele mostra à
   PROPPEX, num lugar só, o que está fora de dia. A cobrança é humana.
  ...
  for (const [cod, r] of Object.entries(RITUAL)) {
    ...
    for (const curso of destinos) {
      ...
      saida.push({
        ...
        detalhe: `${ck.ordinarias} de ${ck.exigidas} sessão(ões) ordinária(s) registrada(s) no semestre` +
          `${ck.extraordinarias ? ` (${ck.extraordinarias} extraordinária[s] não contam para o ciclo)` : ""}.` +
          `${inedito ? " Este órgão nunca registrou ata no sistema." : ""}`,
        acao: `Falta${ck.faltam > 1 ? "m" : ""} ${ck.faltam} sessão(ões) até ${fimDaJanela(hoje)}.`,
    ...
    for (const curso of destinos) {
      ...
      saida.push({
        ...
        severidade: inedito ? "alta"
          ...
          : porPrazo(dias),
        titulo: alvo(cod, curso),
        detalhe: `${pautas.length} tema(s) obrigatório(s) do semestre sem registro em ata` +
          `${nunca ? `, sendo ${nunca} nunca tratado(s)` : ""}.`,
        acao: pautas.slice(0, 3).map((p) => p.titulo).join(" · ")
...
function alertasDeAtaParada(atas, { hoje }) {
  ...
  return (atas || [])
    ...
    .map(({ a, dias }) => ({
      ...
      acao: a.numero ? `Concluir e registrar a ${a.numero}.` : "Concluir a minuta e registrar a ata.",
  ...
  for (const a of atas || []) {
    ...
    for (const p of a.pauta || []) {
      ...
      saida.push({
        ...
        acao: `Prazo venceu em ${e.prazo} (há ${atraso} dias), deliberado na ${a.numero || "ata"}.`,
