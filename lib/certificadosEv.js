/* ========================================================================
   ARCHÉ EV — Certificados dos eventos, emitidos pelo próprio ARCHÉ.
...
import { contaPresente, normalizarProgramacao, temHotsiteEvento } from "./eventos.js";
...
export function assinaturasDoCertificado(acao, institucionais = {}) {
  const dele = acao?.evento?.assinaturas || {};
  const saida = [];
...
export const situacaoEncerramento = (acao) => {
  const s = txt(acao?.evento?.encerramento?.status);
...
/** Só evento APROVADO e com o encerramento VALIDADO certifica. */
export function eventoCertificavel(acao, hoje = hojeISO()) {
  if (!acao?.evento) return { ok: false, motivo: "Esta ação não é um evento." };
  if (!txt(acao.numeroAcao))
    return { ok: false, motivo: "O evento ainda não foi aprovado pela PROPPEX." };
  const s = situacaoEncerramento(acao);
...
export function certificadosDoEvento(acao, { hoje = hojeISO() } = {}) {
  const pode = eventoCertificavel(acao, hoje);
  if (!pode.ok) return [];
  const base = doEvento(acao);
  const parts = acao.participantes || {};
  const out = [];

  for (const i of parts.inscritos || []) {
    if (!txt(i?.nome) || !contaPresente(acao.evento, i)) continue;
    out.push({
...
export function certificadoDe(acao, { cpf, email, nome, tipo = "" } = {}, { hoje = hojeISO() } = {}) {
  const todos = certificadosDoEvento(acao, { hoje });
...
export function certificadosDePessoa(acoes = [], pessoa = {}, { hoje = hojeISO() } = {}) {
  ...
  for (const a of acoes) {
    ...
    for (const c of certificadosDoEvento(a, { hoje })) {
