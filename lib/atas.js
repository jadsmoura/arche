...
export function avisosDaAta(ata) {
  ...
  const anoDaSessao = Number(String(ata?.sessao?.data || "").slice(0, 4)) || 0;
  const anoDoNumero = Number(String(ata?.numero || "").match(/-(\d{4})-\d{3}$/)?.[1]) || 0;
  if (anoDaSessao && anoDoNumero && anoDaSessao !== anoDoNumero) {
    avisos.push({ tipo: "ano-do-numero",
      texto: `O número ${ata.numero} é da série de ${anoDoNumero}, mas a sessão aconteceu em `
        + `${String(ata.sessao.data).split("-").reverse().join("/")}. Fale com a PROPPEX para reemitir o número.` });
  }
  const presentes = (ata?.participantes || []).filter((p) => p.presenca === "presente");
