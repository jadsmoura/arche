...
export const STATUS = ["rascunho", "minuta", "revisao", "aprovada", "registrada"];
...
export function normalizarAta(bruto, { base = null, autor = null, agora = new Date() } = {}) {
  ...
  return {
    ...
    status: umDe(b.status, STATUS, base?.status || "rascunho"),
    ...
    pdf: base?.pdf || null,
    assinada: base?.assinada || null,
    convocacao: base?.convocacao || null,
...
export function validarAta(ata) {
  ...
  if (o?.nomeLivre && !ata.orgaoNome) erros.push("Informe o nome da comissão ou grupo de trabalho.");
  ...
  if (!ata?.secretaria?.nome) erros.push("Informe quem secretariou a sessão.");
