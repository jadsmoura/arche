...
export function normalizarAta(bruto, { base = null, autor = null, agora = new Date() } = {}) {
  ...
  return {
    ...
    // Ata REGISTRADA não volta atrás por gravação nenhuma (achado da revisão
    // adversarial de set/2026): a rota /status já recusava o rebaixamento,
    // mas o POST geral deixava o status da ABA vencer — uma aba carregada
    // antes do registro devolvia a ata a "minuta", e ela sumia da prova de
    // conformidade com o PDF e o registro ainda dentro. A ata registrada
    // continua corrigível: edita-se por cima e gera-se o PDF de novo.
    status: statusVigente(base?.status) === "registrada"
      ? "registrada"
      : umDe(statusVigente(b.status), STATUS, statusVigente(base?.status) || "rascunho"),
