...
export const TEXTO_EDITAL = {
  abertura:
    ...
    + "Comunitária (PROPPEX) e a Reitoria do Centro Universitário Evangélico de Goianésia (UNIEGO), no "
  ...
  secoes: [
    {
      ...
      itens: [
        { n: "1.1.", texto: "O processo seletivo destinar-se-á:", alineas: [
          "Seleção de Projetos de Monitoria Acadêmica propostos por docentes dos cursos de Graduação do Centro Universitário Evangélico de Goianésia;",
    ...
    },
    {
      ...
      itens: [
        { n: "3.1.", texto: "Para atuar como Monitor Acadêmico, o(a) discente deverá atender aos seguintes pré-requisitos:", alineas: [
          "Ser aluno regularmente matriculado em curso de Graduação do Centro Universitário Evangélico de Goianésia;",
...
/* O anexo só vale se subiu pela rota do sistema (`/api/files/<id>`, que é o
   que `POST /api/monitoria/anexo` devolve): o `href` é desenhado na tela e
   sai no PDF, e uma URL arbitrária no corpo virava `javascript:` na página
   (revisão adversarial de set/2026). */
export const URL_DE_ANEXO = /^\/api\/files\/[^/?#]+/;
function normalizarDocumento(d) {
  if (!d || typeof d !== "object") return null;
  const url = txt(d.url || d.link);
  if (!url || !URL_DE_ANEXO.test(url)) return null;
  return {
...
export function normalizarRelatorio(r) {
  ...
  return {
    ...
    // a gestão validou em nome da orientação (professor desligado): marca que
    // sobrevive à normalização, como `validadoPelaGestao` na IC
    ...(r.validadoPelaGestao ? { validadoPelaGestao: true } : {}),
...
/** A ficha já é do MONITOR? Depois que ele a preencheu (ou firmou a
    declaração), nome, CPF e contato são dele — o formulário do professor
    só ajusta o plano de trabalho. */
export const fichaDoMonitor = (m) => !!(m?.cadastradoEm || m?.declaracao?.aceita);

export function normalizarMonitor(m, anterior = null) {
  const base = anterior || {};
  const o = m && typeof m === "object" ? m : {};
  /* A ficha preenchida pelo MONITOR não se sobrescreve pelo formulário do
     professor (revisão adversarial de set/2026): a aba aberta antes da ficha
     mandava `cpf: ""` e o nome antigo, `""` não é nullish, e o CPF que o
     aluno digitou sumia — a ficha voltava a "inscrição pendente". */
  const dele = fichaDoMonitor(base);
  const cpfNovo = soDigitos(o.cpf).slice(0, 11);
  return {
    id: txt(o.id || base.id) || `m${Math.random().toString(36).slice(2, 9)}`,
    nome: dele ? (txt(base.nome) || txt(o.nome).slice(0, 160)) : (txt(o.nome).slice(0, 160) || txt(base.nome)),
    email: dele ? (email(base.email) || email(o.email)) : (email(o.email) || email(base.email)),
    matricula: txt(dele ? (base.matricula ?? o.matricula) : (o.matricula ?? base.matricula)).slice(0, 40),
    cpf: dele ? (soDigitos(base.cpf).slice(0, 11) || cpfNovo) : (cpfNovo || soDigitos(base.cpf).slice(0, 11)),
    telefone: txt(dele ? (base.telefone ?? o.telefone) : (o.telefone ?? base.telefone)).slice(0, 40),
    curso: txt(dele ? (base.curso ?? o.curso) : (o.curso ?? base.curso)),
    periodo: txt(dele ? (base.periodo ?? o.periodo) : (o.periodo ?? base.periodo)).slice(0, 20),
...
/* ======================================================================
   ...
   ====================================================================== */
/**
 * A COORDENAÇÃO DE CURSO alcança a monitoria do curso dela (decisão do dono,
 * ago/2026). A lista vem do cadastro de coordenação do portal — o mesmo que
 * o ARCHÉ AP usa —, porque coordenador de curso é a mesma pessoa nos dois
 * lugares. O alcance é o da gestão, mas SÓ nos projetos daquele curso: a
 * coordenação de Enfermagem não vê a monitoria de Direito.
 *
 * Nota de divergência: o Edital 03/2026 atribui a HOMOLOGAÇÃO à PROPPEX, e
 * aqui a coordenação de curso também homologa — foi decisão do dono. Se o
 * texto do edital precisar acompanhar, ele é gerado pelo próprio ARCHÉ
 * (TEXTO_EDITAL, mais abaixo).
 */
export const coordenaOCurso = (p, quem = {}) =>
  !!p?.curso && (quem.cursos || []).includes(txt(p.curso));

/**
 * É o ORIENTADOR deste projeto? A pergunta é da identidade, não do cargo —
 * e existe separada de `papelNoProjeto` porque a mesma pessoa pode ser as
 * duas coisas (achado do dono, ago/2026: "um coordenador está tentando
 * submeter o projeto de monitoria, mas como docente, e o sistema está
 * travando"). `papelNoProjeto` devolve UM papel só e a gestão vence, então
 * o coordenador que abria o próprio projeto virava "gestao" nele: podia
 * editar, e o `podeSubmeter` — que exigia exatamente "orientador" — recusava.
 * Coordenar o curso não apaga a autoria de quem escreveu o projeto.
 */
export const souOrientadorDo = (p, quem = {}) =>
  !!p && (mesmoEmail(p.criadoPor, quem.email)
    || mesmoEmail(p.orientador?.email, quem.email)
    || mesmoCpf(p.orientador?.cpf, quem.cpf));

export function papelNoProjeto(p, quem = {}) {
  if (!p) return null;
  if (quem.gestao || coordenaOCurso(p, quem)) return "gestao";
  if (souOrientadorDo(p, quem)) return "orientador";
...
/* Submeter é ato de QUEM ESCREVEU o projeto — e o coordenador do curso
   escreve os seus, como qualquer docente. A régua passa a olhar a autoria,
   não o cargo: o que continua valendo é a janela (só enquanto o projeto está
   aberto) e o fato de ninguém submeter projeto alheio — gestão que NÃO é a
   orientação segue sem o botão. */
export const podeSubmeter = (p, quem) =>
  souOrientadorDo(p, quem) && editavelPeloOrientador(p);
/* Decidir é da gestão — inclusive sobre projeto PRÓPRIO (decisão do dono,
   ago/2026: "no modo coordenador, ele pode aprovar seus próprios pedidos
   como docente"). É a mesma régua do ARCHÉ IC: recusar travaria o curso
   inteiro quando quem coordena também leciona, que é o caso comum. O ato não
   fica escondido — `decisaoSobreProjetoProprio` o marca no histórico, e é
   isso que o torna defensável depois. */
export const podeDecidir = (p, quem) =>
  papelNoProjeto(p, quem) === "gestao" && ["submetido", "devolvido"].includes(p?.status);
/** A decisão é sobre projeto do próprio decisor? (para marcar o histórico) */
export const decisaoSobreProjetoProprio = (p, quem) => souOrientadorDo(p, quem);
...
export function visaoDoProjeto(p, quem = {}) {
  ...
  v.papel = papel;
  v.souOrientador = souOrientadorDo(p, quem);
  if (papel === "monitor") {
    ...
    // e o CPF e o telefone do PROFESSOR também não são do monitor (revisão
    // adversarial de set/2026): a régua do colega valia, a do orientador não
    v.orientador = {
      nome: p.orientador?.nome || "", email: p.orientador?.email || "",
      titulacao: p.orientador?.titulacao || "",
    };
  }
  return v;
}
...
export function resumir(p, quem = {}) {
  ...
  return {
    ...
    /* `papel` devolve UM papel só, e a gestão vence: no projeto do próprio
       coordenador ele diz "gestao", e a tela perderia de vista que aquele
       projeto é DELE. `souOrientador` responde a outra pergunta — a da
       autoria —, e é o que separa "meus projetos" de "os do curso" quando a
       mesma pessoa acumula os dois papéis. */
    souOrientador: souOrientadorDo(p, quem),
