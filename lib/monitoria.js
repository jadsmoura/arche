...
export const EDITAIS_MONITORIA = [
  {
    ...
    orgao: "Pró-Reitoria de Pós-Graduação, Pesquisa, Extensão e Ação Comunitária (PROPPEX)",
    ...
    encerrado: false,
    observacao: "Primeiro edital da monitoria sob a PROPPEX. As inscrições, "
      + "o acompanhamento e a certificação correm dentro do ARCHÉ.",
  },
  {
...
/** O edital em vigor — o que o formulário usa como padrão. */
export const EDITAL = EDITAIS_MONITORIA.find((e) => !e.encerrado) || EDITAIS_MONITORIA[0];
...
export const TEXTO_EDITAL = {
  abertura:
    "A Pró-Reitoria de Pós-Graduação, Pesquisa, Extensão e Ação Comunitária (PROPPEX) e a "
    + "Reitoria do Centro Universitário de Goianésia (UNIEGO), no exercício de suas atribuições, "
    + "tornam de conhecimento público o Edital nº 03/2026, datado de 19 de agosto de 2026, o qual "
    + "define as diretrizes gerais para a condução do Processo Seletivo destinado aos acadêmicos "
    + "interessados em participar do Programa de Monitoria Acadêmica durante o segundo período "
    + "letivo de 2026.\n\n"
    + "O plano de Monitoria delineado por este edital seguirá os mesmos critérios preconizados "
    + "pelas regulamentações em vigor. A partir de 2026, por decisão da Reitoria, o Programa de "
    + "Monitoria Acadêmica passa a ser conduzido pela PROPPEX, e a inscrição, o acompanhamento e "
    + "a certificação são feitos no portal ARCHÉ (arche.app.br).",
  secoes: [
    {
    ...
    },
    {
      ...
      itens: [
        ...
        { n: "2.4.", texto: "O Projeto de Monitoria Acadêmica de uma disciplina poderá ter mais de um Monitor, "
          + "desde que seja apresentado o Plano de Trabalho específico para cada Monitor." },
      ],
    },
    {
      ...
      itens: [
        ...
        { n: "3.2.", texto: "No ato da inscrição ao Processo Seletivo de Monitores Acadêmicos, será exigido do candidato(a):", alineas: [
          "Comprovante de matrícula no Centro Universitário de Goianésia, no período letivo em que a inscrição está sendo realizada;",
          "Histórico Escolar da graduação em andamento;",
          "Declaração de disponibilidade para cumprir as horas semanais relativas às atividades de Monitoria Acadêmica da disciplina pretendida, firmada no próprio sistema.",
        ] },
        { n: "3.3.", texto: "A documentação do acadêmico(a) é anexada por ele mesmo no ARCHÉ, junto à ficha de "
          + "inscrição, e fica vinculada ao projeto — não há entrega de papel." },
    ...
    {
      titulo: "4. DA REALIZAÇÃO DE PROCESSO SELETIVO",
      itens: [
        { n: "4.1.", texto: "Havendo mais candidatos às vagas de Monitoria Acadêmica de determinada disciplina, o "
          + "Professor Orientador deverá realizar Processo Seletivo presencial que constará de entrevista com os "
          + "candidatos, avaliando disponibilidade de tempo e competência teórica e técnica para realização das "
          + "atividades propostas." },
        { n: "4.2.", texto: "Realizada a seleção, o Professor Orientador registra no projeto o número de candidatos "
          + "e a data da entrevista, e indica o(s) selecionado(s)." },
      ],
    },
    {
...
export function normalizarMonitor(m, anterior = null) {
  ...
  return {
    ...
    planoTrabalho: txt(o.planoTrabalho ?? base.planoTrabalho).slice(0, 20000),
    declaracao: o.declaracao || base.declaracao
      ...
      : null,
    documentos: {
      matricula: normalizarDocumento(o.documentos?.matricula ?? base.documentos?.matricula),
...
export function normalizarProjeto(p = {}, { anterior = null } = {}) {
  ...
  return {
    ...
    edital: txt(p.edital || base.edital) || EDITAL.numero,
    ciclo: txt(p.ciclo || base.ciclo) || EDITAL.ciclo,
    ...
    selecao: {
      candidatos: num(p.selecao?.candidatos ?? base.selecao?.candidatos),
      realizadaEm: dataISO(p.selecao?.realizadaEm ?? base.selecao?.realizadaEm),
      observacao: txt(p.selecao?.observacao ?? base.selecao?.observacao).slice(0, 4000),
    },
...
export function faltaNoProjeto(p) {
  ...
  mons.forEach((m, i) => {
    ...
    // item 2.4 do edital: mais de um monitor exige plano de trabalho por monitor
    if (mons.length > 1 && txt(m.planoTrabalho).length < 40)
      f.push(`plano de trabalho de ${quem}`);
  });
  return f;
}
...
export function faltaNoCadastroDoMonitor(m) {
  ...
  if (!m?.documentos?.matricula) f.push("comprovante de matrícula");
  return f;
}

/** O histórico escolar é do edital (3.2 b), mas não tranca a inscrição: quem
    depende da secretaria para emitir não pode perder o prazo por isso. Vira
    PENDÊNCIA — amarelo na tela da PROPPEX, que decide caso a caso. */
export function pendenciasDoProjeto(p) {
  const av = [];
  for (const m of p?.monitores || []) {
    if (txt(m.matricula) && !m.documentos?.historico)
      av.push(`${m.nome || "Monitor"}: histórico escolar não anexado`);
  ...
  if (num(p?.selecao?.candidatos) > (p?.monitores || []).length && !dataISO(p?.selecao?.realizadaEm))
    av.push("Houve mais candidatos que vagas e a data da entrevista não foi registrada (item 4 do edital)");
  return av;
}

/** Falta no RELATÓRIO do monitor para ele enviar. */
export function faltaNoRelatorio(r) {
  const f = [];
...
export function panorama(projetos = [], edital = EDITAL.numero) {
