/* ========================================================================
   ...
   API comum: save({buffer, originalName, contentType, prefix, nomeFixo}) -> {fileId, link, name, size}
   nomeFixo: SUBSTITUI o arquivo de mesmo nome na pasta em vez de criar outro
   (decisão do dono, ago/2026: no Drive dele vale a versão MAIS ATUAL de cada
   documento — ata corrigida substitui a anterior, e não fica ao lado dela).
   Vale só para documento GERADO pelo sistema; anexo enviado por gente é
   sempre documento novo e nunca passa por aqui.
...
/* ----------------------------- LOCAL (disco) ---------------------------- */
function initLocal() {
  const abs = (partes) => path.join(UPLOAD_DIR, ...partes.map((x) => slug(x)));
  return {
    mode: "local",
    /* As mesmas operações da migração, no disco — existem para a lógica ser
       testável fora de produção. ATENÇÃO: aqui o fileId É o caminho, então
       mover REESCREVE o id; no Drive (que é onde a migração roda de verdade)
       o id não muda e nenhum link se perde. */
    async acharPasta(partes) {
      const dir = abs(partes);
      try { return (await fsp.stat(dir)).isDirectory() ? dir : null; } catch { return null; }
    },
    async subpastas(partes) {
      try {
        const itens = await fsp.readdir(abs(partes), { withFileTypes: true });
        return itens.filter((i) => i.isDirectory()).map((i) => ({ id: path.join(abs(partes), i.name), nome: i.name }));
      } catch { return []; }
    },
    async contarArquivos(partes) {
      try {
        const itens = await fsp.readdir(abs(partes), { withFileTypes: true });
        return itens.filter((i) => i.isFile()).length;
      } catch { return 0; }
    },
    async arquivos(partes) {
      try {
        const itens = await fsp.readdir(abs(partes), { withFileTypes: true });
        return itens
          .filter((i) => i.isFile())
          .map((i) => ({ id: path.join(abs(partes), i.name), nome: i.name }));
      } catch { return []; }
    },
    async moverPasta(id, destinoPartes, novoNome) {
      const destino = abs(destinoPartes);
      await fsp.mkdir(destino, { recursive: true });
      await fsp.rename(id, path.join(destino, slug(novoNome || path.basename(id))));
      return true;
    },
    async save({ buffer, originalName, prefix, nomeFixo = false }) {
      /* O prefixo passa pelo MESMO `slug` que o Drive aplica em cada nível
         (`ensureFolder`) — senão o disco local guardaria em "Repositório
         Documental/Extensão" o que a migração e a produção guardam em
         "Repositorio-Documental/Extensao": duas árvores paralelas, e o teste
         local deixaria de dizer a verdade sobre produção. */
      const pasta = String(prefix).split("/").filter(Boolean).map(slug).join("/");
      const key = nomeFixo
        ? `${pasta}/${slug(originalName)}`           // substitui o de mesmo nome
        : `${pasta}/${nanoid()}-${slug(originalName)}`;
    ...
    /* Apagar. Existe para a rotina de CÓPIA DIÁRIA aposentar a mais velha
       depois de 30 dias — sem isso o histórico cresceria para sempre no
       Drive do dono. Nenhum fluxo do portal apaga anexo por aqui: o ✕ do
       portfólio tira a REFERÊNCIA e deixa o arquivo onde está. */
    async remove(fileId) {
      const baseAbs = path.resolve(UPLOAD_DIR);
      const dest = path.resolve(UPLOAD_DIR, fileId);
      if (dest !== baseAbs && !dest.startsWith(baseAbs + path.sep)) return false;
...
async function initS3() {
  ...
  return {
    ...
    async save({ buffer, originalName, prefix, nomeFixo = false }) {
      const key = nomeFixo
        ? `${prefix}/${slug(originalName)}`          // mesma chave = substitui
        : `${prefix}/${nanoid()}-${slug(originalName)}`;
    ...
    async remove(fileId) {
      try {
        ...
        await s3.send(new DeleteObjectCommand({ Bucket: process.env.S3_BUCKET, Key: fileId }));
        return true;
      } catch { return false; }
    },
    async serve(fileId, res) {
...
async function initDrive() {
  ...
  /* --------- MIGRAÇÃO PARA O REPOSITÓRIO DOCUMENTAL (ago/2026) ---------
     Mover PASTA no Drive é uma operação só, e o id de tudo que está dentro
     NÃO muda: os links que o sistema já gravou continuam valendo. É por isso
     que a migração move PASTAS e não arquivo por arquivo — mil chamadas de
     API teriam mil chances de falhar pela metade. */
  async function acharPasta(partes) {          // não cria: só encontra
    let atual = ROOT;
    for (const raw of partes) {
      const name = slug(raw);
      const { data } = await drive.files.list({
        q: [`'${atual}' in parents`, "mimeType = 'application/vnd.google-apps.folder'",
          `name = '${name.replace(/'/g, "\\'")}'`, "trashed = false"].join(" and "),
  ...
  return {
    mode: "gdrive",
    acharPasta,
    /** As subpastas de um caminho (para a migração saber o que existe). */
    async subpastas(partes) {
      const pai = await acharPasta(partes);
      if (!pai) return [];
      const out = [];
      let pageToken;
      do {
        ...
        out.push(...(data.files || []).map((f) => ({ id: f.id, nome: f.name })));
        pageToken = data.nextPageToken;
      } while (pageToken);
      return out;
    },
    /** Quantos arquivos há dentro (rasa — serve para o relatório da migração). */
    async contarArquivos(partes) {
      const pai = await acharPasta(partes);
      if (!pai) return 0;
      const { data } = await drive.files.list({
        q: `'${pai}' in parents and trashed = false`,
        fields: "files(id)", pageSize: 1000, supportsAllDrives: true, includeItemsFromAllDrives: true,
      });
      return (data.files || []).length;
    },
    /** Os arquivos soltos de um caminho (a migração precisa movê-los um a um
        quando a pasta de destino já existe e há que FUNDIR as duas). */
    async arquivos(partes) {
      const pai = await acharPasta(partes);
      if (!pai) return [];
      const out = [];
    ...
    /** Move a pasta para outro lugar (e, se pedido, com outro nome). O id
        não muda — nem o dela, nem o de nada que está dentro. Serve também
        para ARQUIVO: no Drive a operação é a mesma. */
    async moverPasta(id, destinoPartes, novoNome) {
      const destino = await ensureFolder(destinoPartes);
      const { data } = await drive.files.get({ fileId: id, fields: "parents", supportsAllDrives: true });
      await drive.files.update({
        fileId: id, addParents: destino,
        removeParents: (data.parents || []).join(","),
        ...(novoNome ? { requestBody: { name: slug(novoNome) } } : {}),
        fields: "id", supportsAllDrives: true,
      });
      return true;
    },
    async save({ buffer, originalName, prefix, nomeFixo = false }) {
      ...
      const media = { mimeType: contentType(originalName), body: Readable.from(buffer) };
      let fileId = null;
      /* nomeFixo: procura o arquivo de mesmo nome NA PASTA e ATUALIZA o
         conteúdo dele. O id não muda (links já dados continuam valendo) e o
         próprio Drive guarda as versões anteriores por 30 dias — a pasta fica
         com uma linha por documento, e o histórico não se perde, só sai da
         frente. Se a busca falhar, cria como sempre: um arquivo a mais é
         melhor que um documento que não foi salvo. */
      if (nomeFixo) {
        try {
          const { data } = await drive.files.list({
            q: `'${folderId}' in parents and name = '${String(originalName).replace(/'/g, "\\'")}' and trashed = false`,
            fields: "files(id)", pageSize: 1, supportsAllDrives: true, includeItemsFromAllDrives: true,
          });
          const achado = data.files?.[0]?.id;
          if (achado) {
            await drive.files.update({ fileId: achado, media, fields: "id", supportsAllDrives: true });
            fileId = achado;
          }
        } catch { /* cai na criação normal */ }
      }
      if (!fileId) {
        const created = await drive.files.create({
          requestBody: { name: originalName, parents: [folderId] },
          media, fields: "id, size", supportsAllDrives: true,
        });
    ...
    async remove(fileId) {
      try { await drive.files.delete({ fileId, supportsAllDrives: true }); return true; }
      catch { return false; }
    },
