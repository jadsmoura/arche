  ...
  return {
    ...
    async serve(fileId, res) {
      const url = await getSignedUrl(
        s3, new GetObjectCommand({ Bucket: process.env.S3_BUCKET, Key: fileId }),
        { expiresIn: 900 },
      );
...
export async function driveContext() {
  ...
  if (!rootId) {
    ...
    const { data } = await drive.files.list({
      q: `'root' in parents and mimeType = 'application/vnd.google-apps.folder' and name = '${name.replace(/'/g, "\\'")}' and trashed = false`,
