...
export function analisarTextoEdital(texto = "") {
  ...
  for (const l of linhas) {
    ...
    if (mAl && item) { item.alineas.push(mAl[2]); continue; }
    const mIn = l.match(RE_INCISO);
    if (mIn && item) { item.romanos.push(mIn[2]); continue; }
    // linha solta: antes da primeira seção é preâmbulo; depois, continuação
    // do item que está aberto (parágrafo sem número, que o edital também usa)
