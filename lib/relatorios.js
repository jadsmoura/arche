...
/* ------------------------------- MONITORIA ----------------------------- */
export function panoramaMonitoria(projetos = [], periodo, { cursos = [] } = {}) {
  const dentro = noSemestre(periodo);
  const nome = (slug) => cursos.find((c) => c.slug === slug)?.nome || slug || "Não informado";
  ...
  const doSem = (projetos || []).filter((p) => p.status !== "rascunho"
    && (txt(p.ciclo) ? txt(p.ciclo) === periodo.chave : dentro(p.inicio, p.fim)));
  const monitores = doSem.flatMap((p) => p.monitores || []);
  const horas = doSem.reduce((s, p) => {
  ...
  const homologados = monitores.filter((m) => m.relatorio?.status === "homologado").length;
  return {
    setor: "monitoria", sigla: "MO", titulo: "Monitoria Acadêmica",
    total: doSem.length,
    numeros: [
      { rotulo: "Projetos de monitoria", valor: doSem.length },
    ...
    quadros: [
      quadro("Projetos por curso", contarPor(doSem, (p) => nome(p.curso))),
      quadro("Situação dos projetos", contarPor(doSem, (p) => p.status)),
      quadro("Monitores por disciplina", Object.fromEntries(doSem.map((p) =>
        [p.disciplina || "(sem disciplina)", (p.monitores || []).length]))),
    ],
    itens: doSem.map((p) => ({
      titulo: p.disciplina || "(sem disciplina)", numero: p.protocolo || "",
      curso: nome(p.curso), responsavel: p.orientador?.nome || "",
      periodo: `${dia(p.inicio)} a ${dia(p.fim)}`,
