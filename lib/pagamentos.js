...
const TRANSICOES = {
  ...
  recusado: ["pago", "expirado", "isento"],        // recusado no cartão: tenta de novo
  expirado: ["pago", "isento"],                     // pagou depois de expirar: vale se houver vaga
...
export function novoPagamento({ valor, categoria, lote, agora = new Date(), reservaMinutos } = {}) {
  ...
  return {
    ...
    preferenciaId: "", pagamentoId: "", link: "",
 ...
 * Confere o VALOR: pago menos que o devido não confirma — vira pendência da
 * coordenação (`divergente`). Mais que o devido confirma e registra a
 * diferença, para a coordenação devolver.
 */
export function lerPagamentoDoProvedor(p, pagamento) {
  if (!p || typeof p !== "object") return { estado: null };
  const estado = estadoDoProvedor(p.status);
  ...
  const divergente = estado === "pago" && pagoCentavos < devido;
  return {
    estado,
    divergente,
    extra: {
      ...
      pagoEmProvedor: p.date_approved ? new Date(p.date_approved).toISOString() : "",
    },
  };
}
