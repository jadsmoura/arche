...
export const centavos = (v) => {
  ...
  if (!s) return 0;
  // só dinheiro escrito como dinheiro: `Number` aceita "1e3" (mil reais) e
  // "0x10" (dezesseis), e a tabela de preços não é lugar para notação
  // exótica passar por engano (endurecimento da revisão de set/2026)
  if (!/^\d{1,3}(?:\.\d{3})+(?:,\d{1,9})?$|^\d{1,12}(?:[.,]\d{1,9})?$/.test(s)) return 0;
...
export function lotesNormalizados(brutos) {
  const lista = (Array.isArray(brutos) ? brutos : []).slice(0, LIMITES.lotes).map((l) => ({
    ...
    ate: dataISO(l?.ate),
    /* O TETO DA CATEGORIA VALE PARA O ACRÉSCIMO (revisão adversarial de
       set/2026): o valor da categoria é capado em `LIMITES.valorMax`, e o
       acréscimo do lote não tinha teto nenhum — como o preço é a soma dos
       dois, o teto era decorativo. Um "999999" digitado onde se queria
       "9,99" publicava um evento cobrando R$ 1 milhão e criava a preferência
       no provedor com esse valor. Não é fraude do participante; é a rede de
       proteção que o arquivo diz ter. */
    acrescimo: Math.min(LIMITES.valorMax * 100, Math.max(0, l?.emCentavos === true
...
export function normalizarVouchers(lista, categorias = []) {
  ...
  return (Array.isArray(lista) ? lista : []).slice(0, LIMITES.vouchers).map((v) => {
    ...
    /* A RESTRIÇÃO POR CATEGORIA NÃO EVAPORA (achado de set/2026): antes, o
       código de categoria que não estivesse mais no catálogo era descartado —
       e lista vazia quer dizer TODAS. Remover ou renomear a categoria na guia
       Cobrança transformava o voucher de 20% "só para Estudante" num voucher
       universal, e um de 100% em inscrição grátis para qualquer um. O que a
       pessoa declarou fica; `voucherValido` é quem recusa, dizendo o motivo. */
    const cats = [...new Set((Array.isArray(v?.categorias) ? v.categorias : []).map((c) => String(c || "").trim()).filter(Boolean))];
 ...
 *  A régua é a MESMA da vaga (`ocupaVaga`, achado de set/2026): a inscrição
 *  com o cartão RECUSADO e a reserva ainda no prazo segurava a vaga e devolvia
 *  o uso do código — o último voucher de um limite de 20 ficava livre para
 *  outra pessoa numa vaga que não existe. Quem ocupa a vaga ocupa o uso. */
...
export function voucherValido(cobranca, codigo, { categoria, hojeISO, usos = 0 } = {}) {
  ...
  if (v.categorias.length && !v.categorias.includes(String(categoria || ""))) {
    ...
    // a categoria a que ele estava preso saiu da guia Cobrança: o voucher não
    // vale para ninguém até alguém corrigi-lo — e a frase diz isso, em vez de
    // "vale só para: ." (o silêncio aqui foi o que já o fez virar universal)
    return { ok: false, motivo: nomes.length ? `Este voucher vale só para: ${nomes.join(", ")}.`
      : "Este código está restrito a uma categoria de inscrição que não existe mais. Fale com a coordenação do evento." };
  }
...
export function valorDaInscricao(cobranca, { categoria, hojeISO, voucher = null } = {}) {
  ...
  // o teto vale no VALOR FINAL, que é onde a garantia importa: categoria e
  // acréscimo são capados um a um, mas quem cobra é a soma
  const semVoucher = cat.valor > 0
    ? Math.min(LIMITES.valorMax * 100, Math.max(0, cat.valor + (lote ? lote.acrescimo : 0)))
    : 0;
...
/* A RESERVA PAUSADA NÃO VENCE (set/2026, com a isenção da taxa pelo CadÚnico):
   quem pede isenção fica esperando a coordenação analisar, e o relógio da
   reserva continuava correndo — a vaga voltava à fila e a pessoa era punida
   pelo tempo de uma análise que não é dela. `pausadoEm` mora DENTRO do
   pagamento de propósito: é aqui que a régua da vaga olha, e passar o inscrito
   inteiro a `reservaVencida` mudaria a assinatura da função mais usada do
   módulo. Ao decidir o pedido a pausa sai e o prazo recomeça inteiro. */
/** Aguardando, não pausada, e já vencida a reserva. */
...
export function transitar(pagamento, para, { por = "sistema", motivo = "", agora = new Date(), extra = {} } = {}) {
  ...
  /* O HISTÓRICO REGISTRA O ATO, não só a mudança de estado (revisão
     adversarial de set/2026): a renovação da reserva é `aguardando →
     aguardando`, e como só se gravava quando o estado mudava, o `motivo`
     "reserva renovada" nunca chegava ao registro — a coordenação não tinha
     como ver que aquela reserva fora renovada sete vezes. Agora o que tem
     MOTIVO entra sempre; o que não tem (releitura idêntica do provedor) só
     entra quando o estado muda, senão o histórico viraria log de poll. */
  if (pagamento.status !== para || motivo) {
    novo.historico = [...(pagamento.historico || []), { em, de: pagamento.status, para, por, ...(motivo ? { motivo } : {}) }].slice(-30);
  }
  if (pagamento.status !== para && para === "pago" && !novo.pagoEm) novo.pagoEm = em;
  return novo;
}
...
export function resumoFinanceiro(inscritos, agora = new Date()) {
  const r = { total: 0, porEstado: {}, porMeio: {}, porCategoria: {}, porVoucher: {},
    bruto: 0, taxa: 0, liquido: 0, aReceber: 0, vencidoSemPagar: 0, descontos: 0 };
  for (const i of inscritos || []) {
    ...
    if (p.status === "pago") {
      ...
      /* "A RECEBER" É DINHEIRO QUE AINDA PODE ENTRAR (revisão adversarial de
         set/2026): o número somava também as reservas já VENCIDAS — a
         varredura que as expira roda de hora em hora, e até ela passar a guia
         Financeiro apresentava como dinheiro a entrar uma vaga que já voltou
         à fila. Separado, `vencidoSemPagar` diz o tamanho do que se perdeu. */
      if (reservaVencida(p, agora)) r.vencidoSemPagar += num(p.valor);
      else r.aReceber += num(p.valor);
    }
  }
  return r;
}
