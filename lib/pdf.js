...
/* ============================ ARCHÉ TR ====================================
   O trabalho submetido a um evento, no modelo do ARCHÉ (pedido do dono,
   set/2026: "se fizermos a submissão na forma de formulário, facilita depois
   gerar os PDFs prontos já nos modelos"). O documento sai do FORMULÁRIO —
   título e título em inglês, autoria com filiação e titulação (o orientador
   por último), resumo e palavras-chave, abstract e keywords e, no trabalho
   completo, as seis seções — no timbre da PROPPEX, com o rodapé de sempre.
   `anonimo: true` tira a autoria (é a cópia que vai ao revisor). */
export async function gerarTrabalhoPdf({ trabalho = {}, evento = {}, versao = null, anonimo = false,
  ...
  const { doc, fim, quebra } = criarDoc();
  const en = v.idioma === "en";
  const titSecao = (pt, ing) => (en ? ing : pt);

  const paragrafo = (s, { italico = false, tamanho = 10, alinhar = "justify", cor = "#222" } = {}) => {
    for (const l of String(s ?? "").split(/\r?\n/)) {
      quebra(14);
      doc.font(italico ? "Helvetica-Oblique" : "Helvetica").fontSize(tamanho).fillColor(cor)
        .text(l || " ", M, doc.y, { width: LARG, align: alinhar, lineGap: 1.5 });
    }
  };
  const rotulo = (s) => { quebra(22); doc.moveDown(0.6);
    doc.font("Helvetica-Bold").fontSize(10.5).fillColor(TEAL).text(s, M, doc.y, { width: LARG }); doc.moveDown(0.15); };

  /* A IDENTIDADE DO EVENTO (pedido do dono, set/2026, mandando a arte do 13º
     CONINT — "Ciência Delas": "seria interessante seguir o tema do evento"):
     a capa cadastrada no ARCHÉ EV entra como faixa no alto da primeira página,
     e o tema sai sob o nome. A arte é a mesma do hotsite; sem capa, o
     cabeçalho é só texto. Formato que o PDFKit não lê (WEBP) é pulado. */
  let y = TOPO;
  if (capa?.buffer) {
    try {
      const altura = 120;   // a arte do CONINT é uma faixa larga: cabe inteira nesta altura
      doc.image(capa.buffer, M, y, { fit: [LARG, altura], align: "center", valign: "center" });
      y += altura + 8;
    } catch { /* arte ilegível: o cabeçalho segue só com o texto */ }
  }
  // cabeçalho: evento, tema, modalidade, número
  const modal = modalidades.find((m) => m.codigo === t.modalidade)?.nome || t.modalidade || "";
  doc.font("Helvetica-Bold").fontSize(9).fillColor(TEAL)
    .text(`${String(evento.nome || "").toUpperCase()}${evento.curso ? ` · ${evento.curso}` : ""}`, M, y, { width: LARG, align: "center" });
  if (evento.tema) doc.font("Helvetica-Oblique").fontSize(9).fillColor(MUTED)
    .text(`Tema: ${evento.tema}`, M, doc.y + 1, { width: LARG, align: "center" });
  doc.font("Helvetica").fontSize(8.5).fillColor(MUTED)
    .text(`${modal}${t.numero ? ` · ${t.numero}` : ""}${v.n > 1 ? ` · versão ${v.n}` : ""}${t.area ? ` · ${t.area}` : ""}`,
      M, doc.y + 2, { width: LARG, align: "center" });
  doc.moveDown(1.2);
  doc.font("Helvetica-Bold").fontSize(14).fillColor("#111")
    .text(String(t.titulo || "").toUpperCase(), M, doc.y, { width: LARG, align: "center" });
  if (t.tituloEn) {
    doc.moveDown(0.3);
    doc.font("Helvetica-Oblique").fontSize(11).fillColor("#333")
      .text(String(t.tituloEn), M, doc.y, { width: LARG, align: "center" });
  }
  doc.moveDown(0.9);

  if (!anonimo) {
    const pessoas = [...(t.autores || []), ...(t.orientador?.nome ? [{ ...t.orientador, orientador: true }] : [])];
    // afiliações numeradas, como nos anais
    const afil = [];
    const idx = (inst) => { const k = String(inst || "").trim() || "—"; let i = afil.indexOf(k); if (i < 0) { afil.push(k); i = afil.length - 1; } return i + 1; };
    const linha = pessoas.map((p) => `${p.nome}${p.orientador ? " (orientador)" : ""}${p.instituicao ? `¹`.replace("¹", String(idx(p.instituicao)).split("").map((d) => "⁰¹²³⁴⁵⁶⁷⁸⁹"[Number(d)]).join("")) : ""}`).join("; ");
    doc.font("Helvetica").fontSize(10).fillColor("#222").text(linha, M, doc.y, { width: LARG, align: "center" });
    doc.moveDown(0.4);
    afil.forEach((a, i) => {
      const quem = pessoas.filter((p) => (String(p.instituicao || "").trim() || "—") === a);
      const tit = [...new Set(quem.map((p) => rotuloTitulacao(p.titulacao)).filter(Boolean))].join(", ");
      doc.font("Helvetica").fontSize(8.5).fillColor(MUTED)
        .text(`${i + 1} ${a}${tit ? ` — ${tit}` : ""}`, M, doc.y, { width: LARG, align: "center" });
    });
    const contato = (t.autores || [])[0]?.email;
    if (contato) doc.font("Helvetica").fontSize(8.5).fillColor(MUTED).text(`Contato: ${contato}`, M, doc.y, { width: LARG, align: "center" });
    doc.moveDown(0.8);
  } else {
    doc.font("Helvetica-Oblique").fontSize(8.5).fillColor(MUTED)
  ...
  rotulo(titSecao("RESUMO", "ABSTRACT"));
  paragrafo(en ? (v.abstract || v.resumo) : v.resumo);
  const pcs = en ? (v.keywords?.length ? v.keywords : v.palavrasChave) : v.palavrasChave;
  if (pcs?.length) paragrafo(`${titSecao("Palavras-chave", "Keywords")}: ${pcs.join("; ")}.`, { italico: false, alinhar: "left" });
  const outroResumo = en ? v.resumo : v.abstract;
  const outrasPcs = en ? v.palavrasChave : v.keywords;
  if (outroResumo) {
    rotulo(titSecao("ABSTRACT", "RESUMO"));
    paragrafo(outroResumo, { italico: true });
    if (outrasPcs?.length) paragrafo(`${titSecao("Keywords", "Palavras-chave")}: ${outrasPcs.join("; ")}.`, { italico: true, alinhar: "left" });
  }
  if (t.modalidade === "completo" && v.secoes) {
    secoes.forEach((s, i) => {
      rotulo(`${i + 1} ${(en ? s.en : s.nome).toUpperCase()}`);
      paragrafo(v.secoes[s.codigo] || "—", { alinhar: s.codigo === "referencias" ? "left" : "justify" });
    });
  }
  finalizar(doc, TIMBRE_PADRAO, marcaEm(evento.periodoFim || undefined));
  return fim;
}
