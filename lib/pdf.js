...
function timbrado(doc, linhas = TIMBRE_PADRAO) {
...
function finalizar(doc, linhasTimbre) {
  const range = doc.bufferedPageRange();
  for (let i = 0; i < range.count; i++) {
    doc.switchToPage(i);
    timbrado(doc, linhasTimbre); rodape(doc);
...
export async function gerarAtaPdf(ata) {
    ...
    for (const { i, reg } of regs) {
      ...
      doc.font("Helvetica").fontSize(8).fillColor(MUTED)
        .text(reg.refs.map((r) => `${r.inst === "curso" ? "Curso" : "IES"} ${r.num} — ${r.nome} (conceito ${r.nivel})`).join(" · ")
          + ` · periodicidade ${reg.periodicidade}`,
          M + 22, doc.y + 1, { width: LARG - 22 });
  ...
  finalizar(doc, timbreDaAta(ata));
  return fim;
}

/* ========================================================================
   ARCHÉ AT — documentos de apoio da sessão.
   A convocação circula ANTES da reunião (o Indicador 2.12 exige que o
   colegiado se reúna "com periodicidade determinada", o que se prova com a
   convocação) e a lista de presença é o papel que roda na mesa.
   ======================================================================== */
export async function gerarConvocacaoPdf(ata) {
  const s = ata.sessao || {};
  const { doc, fim, quebra, secao, campo } = criarDoc();

  doc.font("Helvetica-Bold").fontSize(15).fillColor(TEAL)
    .text("CONVOCAÇÃO", M, TOPO, { width: LARG, align: "center" });
  doc.moveDown(0.3);
  doc.font("Helvetica-Bold").fontSize(11).fillColor("#111")
    .text(tituloDe(ata), { width: LARG, align: "center" });
  doc.font("Helvetica").fontSize(9).fillColor(MUTED)
    .text(`Sessão ${s.tipo || "ordinária"}${s.convocacao ? ` · ${s.convocacao}` : ""}`,
      { width: LARG, align: "center" });
  doc.moveDown(0.8);

  doc.font("Helvetica").fontSize(10).fillColor("#1a1a1a").text(
    `A presidência do ${tituloDe(ata)} convoca os membros para a sessão ` +
    `${s.tipo || "ordinária"} a realizar-se em ${fmtDataLonga(s.data)}` +
    `${s.horaInicio ? `, às ${s.horaInicio}` : ""}` +
    `${s.local ? `, ${s.modalidade === "remota" ? "pela plataforma" : "em"} ${s.local}` : ""}, ` +
    "para deliberar sobre a ordem do dia abaixo.",
    M, doc.y, { width: LARG, align: "justify", lineGap: 1.6, indent: 18 });

  secao("Ordem do dia");
  const pontos = (ata.pauta || []).filter((p) => p.titulo);
  if (!pontos.length) {
    doc.font("Helvetica").fontSize(9.5).fillColor("#222").text("—", M, doc.y, { width: LARG });
  }
  for (const [i, p] of pontos.entries()) {
    ...
    doc.font("Helvetica").fontSize(9.5).fillColor("#1a1a1a")
      .text(p.titulo, M + 22, y, { width: LARG - 22 });
    const reg = pautaDe(p.pautaMec);
    if (reg) {
      doc.font("Helvetica").fontSize(8).fillColor(MUTED)
        .text(`pauta regulatória · ${reg.refs.map((r) => `${r.inst === "curso" ? "Curso" : "IES"} ${r.num}`).join(", ")}`,
          M + 22, doc.y + 1, { width: LARG - 22 });
    }
    doc.moveDown(0.3); doc.x = M;
  }
  ...
  for (const p of ata.participantes || []) {
    ...
    doc.font("Helvetica").fontSize(9).fillColor("#222")
      .text(`• ${p.nome}${p.cargo ? ` — ${p.cargo}` : ""}${p.condicao === "convidado" ? " (convidado)" : ""}`,
        M + 4, doc.y, { width: LARG - 4 });
  }

  quebra(120);
  doc.moveDown(1.8);
  doc.font("Helvetica").fontSize(9.5).fillColor("#222")
    .text(`${CIDADE_PDF}, ${fmtDataLonga(ata.criadoEm ? String(ata.criadoEm).slice(0, 10) : s.data)}.`,
      M, doc.y, { width: LARG, align: "center" });
  doc.moveDown(2.6);
  const y = doc.y;
  doc.moveTo(M + 95, y).lineTo(M + LARG - 95, y).lineWidth(0.8).strokeColor("#333").stroke();
  doc.font("Helvetica").fontSize(8).fillColor(MUTED)
    .text("Presidência", M, y + 3, { width: LARG, align: "center" });
  doc.font("Helvetica-Bold").fontSize(9.5).fillColor("#111")
    .text(ata.presidencia?.nome || "____________________", M, doc.y + 1, { width: LARG, align: "center" });
