...
import { blocosDoRico } from "./richtext.js";
...
/* A MESMA IMAGEM SE ABRE UMA VEZ POR DOCUMENTO (set/2026, achado a partir do
   relato de que os termos "não estavam gerando" e do alerta de health check
   do Render). O PDFKit só REAPROVEITA imagem quando a fonte é uma STRING —
   caminho de arquivo ou data URI, que ele guarda em `_imageRegistry`. Com um
   BUFFER ele cai direto em `openImage()`: decodifica o PNG e embute um
   XObject NOVO a cada chamada. E as assinaturas vêm do estado como base64 →
   Buffer, e são desenhadas uma vez POR PÁGINA: num lote de 54 termos com
   duas assinaturas, são 108 decodificações e 108 cópias da mesma imagem
   dentro do arquivo. Medido aqui: 176 KB → 919 KB só por isso, e em produção
   — onde a CPU é fatiada e as assinaturas digitalizadas são maiores — um PDF
   de 4 páginas levava 6,5 s contra 0,05 s local.
   Abrindo uma vez e passando o objeto adiante, o PDFKit entra no ramo
   `src.width && src.height` e apenas REFERENCIA o XObject já embutido. O
   cache é POR DOCUMENTO (`WeakMap` na instância) porque `image.embed(doc)`
   liga a imagem àquele documento; caminho em string passa direto, que é o
   caso dos logotipos do timbre — esses o PDFKit já cacheava sozinho. */
const IMAGENS_ABERTAS = new WeakMap();
function imagemDoDoc(doc, src) {
  if (!src || typeof src === "string") return src;
  let cache = IMAGENS_ABERTAS.get(doc);
  if (!cache) { cache = new Map(); IMAGENS_ABERTAS.set(doc, cache); }
  if (!cache.has(src)) {
    let img = null;
    try { img = doc.openImage(src); } catch { /* imagem ilegível: fica a linha */ }
    cache.set(src, img);
  }
...
function assinaturaSobreLinha(doc, img, centroX, yLinha) {
  const im = imagemDoDoc(doc, img);
  if (!im) return;
  try {
    doc.image(im, centroX - ASS_LARG / 2, yLinha - ASS_ALT - ASS_FOLGA,
...
/* Data POR EXTENSO a partir de um ISO (dia ou carimbo completo). Existe para
   o documento que afirma um ato datado poder levar a data DO ATO, e não a do
   dia em que alguém o reabriu — o resultado publicado é o caso: ele é gerado
   na hora, a cada visita. Sem data reconhecível, devolve "" e quem chama cai
   no `hojeExtenso()`. */
function dataExtenso(iso) {
  const [y, m, d] = String(iso || "").slice(0, 10).split("-").map(Number);
  return y && m >= 1 && m <= 12 && d ? `${d} de ${MESES_PT[m - 1]} de ${y}` : "";
}
function hojeExtenso() {
  const n = new Date();
  return `${n.getDate()} de ${MESES_PT[n.getMonth()]} de ${n.getFullYear()}`;
}
...
/* A ÚLTIMA DEFESA DO DOCUMENTO OFICIAL (achado do dono, ago/2026): as fontes
   ...
   É também aqui que a MARCA da instalação entra (`vestir`, lib/produto.js):
   o rodapé de cada página, o "Documento emitido pelo ARCHÉ" dos certificados
   e os cabeçalhos de módulo são texto desenhado, e todo texto desenhado
   passa por este ponto. Uma instituição que troque `APP_MARCA` recebe os
   documentos com o nome dela sem que nenhuma linha do gerador mude; com a
   variável no padrão, `vestir` devolve a mesma string e nada acontece. */
function blindarTexto(doc) {
  const original = doc.text.bind(doc);
  doc.text = (t, ...resto) => original(typeof t === "string" ? vestir(limparColagem(t)) : t, ...resto);
  return doc;
}
...
export async function gerarAtaPdf(ata, { assinaturas = {} } = {}) {
  ...
  const desenharImagem = (buf, x, larguraCaixa, yLinha) => {
    const im = imagemDoDoc(doc, buf);   // uma folha de ata tem dezenas de assinantes
    if (!im) return;
    try {
      // ancorada no rodapé da caixa: a assinatura "pousa" sobre a linha
      doc.image(im, x, yLinha - ALTURA_IMG - 1, {
...
/* ========================================================================
   ...
   `publicadoEm` é a data em que AQUELA FASE foi publicada: o documento não
   se arquiva, se regera a cada visita, e sem ela o preliminar reaberto em
   dezembro sairia datado de dezembro — contradizendo o prazo de contestação,
   que corre justamente da publicação dele. Sem a data, vale a de hoje.
   ======================================================================== */
const ROTULO_LINHA = { ic: "IC", it: "IT", ie: "IE" };

export async function gerarResultadoEditalPdf({ edital = {}, projetos = [], emitidoPor = "", fase = "final", publicadoEm = "", assinaturas = {} } = {}) {
  ...
  const quadro = (titulo, lista) => {
    ...
    for (const p of lista) {
      ...
      /* COMO A PROPOSTA ENTROU NÃO SAI NO RESULTADO (pedido do dono,
         ago/2026, revendo a régua anterior — que a escondia só no
         preliminar): o resultado publica MÉRITO. "Inclusão deferida fora do
         prazo" é ato interno do processo, e impresso ao lado do nome de um
         professor num documento público lê-se como ressalva sobre ele. O
         registro continua onde prova: no histórico do projeto, na ficha da
         gestão e na tela de quem coordena. */
  ...
  doc.moveDown(1.4);
  doc.font("Helvetica").fontSize(9.5).fillColor("#222")
    .text(`${CIDADE_PDF}, ${dataExtenso(publicadoEm) || hojeExtenso()}.`, M, doc.y, { width: LARG, align: "center" });
...
export async function gerarResultadoEMPdf({ turma = {}, bolsistas = [], emitidoPor = "", fase = "final", publicadoEm = "", assinaturas = {} } = {}) {
  const { doc, fim, quebra, secao } = criarDoc();
  const preliminar = fase === "preliminar";
...
function desenharAssinaturas(doc, assinaturas, { X, L, y }) {
  ...
  lista.forEach((a, i) => {
    const x = X + i * w;
    if (a.img) {
      try { const im = imagemDoDoc(doc, a.img); if (im) doc.image(im, x + w / 2 - 55, y - 46, { fit: [110, 42], align: "center" }); }
...
/* ========================= termos do ICEM (Ensino Médio) =================
   ...
   responsável e, pela instituição, o pró-reitor e o reitor (os mesmos dos
   termos da graduação); a autorização é o que permite ao aluno frequentar a
   instituição e receber a bolsa, e ela é declaração do responsável — por
   isso continua com as duas linhas dele e do aluno, e só.
   ======================================================================== */
export async function gerarTermosEMPdf({ turma = {}, bolsistas = [], assinaturas = {}, emitidoPor = "" } = {}) {
  ...
  bolsistas.forEach((b, i) => {
    ...
    doc.font("Helvetica").fontSize(9.5).fillColor("#222")
      .text(`${CIDADE_PDF}, ${hojeExtenso()}.`, M, doc.y, { width: LARG, align: "right" });
    /* QUEM ASSINA PELA INSTITUIÇÃO É O PRÓ-REITOR E O REITOR, como nos termos
       da graduação (correção do dono, set/2026: "tem a assinatura só do
       Wagner"). O termo de compromisso é ato da INSTITUIÇÃO com o bolsista e
       o responsável dele — quem o firma por ela é quem firma os demais, e a
       coordenação de pesquisa conduz o programa sem ser parte do contrato.
       Duas RÉGUAS de duas colunas, não uma de quatro: as partes em cima (que
       assinam à caneta na cerimônia) e a instituição embaixo, com as imagens
       do banco — em quatro colunas de 120 pt o cargo do pró-reitor sairia em
       cinco linhas. */
    regua([
...
/* ============ O EDITAL ESCRITO NO PORTAL, no layout institucional ==========
   ...
   Podem, e sem passar por mim. Este é o MESMO desenho do edital da monitoria
   — caixa do número, título em versalete, seções em faixa, itens numerados
   com alíneas e incisos, cronograma em quadro e as assinaturas do banco —,
   com uma diferença que é o ponto: o TEXTO vem do cadastro, escrito no
   portal, e não de uma constante do código.

   O timbre é o do ÓRGÃO que expede (`orgao`): um edital da PROAC com o timbre
   da PROPPEX afirmaria que quem o expediu foi outra pró-reitoria. */
export async function gerarEditalPdf({ edital = {}, corpo = {}, cronograma = [],
  orgao = "proppex", assinam = [], assinaturas = {} } = {}) {
  const marca = marcaEm(edital.publicadoEm || edital.vigencia?.inicio);
  const { doc, fim, quebra, secao } = criarDoc();
  const proac = orgao === "proac";

  doc.font("Helvetica-Bold").fontSize(15).fillColor(TEAL)
    .text(`Edital ${edital.numero || "—"}`, M, TOPO, { width: LARG, align: "center" });
  doc.font("Helvetica").fontSize(10).fillColor(MUTED)
    .text(proac ? "PROAC" : "PROPPEX", M, doc.y + 2, { width: LARG, align: "center" });
  doc.moveTo(M + 150, doc.y + 8).lineTo(M + LARG - 150, doc.y + 8)
    .lineWidth(0.8).strokeColor(LINE).stroke();
  doc.font("Helvetica-Bold").fontSize(11.5).fillColor("#111")
    .text((edital.titulo || "").toUpperCase(), M, doc.y + 16, { width: LARG, align: "center" });
  if (edital.vigencia?.inicio && edital.vigencia?.fim) {
    doc.font("Helvetica").fontSize(9).fillColor(MUTED)
      // ano por INTEIRO: `fmtData` corta em dois dígitos (convenção do
      // formulário da extensão), e num edital que vai à comunidade "01/01/27"
      // é o tipo de economia que gera dúvida
      .text(`Vigência: ${fmtDataLonga(edital.vigencia.inicio)} a ${fmtDataLonga(edital.vigencia.fim)}`,
        M, doc.y + 4, { width: LARG, align: "center" });
  }
  doc.moveDown(1);
  ...
  if (corpo.abertura) {
    doc.font("Helvetica").fontSize(9.5).fillColor("#222")
      .text(String(corpo.abertura), M, doc.y + 6, { width: LARG, align: "justify", lineGap: 3 });
  }

  for (const s of corpo.secoes || []) {
    if (s.titulo) secao(s.titulo);
    for (const it of s.itens || []) {
      quebra(24);
      if (it.n) {
        doc.font("Helvetica-Bold").fontSize(9.5).fillColor("#111")
        ...
        doc.font("Helvetica").fontSize(9.5).fillColor("#222")
          .text(it.texto, M, doc.y, { width: LARG, align: "justify", lineGap: 2 });
      }
      /* O rótulo impresso é o que a pessoa ESCREVEU (`{ r, t }` vindo de
         analisarTextoEdital); a string solta continua aceita, numerada em
         sequência, para o texto da monitoria que vive no código. */
      const ROM = ["I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII"];
      (it.alineas || []).forEach((a, i) => {
        quebra(16);
        const rot = typeof a === "object" ? a.r : `${String.fromCharCode(97 + i)})`;
        doc.font("Helvetica").fontSize(9).fillColor("#222")
          .text(`${rot} ${typeof a === "object" ? a.t : a}`, M + 18, doc.y + 2,
            { width: LARG - 18, align: "justify", lineGap: 1.5 });
      });
      (it.romanos || []).forEach((r, i) => {
        quebra(16);
        const rot = typeof r === "object" ? r.r : `${ROM[i] || i + 1}.`;
        doc.font("Helvetica").fontSize(9).fillColor("#222")
          .text(`${rot} ${typeof r === "object" ? r.t : r}`,
            M + 18, doc.y + 2, { width: LARG - 18, align: "justify", lineGap: 1.5 });
      });
      doc.x = M;
    }
  ...
  /* O CRONOGRAMA em quadro — o mesmo do edital da monitoria. A altura da
     linha é a da célula mais alta, medida ANTES de escrever: deduzi-la do
     doc.y depois daria a altura da última, e a linha seguinte subiria por
     cima de um responsável que ocupa duas linhas. */
  if (cronograma.length) {
    secao("CRONOGRAMA");
    const cols = [230, 165, 88];
    const linha = (a, b, c, cab = false) => {
    ...
    linha("Etapa", "Responsável", "Até", true);
    for (const c of cronograma) linha(c.etapa, c.responsavel || "", fmtDataLonga(c.ate));
  }

  quebra(60);
  doc.font("Helvetica").fontSize(9.5).fillColor("#222")
    .text(`${CIDADE_PDF}-GO, ${fmtDataLonga(edital.publicadoEm || new Date().toISOString().slice(0, 10))}.`,
      M, doc.y + 20, { width: LARG, align: "right" });
  /* Quem assina é o ÓRGÃO que expede: a PROAC pela pró-reitora acadêmica, a
     PROPPEX pelo pró-reitor — e o reitor nos dois, que é como os editais do
     acervo saem. As imagens vêm do banco, como em todo documento do portal. */
  const padrao = proac
    ? [{ ...ASSINA.proReitoraAcademica, img: "proacademica" }, { ...ASSINA.reitor, img: "reitor" }]
    : [{ ...ASSINA.proReitor, img: "proreitor" }, { ...ASSINA.reitor, img: "reitor" }];
  blocoAssinaturas(doc, quebra, assinam.length ? assinam : padrao, assinaturas);

  finalizar(doc, proac ? TIMBRE_PROAC : TIMBRE_PADRAO, marca);
  return fim;
}
...
/* Os 17 ODS pelo número — o registro guarda só o número, e é aqui que ele
   vira nome no documento. A lista canônica está em lib/praticas.js. */
const ODS_NOMES = {
...
export async function gerarRelatorioAulaPraticaPdf({
  ...
  if (ext) {
    ...
    if ((r.ods || []).length) {
      secao("OBJETIVOS DE DESENVOLVIMENTO SUSTENTÁVEL (ODS) CONTEMPLADOS");
      for (const c of r.ods) campo(`ODS ${c}`, ODS_NOMES[c] || "");
    }
    ...
    /* Os anexos que NÃO são foto (pedido do dono, ago/2026: "permita anexo de
       outros documentos além de fotos") saem também no relatório de aula
       prática — plano de aula, roteiro do laboratório, lista de presença. Como
       na extensão, vão por LINK: o arquivo já está no sistema com endereço
       próprio, e embuti-lo deixaria o documento muitas vezes maior. */
    if ((r.evidencias || []).length) {
      secao("4 — DOCUMENTOS ANEXADOS");
      for (const e of r.evidencias) campo(e.nome || "anexo", e.link || `/api/files/${e.fileId || ""}`);
    }

    if (r.observacao) { secao(`${(r.evidencias || []).length ? 5 : 4} — OBSERVAÇÕES`); texto(r.observacao); }
  }
  ...
  if (r.parecer?.decisao === "validado") {
    ...
    campo("Validado por", (r.parecer.nome || r.parecer.por || "—")
      /* As duas assinaturas do pé serão da MESMA pessoa quando quem conduziu
         a atividade também coordena o curso (decisão do dono, set/2026). O
         documento DIZ isso: dois papéis, não duas pessoas. */
      + (r.parecer.proprioRelatorio ? " — próprio responsável, na condição de coordenação do curso" : ""));
...
/* ============================ ARCHÉ TR ====================================
   O trabalho submetido a um evento, no MODELO DO EVENTO (o dono mandou o
   Modelo_CONINT13.docx em set/2026, depois de recusar a primeira versão —
   "não gostei dos modelos em PDF; é possível eu enviar um?"). O documento
   sai do FORMULÁRIO, e o desenho é o do Word, medido no próprio arquivo:

   · A4 com 2,5 cm de margem em todos os lados, Times New Roman 12, entrelinha
     1,5 e texto justificado — é o que a comissão pede aos autores, e o PDF
     gerado não pode contradizer as normas que a página de submissão publica;
   · a ARTE do evento (a capa cadastrada no ARCHÉ EV) como faixa no alto da
     primeira página, e sob ela o nome do evento em ciano (#00C8E8), o azul
     da própria arte;
   · título em negrito 14 no azul-marinho (#061B2A); autores em 11 com a
     afiliação em SOBRESCRITO e o orientador por ÚLTIMO; as afiliações
     numeradas em cinza 10; a linha "Autor apresentador | E-mail" em itálico
     azul-petróleo (#0B5E7A);
   · "Resumo" como título de seção, "Palavras-chave:" em negrito na mesma
     linha do texto, e as seções numeradas em caixa alta — MATERIAL E
     MÉTODOS, RESULTADOS E DISCUSSÃO —, com REFERÊNCIAS sem número;
   · o rodapé do modelo em TODAS as páginas: a linha ciano de 2 pt e os dois
     logotipos — UNIEGO à esquerda, PROPPEX à direita —, fora do timbre da
     pró-reitoria que os demais documentos do ARCHÉ usam. O logotipo da
     PROPPEX que veio no .docx tinha FUNDO CINZA (achado do dono: "as logos
     do PROPPEX também não estão com fundo transparente"); a versão em
     templates/ saiu dele com o fundo removido.

   O que o modelo não tem e o formulário pede — o título em inglês, o
   abstract e as keywords — entra no mesmo desenho, logo depois do bloco
   correspondente em português; a TITULAÇÃO de cada autor fica fora do PDF
   (o modelo não a imprime; ela vive no registro e na planilha da gestão).
   No trabalho em inglês, o abstract vem antes do resumo. `anonimo: true`
   tira a autoria inteira (é a cópia que vai ao revisor).

   Os sobrescritos são DESENHADOS (fonte menor, levantada): o repertório
   WinAnsi das fontes padrão só tem ¹²³, e o quarto autor não pode sair sem
   número. */
const LOGO_PROPPEX = path.join(__dirname, "..", "templates", "logo-proppex.png");
const TR_MARGEM = 71, TR_LARG = 453.5;      // 2,5 cm; a largura útil do A4
const TR_LIMITE = 742;                       // onde o corpo termina (o rodapé começa em 760)
const TR_AZUL = "#061b2a", TR_CIANO = "#00c8e8", TR_CINZA = "#666666", TR_PETROLEO = "#0b5e7a";
const TR_CORPO = 12, TR_ENTRELINHA = 6.5;    // Times 12 com entrelinha 1,5

export async function gerarTrabalhoPdf({ trabalho = {}, evento = {}, versao = null, anonimo = false,
  secoes = [], rotuloTitulacao = (x) => x, modalidades = [], capa = null, imagens = new Map() } = {}) {
  const t = trabalho;
  const v = versao || (t.versoes || [])[(t.versoes || []).length - 1] || {};
  const M = TR_MARGEM, LARG = TR_LARG;
  const doc = blindarTexto(new PDFDocument({
    size: "A4", margins: { top: M, bottom: 842 - TR_LIMITE, left: M, right: M }, bufferPages: true,
  }));
  const chunks = [];
  doc.on("data", (c) => chunks.push(c));
  const fim = new Promise((res) => doc.on("end", () => res(Buffer.concat(chunks))));
  ...
  const quebra = (alt) => { if (doc.y + alt > TR_LIMITE) doc.addPage(); };
  const tituloSecao = (s) => {
    quebra(64); doc.y += 8;
    doc.font("Times-Bold").fontSize(TR_CORPO).fillColor(TR_AZUL).text(s, M, doc.y, { width: LARG });
    doc.y += 4;
  };
  const paragrafoSimples = (s, { alinhar = "justify", italico = false } = {}) => {
    const linhas = String(s ?? "").split(/\r?\n/);
    for (const l of linhas) {
      if (!l.trim()) { doc.y += 6; continue; }
      quebra(26);
      doc.font(italico ? "Times-Italic" : "Times-Roman").fontSize(TR_CORPO).fillColor("#111")
        .text(l, M, doc.y, { width: LARG, align: alinhar, lineGap: TR_ENTRELINHA });
      doc.y += 4;
    }
  };
  /* ---- O TEXTO RICO (set/2026) ----------------------------------------
     O autor escreve com ênfase, listas e imagens (pedido do dono: "alguns
     trabalhos precisam disso"), e o que fica gravado é HTML saneado. Aqui ele
     vira DESENHO: `blocosDoRico` parte o HTML em parágrafos, itens de lista e
     imagens, e cada parágrafo em TRECHOS com as marcas que valem em cada
     pedaço — é isso que permite trocar de fonte no meio da linha
     (`continued`). Texto sem marcação nenhuma (o campo antigo) sai por aqui
     exatamente como saía antes: um parágrafo por linha.

     Sobrescrito e subscrito saem em corpo MENOR: as fontes padrão do PDF só
     têm ¹²³ no repertório WinAnsi, e o PDFKit não desloca a linha de base no
     meio de um parágrafo justificado. O expoente que o WinAnsi tem (m², cm³)
     usa o caractere de verdade; o resto encolhe, que é legível e não inventa
     glifo. */
  const SUP_WINANSI = { 1: "¹", 2: "²", 3: "³" };
  const fonteDoTrecho = (r) => (r.b && r.i ? "Times-BoldItalic" : r.b ? "Times-Bold" : r.i ? "Times-Italic" : "Times-Roman");
  const desenharTrechos = (trechosCrus, { alinhar = "justify", recuo = 0 } = {}) => {
    const larg = LARG - recuo;
    /* O ESPAÇO DA FRONTEIRA vai para o trecho ANTERIOR: com `continued`, o
       PDFKit apara o espaço INICIAL de cada pedaço ao quebrar a linha, e
       "mais <b>prevalente</b> no mundo" saía "prevalentено mundo". Empurrar
       o espaço para o fim do pedaço de trás resolve sem mexer no texto. */
    const trechos = trechosCrus.map((r) => ({ ...r }));
    for (let i = 1; i < trechos.length; i++) {
      const m = /^(\s+)([\s\S]*)$/.exec(trechos[i].t);
      if (!m) continue;
      trechos[i].t = m[2];
      // o espaço não pode entrar SUBLINHADO: ele vira um pedaço só dele,
      // sem a marca, senão o traço passa do fim da palavra
      if (trechos[i - 1].u) trechos.splice(i, 0, { ...trechos[i - 1], t: " ", u: false });
      else trechos[i - 1].t += " ";
    }
    trechos.forEach((r, i) => {
      ...
      let texto = r.t;
      let corpo = TR_CORPO;
      if (r.sup && [...texto].every((c) => SUP_WINANSI[c])) texto = [...texto].map((c) => SUP_WINANSI[c]).join("");
      else if (r.sup || r.sub) corpo = TR_CORPO * 0.72;
      doc.font(fonteDoTrecho(r)).fontSize(corpo).fillColor("#111")
        .text(texto, i === 0 ? M + recuo : doc.x, i === 0 ? doc.y : doc.y, {
          width: larg, align: alinhar, lineGap: TR_ENTRELINHA, continued: !ultimo,
          underline: !!r.u,
        });
    });
  };
  /** Uma imagem colada pelo autor: centrada, na largura útil, com a legenda. */
  const desenharImagemDoTexto = (bloco) => {
    const buf = imagens.get(bloco.id);
    if (!buf) return;                                     // arquivo ilegível: a linha some
    try {
      const img = doc.openImage(buf);
      const escala = Math.min(1, LARG / img.width, 320 / img.height);
      const w = img.width * escala, h = img.height * escala;
      quebra(h + 26);
      doc.y += 6;
      doc.image(img, M + (LARG - w) / 2, doc.y, { width: w, height: h });
      doc.y += h + 3;
      if (bloco.alt) {
        doc.font("Times-Italic").fontSize(9.5).fillColor(TR_CINZA)
          .text(bloco.alt, M, doc.y, { width: LARG, align: "center" });
      }
      doc.y += 6; doc.x = M;
    } catch { /* imagem que o PDFKit não lê (HEIC, SVG): o texto segue */ }
  };
  const corpoRico = (html, { alinhar = "justify" } = {}) => {
    const blocos = blocosDoRico(html);
    if (!blocos.length) { paragrafoSimples("—"); return; }
    for (const b of blocos) {
      if (b.tipo === "img") { desenharImagemDoTexto(b); continue; }
      if (!b.trechos.length) { doc.y += 6; continue; }     // linha em branco
      quebra(26);
      if (b.tipo === "li") {
        const recuo = 16 + (b.nivel || 0) * 14;
        const y = doc.y;
        doc.font("Times-Roman").fontSize(TR_CORPO).fillColor("#111")
          .text(b.marcador || "•", M + recuo - 14, y, { width: 14, lineBreak: false });
        doc.y = y;
        desenharTrechos(b.trechos, { alinhar: "left", recuo });
      } else {
        desenharTrechos(b.trechos, { alinhar });
      }
      doc.x = M; doc.y += 4;
    }
  };
  // "Palavras-chave: a; b; c." — o rótulo em negrito, na mesma linha do texto
  const rotuloInline = (r, s) => {
    quebra(26);
    doc.font("Times-Bold").fontSize(TR_CORPO).fillColor(TR_AZUL)
      .text(`${r}: `, M, doc.y, { width: LARG, align: "justify", lineGap: TR_ENTRELINHA, continued: true })
      .font("Times-Roman").fillColor("#111").text(String(s));
    doc.y += 8;
  };

  /* Uma linha CENTRADA com sobrescritos: `palavras` é a lista de
     { t, pre, sup, apos } — `pre` é o sobrescrito ANTES da palavra (as
     afiliações), `sup` o de DEPOIS (os autores), `apos` o que gruda sem
     espaço (o ponto e vírgula). Quebra em várias linhas quando não cabe. */
  const linhaComSobrescritos = (palavras, { fonte = "Times-Roman", tamanho = 11, cor = "#111" } = {}) => {
    const sup = tamanho * 0.62, sobe = tamanho * 0.36;
    const larg = (p) => {
      doc.font(fonte).fontSize(tamanho); let w = doc.widthOfString(p.t);
      doc.fontSize(sup); if (p.pre) w += doc.widthOfString(p.pre); if (p.sup) w += doc.widthOfString(p.sup);
      doc.fontSize(tamanho); if (p.apos) w += doc.widthOfString(p.apos);
      return w;
    };
    doc.font(fonte).fontSize(tamanho);
    const espaco = doc.widthOfString(" ");
    const linhas = [[]]; let atual = 0;
    for (const p of palavras) {
      const w = larg(p);
      if (linhas[linhas.length - 1].length && atual + espaco + w > LARG) { linhas.push([]); atual = 0; }
      const l = linhas[linhas.length - 1];
      atual += (l.length ? espaco : 0) + w; l.push({ ...p, w });
    }
    const altura = tamanho * 1.25;
    for (const l of linhas) {
      quebra(altura + 2);
      const total = l.reduce((s, p) => s + p.w, 0) + espaco * (l.length - 1);
      let x = M + (LARG - total) / 2; const y = doc.y;
      for (const p of l) {
        if (p.pre) { doc.font(fonte).fontSize(sup).fillColor(cor).text(p.pre, x, y - sobe, { lineBreak: false }); x += doc.widthOfString(p.pre); }
        doc.font(fonte).fontSize(tamanho).fillColor(cor).text(p.t, x, y, { lineBreak: false }); x += doc.widthOfString(p.t);
        if (p.sup) { doc.fontSize(sup).text(p.sup, x, y - sobe, { lineBreak: false }); x += doc.widthOfString(p.sup); doc.fontSize(tamanho); }
        if (p.apos) { doc.text(p.apos, x, y, { lineBreak: false }); x += doc.widthOfString(p.apos); }
        x += espaco;
      }
      doc.y = y + altura;
    }
  ...
  /* ---- a primeira página: a arte do evento e o nome dele ---- */
  doc.y = M;
  if (capa?.buffer) {
    try {
      // a altura da faixa se calcula ANTES de desenhar: o PDFKit não anda o
      // doc.y quando recebe a posição explícita, e o título saía por cima da arte
      const img = doc.openImage(capa.buffer);
      const escala = Math.min(LARG / img.width, 200 / img.height);
      const w = img.width * escala, h = img.height * escala;
      doc.image(img, M + (LARG - w) / 2, doc.y, { width: w, height: h });
      doc.y += h + 3;
    } catch { /* arte ilegível: o cabeçalho segue só com o nome */ }
  }
  const nomeEvento = String(evento.nome || "").replace(/\s+[—–-]\s+/g, "  |  ");
  const linhaEvento = nomeEvento + (!capa?.buffer && evento.tema ? `  |  ${evento.tema}` : "");
  if (linhaEvento) {
    doc.font("Times-Bold").fontSize(10).fillColor(TR_CIANO).text(linhaEvento, M, doc.y, { width: LARG, align: "center" });
    doc.y += 2;
  }

  /* ---- título, autoria e afiliações ---- */
  doc.y += 8;
  doc.font("Times-Bold").fontSize(14).fillColor(TR_AZUL).text(String(t.titulo || ""), M, doc.y, { width: LARG, align: "center" });
  doc.y += 4;
  if (t.tituloEn) {
    doc.font("Times-Italic").fontSize(11).fillColor(TR_AZUL).text(String(t.tituloEn), M, doc.y, { width: LARG, align: "center" });
    doc.y += 4;
  }
  if (!anonimo) {
    const pessoas = [...(t.autores || []), ...(t.orientador?.nome ? [{ ...t.orientador, orientador: true }] : [])];
    const afil = [];
    const idx = (inst) => { const k = String(inst || "").trim() || "—"; let i = afil.indexOf(k); if (i < 0) { afil.push(k); i = afil.length - 1; } return String(i + 1); };
    const palavras = [];
    pessoas.forEach((p, i) => {
      const ws = String(p.nome || "").trim().split(/\s+/).filter(Boolean);
      const n = idx(p.instituicao);
      ws.forEach((w, j) => {
        const ultima = j === ws.length - 1;
        palavras.push({ t: w, sup: ultima ? n : "", apos: ultima && i < pessoas.length - 1 ? ";" : "" });
      });
    });
    linhaComSobrescritos(palavras, { tamanho: 11, cor: TR_AZUL });
    doc.y += 1;
    const afils = [];
    afil.forEach((a, i) => {
      const ws = a.split(/\s+/).filter(Boolean);
      ws.forEach((w, j) => afils.push({ t: w, pre: j === 0 ? String(i + 1) : "", apos: j === ws.length - 1 && i < afil.length - 1 ? ";" : "" }));
    });
    linhaComSobrescritos(afils, { tamanho: 10, cor: TR_CINZA });
    doc.y += 2;
    const ap = (t.autores || [])[0] || {};
    if (ap.nome) {
      doc.font("Times-Italic").fontSize(10).fillColor(TR_PETROLEO)
        .text(`${pt("Autor apresentador", "Presenting author")}: ${ap.nome}${ap.email ? `  |  E-mail: ${ap.email}` : ""}`, M, doc.y, { width: LARG, align: "center" });
    }
    doc.y += 7;
  } else {
    doc.font("Times-Italic").fontSize(10).fillColor(TR_CINZA)
      .text("Cópia para avaliação cega — a autoria foi suprimida.", M, doc.y, { width: LARG, align: "center" });
    doc.y += 7;
  }

  /* ---- resumo e palavras-chave (e o par no outro idioma, quando há) ---- */
  const blocoResumo = (titulo, texto, rotuloPc, pcs) => {
    if (!texto) return;
    tituloSecao(titulo);
    corpoRico(texto);
    if (pcs?.length) rotuloInline(rotuloPc, `${pcs.join("; ")}.`);
  };
  const principal = en ? [pt("Resumo", "Abstract"), v.abstract || v.resumo, "Keywords", v.keywords?.length ? v.keywords : v.palavrasChave]
    : ["Resumo", v.resumo, "Palavras-chave", v.palavrasChave];
  const secundario = en ? ["Resumo", v.resumo, "Palavras-chave", v.palavrasChave]
    : ["Abstract", v.abstract, "Keywords", v.keywords];
  blocoResumo(principal[0], principal[1], principal[2], principal[3], false);
  // o par no outro idioma só sai quando o autor o escreveu (é opcional)
  if (secundario[1] && secundario[1] !== principal[1]) blocoResumo(secundario[0], secundario[1], secundario[2], secundario[3], false);

  /* ---- as seções do trabalho completo ---- */
  if (t.modalidade === "completo" && v.secoes) {
    let n = 0;
    for (const s of secoes) {
      const nome = String(en ? s.en : s.nome).toUpperCase();
      tituloSecao(s.semNumero ? nome : `${++n} ${nome}`);
      corpoRico(v.secoes[s.codigo] || "", { alinhar: s.codigo === "referencias" ? "left" : "justify" });
    }
  }

  /* ---- o rodapé do modelo, em todas as páginas ---- */
  const modal = modalidades.find((m) => m.codigo === t.modalidade)?.nome || "";
  const range = doc.bufferedPageRange();
  for (let i = 0; i < range.count; i++) {
    doc.switchToPage(i);
    const margemAntes = doc.page.margins.bottom;
    doc.page.margins.bottom = 0;
    try {
      doc.moveTo(M, 760).lineTo(M + LARG, 760).lineWidth(2).strokeColor(TR_CIANO).stroke();
      try { doc.image(LOGO_UNIEGO, 34, 782, { height: 30 }); } catch { /* segue sem logo */ }
      try { doc.image(LOGO_PROPPEX, 595.28 - 24 - 46, 766, { height: 55 }); } catch { /* segue sem logo */ }
      doc.font("Times-Roman").fontSize(7.5).fillColor(TR_CINZA)
        .text([modal, t.numero, v.n > 1 ? `versão ${v.n}` : "", `${i + 1}/${range.count}`].filter(Boolean).join(" · "),
          M + 80, 792, { width: LARG - 160, align: "center", lineBreak: false });
    } finally {
      doc.page.margins.bottom = margemAntes;
    }
  }
