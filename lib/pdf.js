...
import { classificarProjetos, normalizarTitulacao, TITULACOES } from "./edital.js";
...
export async function gerarResultadoEditalPdf({ edital = {}, projetos = [], emitidoPor = "" } = {}) {
  ...
  // seleção não deu.
  const { doutores, geral } = classificarProjetos(projetos);
  ...
  doc.font("Helvetica").fontSize(8.5).fillColor("#222").text(
    ...
    + "conduzida fora do sistema. CL é a pontuação do currículo (0 a 100): a da planilha oficial de produção "
    + "acadêmica do coordenador, em valor absoluto. O quadro dos professores doutores atende às "
