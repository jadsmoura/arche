...
export async function gerarResultadoEditalPdf({ edital = {}, projetos = [], emitidoPor = "", fase = "final" } = {}) {
  ...
  const categoriaDe = (p) => {
    ...
    return m
      ? { ordem: MODALIDADES.indexOf(m), rot: `${m.nome}${m.valor ? ` — R$ ${m.valor} mensais` : ""}` }
  ...
  /* -------------- e, no final, a ordem de mérito completa ------------------
     Só no FINAL (pedido do dono, ago/2026): no preliminar os quadros por
     linha já dizem tudo o que existe naquele momento, e repetir as mesmas
     propostas em mais dois quadros só faria o documento cansar quem lê. Os
     dois quadros de mérito importam quando há bolsa a justificar. */
  if (!preliminar) {
    quadro("Classificação — professores doutores", doutores);
    quadro("Classificação geral — todos os professores", geral);
  }
  if (!geral.length) {
    doc.font("Helvetica").fontSize(9.5).fillColor(MUTED)
  ...
  doc.font("Helvetica").fontSize(8.5).fillColor("#222").text(
    (preliminar
      ...
      : "Os quadros iniciais reúnem as propostas pela bolsa concedida a cada uma; "
        + "em seguida vem a ordem de mérito completa. ")
    ...
    + (preliminar ? "" : "O quadro dos professores doutores atende às "
...
/* ======================= termo de compromisso do bolsista ================
   O documento que a PROPPEX entrega ao aluno contemplado, para assinatura
   dele, de quem orienta e da pró-reitoria. Tudo o que ele afirma — projeto,
   vigência, modalidade, valor, dados bancários — já está no ARCHÉ: redigitar
   à mão só cria divergência entre o contrato e o sistema.

   Uma folha por bolsista. Campo que o aluno ainda não preencheu sai como
   linha pontilhada, para não travar a emissão: o termo é impresso e o que
   faltar se completa à caneta na assinatura.
   ======================================================================== */
const LINHA_VAZIA = "............................................................";
...
// idade em anos completos (mesma conta de idadeEm, em lib/ic.js)
function idadeEmPdf(nascimento) {
  const n = String(nascimento || "").slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(n)) return null;
  const [an, mn, dn] = n.split("-").map(Number);
  const h = new Date();
  let idade = h.getFullYear() - an;
  if (h.getMonth() + 1 < mn || (h.getMonth() + 1 === mn && h.getDate() < dn)) idade -= 1;
  return idade >= 0 && idade < 130 ? idade : null;
}

export async function gerarTermoCompromissoPdf({ edital = {}, projetos = [], emitidoPor = "" } = {}) {
  const { doc, fim, quebra, secao, campo } = criarDoc();
  const marca = marcaEm(edital.vigencia?.inicio);
  const rot = (c, lista) => (lista.find((x) => x.codigo === c) || {}).nome || c || "—";
  const vlr = (n) => (typeof n === "number" && n > 0
    ? `R$ ${n.toFixed(2).replace(".", ",")}` : "conforme a tabela do órgão de fomento");

  // parágrafo corrido justificado — o corpo do termo é texto, não formulário
  const par = (t, { negrito = false, tamanho = 9.5 } = {}) => {
    quebra(30);
    doc.font(negrito ? "Helvetica-Bold" : "Helvetica").fontSize(tamanho).fillColor("#222")
      .text(t, M, doc.y, { width: LARG, align: "justify" });
    doc.moveDown(0.45);
    doc.x = M;
  };
  // cláusulas numeradas, com recuo — a numeração é do próprio termo
  const cl = (lista) => lista.forEach((t, i) => {
    quebra(26);
    const y = doc.y;
    doc.font("Helvetica-Bold").fontSize(9).fillColor(TEAL)
      .text(`${i + 1}.`, M + 4, y, { width: 18, lineBreak: false });
    doc.font("Helvetica").fontSize(9).fillColor("#222")
      .text(t, M + 24, y, { width: LARG - 28, align: "justify" });
    doc.moveDown(0.25);
    doc.x = M;
  });

  // uma folha por bolsista: o termo é assinado individualmente
  const folhas = [];
  for (const p of projetos) {
    for (const a of (p.alunos || []).filter((x) => x.bolsista)) folhas.push({ p, a });
  }

  folhas.forEach(({ p, a }, i) => {
    if (i) doc.addPage();
    const mod = modalidadeEfetivaPdf(p);
    const cnpq = p.fomento?.tipo === "cnpq";
    const inicio = p.inicio || edital.vigencia?.inicio;
    const termino = p.fim || edital.vigencia?.fim;

    doc.font("Helvetica-Bold").fontSize(13.5).fillColor(TEAL)
      .text("TERMO DE COMPROMISSO DO BOLSISTA", M, TOPO, { width: LARG, align: "center" });
    doc.font("Helvetica-Bold").fontSize(10).fillColor("#111")
      .text(`${mod?.nome || (cnpq ? "Bolsa CNPq" : "Bolsa UNIEGO")} · Edital nº ${edital.numero || "—"}`,
        M, doc.y + 3, { width: LARG, align: "center" });
    doc.moveDown(0.7); doc.x = M;

    par(`Pelo presente instrumento, o(a) aluno(a) abaixo identificado(a) declara aceitar a bolsa de ${rot(p.linha, LINHAS_FICHA).toLowerCase()} que lhe foi concedida pelo ${marca.nome}, no âmbito do Edital nº ${edital.numero || "—"} da Pró-Reitoria de Pós-Graduação, Pesquisa, Extensão e Ação Comunitária, e assume os compromissos adiante estabelecidos, sob a orientação do(a) professor(a) responsável pelo projeto.`);

    secao("Identificação do bolsista");
    campo("Nome completo", a.nome || LINHA_VAZIA);
    campo("CPF", formatarCpf(a.cpf) || LINHA_VAZIA);
    campo("RG", a.rg || LINHA_VAZIA);
    const idade = idadeEmPdf(a.nascimento);
    campo("Data de nascimento", a.nascimento
      ? `${fmtDataLonga(a.nascimento)}${idade != null ? ` (${idade} anos)` : ""}` : LINHA_VAZIA);
    campo("Endereço", a.endereco || LINHA_VAZIA);
    campo("Curso", a.curso || cursoDe(p.curso)?.nome || p.curso || LINHA_VAZIA);
    campo("Período/semestre", a.periodo || LINHA_VAZIA);
    campo("E-mail", a.email || LINHA_VAZIA);
    campo("Telefone (WhatsApp)", a.telefone || LINHA_VAZIA);
    campo("Possui vínculo empregatício", a.vinculo === "sim"
      ? `sim${a.vinculoOnde ? ` — ${a.vinculoOnde}` : ""}`
      : a.vinculo === "nao" ? "não" : LINHA_VAZIA);

    secao("Projeto e orientação");
    campo("Protocolo", p.numero || "—");
    campo("Título do projeto", p.titulo || "—");
    campo("Linha", rot(p.linha, LINHAS_FICHA));
    campo("Curso do projeto", cursoDe(p.curso)?.nome || p.curso || "—");
    campo("Orientador(a)", p.orientador?.nome || "—");
    campo("Titulação", rot(normalizarTitulacao(p.orientador?.titulacao), TITULACOES) || "—");
    campo("E-mail da orientação", p.orientador?.email || "—");

    secao("Vigência e pagamento");
    campo("Vigência do plano de trabalho", `${fmtDataLonga(inicio)} a ${fmtDataLonga(termino)}`);
    campo("Modalidade", mod?.nome || (cnpq ? "Bolsa CNPq" : "Bolsa UNIEGO"));
    campo("Fomento", cnpq ? "Conselho Nacional de Desenvolvimento Científico e Tecnológico (CNPq)" : marca.nome);
    campo("Valor mensal", cnpq ? "conforme a tabela vigente do CNPq" : vlr(mod?.valor ?? 350));
    campo("Banco", a.banco || LINHA_VAZIA);
    campo("Agência", a.agencia || LINHA_VAZIA);
    campo("Conta corrente", a.conta || LINHA_VAZIA);
    campo("Chave Pix (vinculada à conta)", a.pix || LINHA_VAZIA);
    doc.font("Helvetica-Oblique").fontSize(7.5).fillColor(MUTED)
      .text("A conta informada deve ser de titularidade do próprio bolsista — não se paga bolsa em conta de terceiros.",
        M + 4, doc.y + 2, { width: LARG - 8 });
    doc.moveDown(0.3); doc.x = M;

    secao("Compromissos do bolsista");
    cl([
      "Executar o plano de trabalho aprovado, sob a orientação do(a) professor(a) responsável, dedicando-lhe a carga horária prevista no edital.",
      "Entregar o relatório parcial e o relatório final nos prazos do edital, pelo sistema ARCHÉ, com a validação da orientação.",
      "Apresentar os resultados do projeto nos eventos científicos institucionais, quando convocado.",
      "Fazer referência à sua condição de bolsista e à instituição em toda publicação ou trabalho apresentado que resulte do projeto.",
      "Comunicar imediatamente à orientação e à PROPPEX qualquer alteração de matrícula, trancamento, transferência, conclusão do curso ou impedimento de continuar as atividades.",
      "Não acumular esta bolsa com outra de qualquer natureza, bolsa de estágio remunerado ou vínculo empregatício, ressalvado o que o edital expressamente permitir.",
      "Manter atualizados, no sistema, os seus dados cadastrais e bancários.",
      "Devolver os valores recebidos indevidamente, no caso de descumprimento dos compromissos aqui assumidos.",
    ]);

    secao("Compromissos da orientação");
    cl([
      "Orientar o bolsista nas distintas fases do trabalho, incluindo a elaboração dos relatórios e a apresentação dos resultados.",
      "Acompanhar o cronograma no sistema e validar os relatórios entregues pelo bolsista.",
      "Comunicar à PROPPEX qualquer fato que altere a execução do plano de trabalho ou que justifique a substituição do bolsista.",
    ]);

    secao("Vigência e desligamento");
    par("A bolsa vigora pelo período indicado neste termo e pode ser encerrada, a qualquer tempo, por solicitação do bolsista, por pedido fundamentado da orientação ou por decisão da PROPPEX, no caso de descumprimento dos compromissos acima. O desligamento não gera direito a indenização, e os valores já pagos referentes a período não trabalhado devem ser devolvidos.");
    par("E, por estarem de acordo, as partes firmam o presente termo.");

    quebra(120);
    doc.font("Helvetica").fontSize(9.5).fillColor("#222")
      .text(`${CIDADE_PDF}, ${hojeExtenso()}.`, M, doc.y + 6, { width: LARG, align: "right" });

    blocoAssinaturas(doc, quebra, [
      { nome: a.nome || "", cargo: "Bolsista" },
      { nome: p.orientador?.nome || "", cargo: "Orientador(a)" },
      ASSINA.proReitor,
    ]);
  });
  ...
  if (!folhas.length) {
    doc.font("Helvetica-Bold").fontSize(13.5).fillColor(TEAL)
      .text("TERMO DE COMPROMISSO DO BOLSISTA", M, TOPO, { width: LARG, align: "center" });
    doc.font("Helvetica").fontSize(9.5).fillColor(MUTED)
      .text(`Nenhum bolsista com bolsa concedida no Edital nº ${edital.numero || "—"}. Os termos saem depois da distribuição das bolsas, na guia Bolsas.`,
