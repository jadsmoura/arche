...
import { classificarProjetos, normalizarTitulacao, TITULACOES, EDITAL, MODALIDADES } from "./edital.js";
...
export async function gerarResultadoEditalPdf({ edital = {}, projetos = [], emitidoPor = "", fase = "final" } = {}) {
  ...
  /* ---------------------- quadros por categoria de bolsa ------------------
     A apresentação principal do documento (pedido do dono, ago/2026): quem lê
     o resultado quer saber quem ficou com cada modalidade. No FINAL a
     categoria é a bolsa concedida; no PRELIMINAR ela ainda não existe, então
     o agrupamento é pela modalidade PRETENDIDA — e o rótulo diz isso, para o
     documento não prometer bolsa que a cota ainda não definiu. */
  const categoriaDe = (p) => {
    if (preliminar) {
      const m = MODALIDADES.find((x) => x.codigo === p.modalidade);
      return m ? { ordem: MODALIDADES.indexOf(m), rot: `Modalidade pretendida — ${m.nome}` }
        : { ordem: 90, rot: "Sem modalidade pretendida indicada" };
    }
  ...
  doc.font("Helvetica").fontSize(8.5).fillColor("#222").text(
    "Os quadros iniciais reúnem as propostas por categoria de bolsa"
    + (preliminar ? " pretendida — a concessão será divulgada no resultado final" : " concedida")
