...
/* ========================================================================
   ...
   ======================================================================== */
// O timbrado é o mesmo dos demais documentos da pró-reitoria: o nome completo
// do órgão não cabe na faixa sem passar por cima do logotipo.
const TIMBRE_MON = ["PRÓ-REITORIA DE PESQUISA", "E EXTENSÃO"];
...
export async function gerarEditalMonitoriaPdf({ edital = {}, texto = {}, cronograma = [] } = {}) {
  ...
  doc.font("Helvetica").fontSize(10).fillColor(MUTED)
    .text("PROPPEX", M, doc.y + 2, { width: LARG, align: "center" });
  ...
  doc.font("Helvetica").fontSize(9.5).fillColor("#222")
    .text(texto.abertura || "", M, doc.y + 6, { width: LARG, align: "justify", lineGap: 3 });
  ...
  if (cronograma.length) {
    ...
    const linha = (a, b, c, cab = false) => {
      quebra(20);
      const y = doc.y;
      if (cab) doc.rect(M, y, LARG, 16).fill(CINZA);
      doc.font(cab ? "Helvetica-Bold" : "Helvetica").fontSize(8.5).fillColor(cab ? TEAL : "#222");
      doc.text(a, M + 4, y + 4, { width: cols[0] - 8 });
      doc.text(b, M + cols[0] + 4, y + 4, { width: cols[1] - 8 });
      doc.text(c, M + cols[0] + cols[1] + 4, y + 4, { width: cols[2] - 8 });
      const h = Math.max(16, doc.y - y + 4);
...
export async function gerarProjetoMonitoriaPdf(p = {}) {
  ...
  if (mons.length) {
    ...
    mons.forEach((m, i) => {
      ...
      if (mons.length > 1 || m.planoTrabalho) {
        doc.font("Helvetica-Bold").fontSize(9).fillColor("#111")
          .text("Plano de trabalho:", M, doc.y + 3, { width: LARG });
        texto(m.planoTrabalho);
      }
    });
  }
  ...
  blocoAssinaturas(doc, quebra, [
    { nome: nn(p.orientador?.nome), cargo: "Professor(a) Orientador(a)" },
    ASSINA.proReitor,
  ]);
...
export async function gerarFichaMonitoriaPdf(p = {}, m = {}) {
  const marca = marcaEm(p.fim);
  const { doc, fim, quebra, secao, campo, texto } = criarDoc();
  ...
  if (m.planoTrabalho) { secao("PLANO DE TRABALHO"); texto(m.planoTrabalho); }

  secao("3 — DECLARAÇÃO DE DISPONIBILIDADE DE CARGA HORÁRIA");
  ...
  secao("4 — RELAÇÃO DE DOCUMENTOS ANEXADOS");
  campo("Comprovante de matrícula", m.documentos?.matricula ? "anexado no ARCHÉ" : "não anexado");
  campo("Histórico escolar da graduação", m.documentos?.historico ? "anexado no ARCHÉ" : "não anexado");
  campo("Ficha de inscrição preenchida", "este documento");

  quebra(70);
  doc.font("Helvetica").fontSize(9.5).fillColor("#222")
    .text(`${CIDADE_PDF}-GO, ${fmtDataLonga(soDia(m.cadastradoEm)) || hojeExtenso()}.`,
      M, doc.y + 18, { width: LARG, align: "center" });
  blocoAssinaturas(doc, quebra, [{ nome: nn(m.nome), cargo: "Assinatura do(a) Aluno(a)" }]);
...
export async function gerarRelatorioMonitoriaPdf(p = {}, m = {}, { criterios = [] } = {}) {
  const marca = marcaEm(p.fim);
  const { doc, fim, quebra, secao, campo, texto } = criarDoc();
  const r = m.relatorio || {};
  ...
  doc.font("Helvetica").fontSize(9.5).fillColor("#222")
    .text(`${CIDADE_PDF}-GO, ${fmtDataLonga(soDia(r.validadoEm || r.enviadoEm)) || hojeExtenso()}.`,
      M, doc.y + 18, { width: LARG, align: "center" });
  blocoAssinaturas(doc, quebra, [
    { nome: nn(m.nome), cargo: "Monitor(a) Acadêmico(a)" },
    { nome: nn(p.orientador?.nome), cargo: "Professor(a) Orientador(a)" },
  ]);
