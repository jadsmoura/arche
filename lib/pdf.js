...
function timbrado(doc) {
  ...
  doc.font("Helvetica").fontSize(9).fillColor(MUTED)
    ...
    .text("Coordenadoria de Extensão", M + 44, 74);
  doc.moveTo(M, 92).lineTo(M + LARG, 92).lineWidth(1).strokeColor(CYAN).stroke();
}
function rodape(doc) {
  doc.moveTo(M, RODAPE_Y).lineTo(M + LARG, RODAPE_Y).lineWidth(0.7).strokeColor(LINE).stroke();
  doc.font("Helvetica").fontSize(7.5).fillColor(MUTED)
    .text("ARCHÉ · PROPPEX / UNIEGO · Goianésia – GO", M, RODAPE_Y + 6, { width: LARG, align: "left" });
}
