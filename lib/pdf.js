...
export async function gerarTrabalhoPdf({ trabalho = {}, evento = {}, versao = null, anonimo = false,
  secoes = [], rotuloTitulacao = (x) => x, modalidades = [], capa = null } = {}) {
  const t = trabalho;
  ...
  const paragrafo = (s, { alinhar = "justify", italico = false } = {}) => {
  ...
  const blocoResumo = (titulo, texto, rotuloPc, pcs, italico) => {
    if (!texto) return;
    tituloSecao(titulo);
    paragrafo(texto, { italico });
  ...
  if (t.modalidade === "completo" && v.secoes) {
    ...
    for (const s of secoes) {
      ...
      paragrafo(v.secoes[s.codigo] || "—", { alinhar: s.codigo === "referencias" ? "left" : "justify" });
    }
  }
