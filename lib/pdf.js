...
/* Assinatura digitalizada, usada SÓ nos certificados (decisão do dono,
   ...
   para assinatura à mão. O arquivo é opcional — sem ele, sai a linha, e o
   documento não quebra. Para trocar a assinatura de alguém, é substituir o
   PNG (fundo transparente, traço escuro). */
const ASSINATURA_IMG = {
  reitor: path.join(__dirname, "..", "templates", "assinatura-reitor.png"),
  proReitor: path.join(__dirname, "..", "templates", "assinatura-proreitor.png"),
};
...
export async function gerarCertificadoPdf(cert = {}) {
  ...
  const assina = [
    { ...ASSINA.proReitor, img: ASSINATURA_IMG.proReitor },
    { ...ASSINA.reitor, img: ASSINATURA_IMG.reitor },
  ];
...
export async function gerarProjetosPdf({ projetos = [], titulo = "", filtros = "", emitidoPor = "" } = {}) {
  ...
  for (const p of projetos) {
    ...
    doc.font("Helvetica").fontSize(8.5).fillColor(MUTED)
      .text(`${rot(p.status, STATUS_FICHA)} · edital ${p.edital || "—"}`, M, doc.y + 1, { width: LARG });
