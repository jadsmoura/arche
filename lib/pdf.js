...
export async function gerarRelatorioPdf(acao, { fotos = [], assinaturas = {} } = {}) {
  ...
  const ass = (linhaTopo, nome, cargo, img) => {
    ...
    const y = doc.y;
    if (img) {
      try { doc.image(img, M + LARG / 2 - 48, y - 42, { fit: [96, 38], align: "center" }); }
...
export async function gerarPropostaPdf(acao, { assinaturas = {} } = {}) {
  ...
  if (assinaturas.responsavel) {
    try { doc.image(assinaturas.responsavel, M + LARG / 2 - 48, y - 42, { fit: [96, 38], align: "center" }); }
    catch { /* sem a imagem, sai só a linha */ }
  }
  doc.moveTo(M + 95, y).lineTo(M + LARG - 95, y).lineWidth(0.8).strokeColor("#333").stroke();
  doc.font("Helvetica").fontSize(8.5).fillColor(MUTED)
...
/* ========================================================================
   ...
   ======================================================================== */
export async function gerarResultadoMonitoriaPdf({ edital = {}, projetos = [], emitidoPor = "" } = {}) {
  const { doc, fim, quebra, secao } = criarDoc();
  ...
  blocoAssinaturas(doc, quebra, [ASSINA.proReitoraAcademica, ASSINA.reitor]);
  if (emitidoPor) {
    doc.font("Helvetica").fontSize(7.5).fillColor(MUTED)
      .text(`Documento emitido por ${emitidoPor}.`, M, doc.y + 8, { width: LARG, align: "center" });
  }
...
export async function gerarEditalMonitoriaPdf({ edital = {}, texto = {}, cronograma = [], acessos = [],
  assinam = [ASSINA.proReitoraAcademica, ASSINA.proReitor, ASSINA.reitor] } = {}) {
  ...
  blocoAssinaturas(doc, quebra, assinam.length ? assinam : [ASSINA.proReitor, ASSINA.reitor]);
...
export async function gerarProjetoMonitoriaPdf(p = {}, { campos = [] } = {}) {
...
export async function gerarFichaMonitoriaPdf(p = {}, m = {}, { campos = [] } = {}) {
  const marca = marcaEm(p.fim);
  const { doc, fim, quebra, secao, campo, texto } = criarDoc();
...
export async function gerarRelatorioSemestralPdf({ setor = {}, periodo = {}, panorama = {},
  emitidoPor = "", marca = MARCAS.uniego } = {}) {
  const { doc, fim, quebra, secao, campo, texto } = criarDoc();
  ...
  blocoAssinaturas(doc, quebra, [ASSINA.proReitor]);
  if (emitidoPor) {
    quebra(24);
    doc.font("Helvetica").fontSize(7).fillColor(MUTED)
...
export async function gerarRelatorioAulaPraticaPdf({
  ...
  doc.font("Helvetica-Bold").fontSize(14).fillColor(TEAL)
    .text("RELATÓRIO DE AULA PRÁTICA", M, TOPO, { width: LARG, align: "center" });
  doc.font("Helvetica").fontSize(9).fillColor(MUTED)
  ...
  campo("Professor(a)", professor.nome || r.professor?.nome);
  campo("Curso", curso);
  campo("Disciplina", r.disciplina);
  campo("Data da aula", fmtDataLonga(r.data));
  campo("Local", r.local);
  ...
  paginasDeFotos(doc, fotos, { M, LARG, RODAPE_Y, TOPO,
    legenda: `${fotos.length} imagem(ns) da aula prática, na ordem em que foram anexadas.` });
...
export async function gerarProducaoDocentePdf({ curso = "", cursoNome = "", dossie = {} } = {}) {
  const { doc, fim, quebra, secao } = criarDoc();
