...
/* ========================================================================
   ...
   As assinaturas chegam prontas de quem chamou (lib/certificadosEv.js
...
export async function gerarCertificadoEventoPdf({ cert = {}, programacao = [], assinaturas = [] } = {}) {
  ...
  const corpo = cert.tipo === "palestrante"
    ? `Certificamos que ${String(cert.pessoa || "").toUpperCase()}${cert.cpf ? `, inscrito(a) no CPF nº ${formatarCpfPdf(cert.cpf)},` : ","} `
      + `atuou como PALESTRANTE no evento “${cert.evento || ""}”, promovido ${pelaInst} ${marca.sigla}, `
      + `${ondeQuando}${horas}${cert.palestra ? `, com a apresentação intitulada “${cert.palestra}”` : ""}.`
    : cert.tipo === "comissao"
      ? `Certificamos que ${String(cert.pessoa || "").toUpperCase()}${cert.cpf ? `, inscrito(a) no CPF nº ${formatarCpfPdf(cert.cpf)},` : ","} `
        + `integrou a COMISSÃO ORGANIZADORA do evento “${cert.evento || ""}”${cert.papel ? `, na função de ${cert.papel}` : ""}, `
        + `promovido ${pelaInst} ${marca.sigla}, ${ondeQuando}${horas}.`
      : `Certificamos que ${String(cert.pessoa || "").toUpperCase()}${cert.cpf ? `, inscrito(a) no CPF nº ${formatarCpfPdf(cert.cpf)},` : ","} `
        + `participou do evento “${cert.evento || ""}”, promovido ${pelaInst} ${marca.sigla}, `
        + `${ondeQuando}${horas}.`;
