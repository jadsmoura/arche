...
/* ========================================================================
   ...
   gerarResultadoEditalPdf({ edital, projetos, emitidoPor }) -> Promise<Buffer>
   ======================================================================== */
const ROTULO_LINHA = { ic: "IC", it: "IT", ie: "IE" };

export async function gerarResultadoEditalPdf({ edital = {}, projetos = [], emitidoPor = "" } = {}) {
  const { doc, fim, quebra, secao } = criarDoc();
  const numero = edital.numero || "—";

  /* -------------------------------- cabeçalho ----------------------------- */
  doc.font("Helvetica-Bold").fontSize(15).fillColor(TEAL)
    .text("RESULTADO DO PROCESSO SELETIVO", M, TOPO, { width: LARG, align: "center" });
  doc.moveDown(0.3);
  doc.font("Helvetica-Bold").fontSize(11).fillColor("#111")
  ...
  doc.font("Helvetica-Oblique").fontSize(8).fillColor(MUTED)
    .text(`${comBolsa} proposta(s) contemplada(s) com bolsa.`, M + 4, doc.y + 3, { width: LARG - 8 });

  /* ------------------------------ classificação --------------------------- */
  // Ordem do resultado: quem tem nota vem antes, da maior para a menor; o
  // resto segue pelo protocolo, para o documento não sugerir posição que a
  // seleção não deu.
  const nfcDe = (p) => (p.classificacao?.np != null ? p.classificacao.nfc : undefined);
  const ordenados = [...projetos].sort((a, b) => {
    const na = nfcDe(a), nb = nfcDe(b);
    if (typeof na === "number" && typeof nb === "number" && na !== nb) return nb - na;
    if (typeof na === "number" !== (typeof nb === "number")) return typeof na === "number" ? -1 : 1;
    return String(a.numero || "").localeCompare(String(b.numero || ""));
  });

  secao("Propostas e resultado");
  const COL = { n: 0.13, t: 0.42, o: 0.23, c: 0.10, f: 0.12 };
  const cab = () => {
    quebra(20);
    const y = doc.y;
    ...
    let x = M + 4;
    for (const [rot, w] of [["Protocolo", COL.n], ["Título", COL.t], ["Orientação", COL.o],
      ["NFC", COL.c], ["Resultado", COL.f]]) {
      doc.text(rot.toUpperCase(), x, y + 4.5, { width: LARG * w - 6, lineBreak: false });
      x += LARG * w;
    }
    doc.y = y + 18; doc.x = M;
  };
  ...
  for (const p of ordenados) {
    ...
    doc.font("Helvetica").fontSize(8);
    const alt = Math.max(
      doc.heightOfString(titulo, { width: LARG * COL.t - 8 }),
      doc.heightOfString(orient, { width: LARG * COL.o - 8 }),
      doc.heightOfString(resultado, { width: LARG * COL.f - 6 }), 11) + 4;
    quebra(alt + 4);
    if (doc.y < TOPO + 6) cab();          // recomeçou a página: repete o cabeçalho
    const y = doc.y;
    let x = M + 4;
    const celula = (txt, w, opc = {}) => {
      doc.text(txt, x, y, { width: LARG * w - 8, ...opc });
      x += LARG * w;
    };
    ...
    // NFC só quando há parecer: sem NP a conta daria uma nota que a seleção
    // ainda não apurou, e num documento impresso ela passaria por definitiva.
    celula(p.classificacao?.np != null ? String(p.classificacao.nfc) : "—", COL.c, { lineBreak: false });
    ...
    doc.font("Helvetica").fontSize(7).fillColor(MUTED)
      .text(`${(p.curso || "—")} · ${ROTULO_LINHA[p.linha] || p.linha || "—"}` +
        ...
        M + 4 + LARG * COL.n, y + alt - 3, { width: LARG * (COL.t + COL.o) - 8 });
    doc.y = y + alt + 6; doc.x = M;
    doc.moveTo(M, doc.y - 3).lineTo(M + LARG, doc.y - 3).lineWidth(0.4).strokeColor(LINE).stroke();
  }
  if (!ordenados.length) {
    doc.font("Helvetica").fontSize(9.5).fillColor(MUTED)
      .text("Nenhuma proposta registrada para este edital.", M + 4, doc.y, { width: LARG });
  }

  /* -------------------------------- critério ------------------------------ */
  secao("Como a classificação foi apurada");
  doc.font("Helvetica").fontSize(8.5).fillColor("#222").text(
    "NFC = (NP × 0,6) + (CL × 0,4), conforme o item 9.4 do edital, onde NP é a média das notas dos "
    + "pareceres (0 a 10) e CL é a pontuação da produção acadêmica do coordenador, apurada pela planilha "
    + "oficial (0 a 100) e convertida para a escala de 0 a 10. Proposta ainda sem parecer entregue "
    + "aparece sem NFC; planilha não preenchida entra como CL zero.",
  ...
  doc.font("Helvetica").fontSize(8.5).fillColor(MUTED)
    .text("Pró-Reitoria de Pós-Graduação, Pesquisa, Extensão e Ação Comunitária",
      M, y + 3, { width: LARG, align: "center" });
  if (emitidoPor) {
    doc.font("Helvetica").fontSize(7.5).fillColor(MUTED)
      .text(`Documento emitido por ${emitidoPor}.`, M, doc.y + 2, { width: LARG, align: "center" });
  }
